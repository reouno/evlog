...
impl Food {
    ...
    /// gets WATER_RAIN from the sky. `delta` and `delta_soil` are scratch. Returns the water
    /// moved and the soil leached.
    fn water_step(&mut self, g: Grid, height: &[f32], rate: f64, leach: f64, depth: f64, delta: &mut [f64], delta_soil: &mut [f64]) -> (f64, f64) {
        ...
        for ((wtr, d), (soil, ds)) in self.water.iter_mut().zip(delta.iter()).zip(self.soil.iter_mut().zip(delta_soil.iter())) {
            *wtr = (*wtr + d) * (1.0 - WATER_EVAP) + WATER_RAIN;
...
fn main() {
    ...
    let motor: u8 = args.get(44).and_then(|s| s.parse().ok()).unwrap_or(0);
    ...
    let prefix = format!("experiments/e048_motor/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}{sat_tag}{hold_tag}{wear_tag}{connect_tag}{yield_tag}{motor_tag}_seed{seed}", rain.name());
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height,fat,size,density,digest,side,water,worn,travel").unwrap();
    ...
    for step in 1..=steps {
        ...
        if water_rate > 0.0 {
            let (_, ls) = food.water_step(g, &terrain.height, water_rate, leach, depth, &mut delta, &mut delta_soil);
            leached += ls;
            if mix > 0.0 {
        ...
        if step % AGENT_DUMP_INTERVAL == 0 {
            ...
            for a in &agents {
                ...
                write!(agents_csv, ",{:.3},{},{:.3},{:.3},{},{:.3},{},{:.2}", a.fat, b.size, b.density, b.digest, b.side, a.water, a.worn, a.travel(g)).unwrap();
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.4},{:.4}", stalled as f64 / tried, sub_moved as f64 / LOG_INTERVAL as f64 / pop as f64).unwrap();
