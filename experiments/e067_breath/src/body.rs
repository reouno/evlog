...
impl Laws {
    pub fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        let morph_level = (0..=SIDE_MAX)
            ...
            .collect();
        Laws { morphogen, table, density, side, water, breath, morph_level }
    }
}
...
impl Body {
    pub fn new(cells: [u8; CELLS], side: usize, policy: [f32; N_POLICY], n_genes: u16, density: f32) -> Self {
        let mut b = Body { cells, side: side as u8, size: 0, mass: 0.0, density, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE_MAX]; 4], extent: [0; 2], policy, n_genes, cut: 0, water_policy: [0.0; N_WATER_POLICY], open_soft: 0, breath_policy: [0.0; N_OUT] };
        b.refresh();
        b
    }
    pub fn empty() -> Self {
...
impl Agent {
    ...
    pub fn new(id: u64, genome: Vec<u8>, keys: Vec<u16>, gene_ids: Vec<u16>, body: Body, facing: u8, energy: f64, lineage: u32) -> Agent {
        let mut a = Agent {
            ...
            wcells: [0; CELLS], tips: [[Tip::default(); SIDE_MAX]; 4], filled: [0; CELLS], n_filled: 0, bbox: [0; 4], open: [0; CELLS], water: 1.0, drank: 0, breath: 1.0, inhaled: 0,
        };
        a.set_body(body);
        a
    }
