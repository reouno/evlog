...
params! {
    ...
    // e093's light (#103, P3 step 1), 0 being e092's control world
    light_gain = 0.0, "e093 (#103): matter a leaf block takes a step per face open to the air, times the light a block of its body reads on the cell it stands over, out of the soil of that cell (0: no body develops a leaf block and the world is e092's)";
    // e096's shade (#108, P3 step 3), 0 leaving the light granted to everything that stands in it
    shade = 0.0, "e096 (#108): the sub-cells a block's shadow covers, of the SUB^2 a cell holds - the cell's light becomes one flux, the ground grows at what the blocks over it leave and those blocks divide what they took (0: no shade, and a leaf block reads the whole light of its cell as in e093)";
}
...
const BODY_KEYS: [&str; 53] = [
    ...
    // e093's (#103): the light a leaf block lives by; e096's (#108) shade, which divides it
    "light_gain", "shade",
];
...
/// e096 (#108): the cell's light as one flux. A block's shadow covers `shade` of the SUB^2 sub-cells
/// a cell holds, so `n` blocks standing over it darken `min(1, shade * n / SUB^2)` of its light: that
/// is the share the ground loses, and it is lost, not moved (as a dry cell's light is). The blocks
/// divide what they darkened evenly - `block_light` is one block's part of it, counted in the light
/// of a block's own footprint (1 is what e093 granted every face). So at `shade` 1 a block alone on
/// its cell reads e093's grant and a cell carrying twice the blocks its face holds halves it; the
/// two together conserve the cell's light, which `light_is_conserved` checks.
fn shaded(shade: f64, n: u32) -> f64 {
    (shade * n as f64 / SUB_CELLS as f64).min(1.0)
}

fn block_light(shade: f64, n: u32) -> f64 {
    if n == 0 {
        0.0
    } else {
        shade.min(SUB_CELLS as f64 / n as f64)
    }
}
...
struct Tally {
    ...
    light: f64,        // e093 (#103): what leaf blocks took from the soil by the light, in a body's units
...
struct Sim {
    ...
    sun: Vec<f32>,      // e093 (#103): the light itself on the cell, 0 to 1 - the same quantity `vis` damps
...
impl Sim {
    ...
    fn with_pools(p: Params, w: World, pl: Plants, update_i: u64, threads: usize, pools: Pools) -> Sim {
        ...
        laws.leaf_on = p.light_gain > 0.0; // e093 (#103): off, no cell develops a leaf block
        ...
        let mut sim = Sim {
            ...
            wet: vec![false; n * n], dryness: vec![0.0; n * n], drink_at: vec![0.0; n * n], vis: vec![1.0; n * n], sun: vec![0.0; n * n], air_sum: vec![0.0; n * n], ground_sum: vec![0.0; n * n], air_n: 0, land,
    ...
    fn refresh_air(&mut self, count: bool) {
        ...
        let light_gain = self.p.light_gain;
        ...
        for c in 0..self.w.n * self.w.n {
            ...
            if light > 0.0 || light_gain > 0.0 {
                let lit = (self.w.light[c] / (1.0 + depth(&self.w, &self.p, c).unwrap_or(0.0) / LIGHT_DEPTH) * (1.0 - self.pl.wood[c] / (self.pl.wood[c] + WOOD_HALF))).clamp(0.0, 1.0);
                if light > 0.0 {
                    self.vis[c] = (1.0 - light * (1.0 - lit)) as f32;
                }
                // e093 (#103): a leaf block lives by the light itself, not by the reach an eye has in it.
                self.sun[c] = lit as f32;
            }
            if count {
                self.air_sum[c] += air;
    ...
    fn world(&mut self) {
        ...
        // e096 (#108): the shade the bodies cast on the ground of every cell, as they stand now -
        // one pass over the cells off the occupancy's own per-cell block counts, so the law costs
        // no pass over the bodies (the producers' update is already a pass over the cells).
        if self.p.shade > 0.0 {
            for c in 0..self.pl.shade_left.len() {
                self.pl.shade_left[c] = (1.0 - shaded(self.p.shade, self.occ.blocks_over(c))) as f32;
            }
        }
    ...
    fn bodies(&mut self) {
        let Sim { p, w, agents, pl, occ, g, rng, wear_rng, k, hab, wet: wet_cells, dryness, drink_at, vis, sun, laws, cache, threads, next_id, born, dead, wsea, .. } = self;
        let g = *g;
        ...
        let light_gain = p.light_gain; // e093 (#103): what a leaf block's open face takes in full light
        let shade = p.shade; // e096 (#108): the shadow a block casts, which divides that light
        ...
        for i in 0..agents.len() {
            ...
            // e093 (#103): every leaf block takes `light_gain` per face open to the air, times the
            // light it reads on the cell it stands over, straight out of that cell's soil - the same
            // store a producer grows out of, so the ledger closes and the body competes with the
            // grass under it. The soil gives what it has; the faces that gain are the faces that lose
            // water to dry air (e067), pass heat (e072) and are what a tooth breaks (e075).
            // e096 (#108): the light it reads is its share of its cell's flux (`block_light`), so
            // every block standing on that cell - its own body's and any other's - cuts it; with
            // `shade` 0 it reads the whole light of the cell, as in e093.
            let mut lit_in = 0.0f64;
            if light_gain > 0.0 {
                for p in a.cells_held() {
                    if a.wcells[p] != LEAF as u8 || a.open[p] == 0 {
                        continue;
                    }
                    let (sx, sy) = a.sub_at(g, p, NORTH, 0);
                    let c = g.wcell(sx, sy);
                    let lit = if shade > 0.0 { sun[c] as f64 * block_light(shade, occ.blocks_over(c)) } else { sun[c] as f64 };
                    let want = light_gain * a.open[p] as f64 * lit * s;
                    let got = pl.take_soil(c, want);
                    lit_in += got / s;
                    if open && w.sea[c] {
                        from_sea += got / s;
                    }
            ...
            let eaten = plant + scavenged + lit_in;
    ...
    const LOG_HEADER: &'static str = "step,pop,grown,births,children,sexual,no_room,empty,deaths_hunger,deaths_broken,deaths_wear,cells_broken,contacts,\
        ...
        leaf_mean,leaf_open,light_intake,leaf_led,open_mean,per_cell,\
        blocks_cell,shaded,grass_under,grass_free";
    ...
    fn log_row(&mut self, steps: u64, matter0: f64, wall: f64) -> (String, Point) {
        ...
        // e093 (#103): the leaf blocks a body holds and the faces of theirs open to the air, what the
        // light gave (a body's units a step, as `fruit_intake`; `plant_intake` is the world's matter
        // over the interval), the share of grown bodies that took
        // most of their life's matter from it, the open soft faces of a body (the shape), and the
        // bodies per cell that holds one (the crowd, read where the bodies are).
        let leaf_open = agents
            .iter()
            .map(|a| a.cells_held().filter(|&q| a.wcells[q] == LEAF as u8).map(|q| a.open[q] as f64).sum::<f64>())
            .sum::<f64>()
            / n;
        let led = grown
            .iter()
            .filter(|a| {
                let all = a.plant as f64 + a.scavenged as f64 + a.killed as f64 + a.light as f64;
                all > 0.0 && a.light as f64 / all > 0.5
            })
            .count() as f64
            / grown.len().max(1) as f64;
        let mut held = vec![false; g.cells()];
        for a in agents.iter() {
            held[a.here(g)] = true;
        }
        let cells_held = held.iter().filter(|&&h| h).count().max(1) as f64;
        s.push_str(&format!(
            ",{:.2},{:.2},{:.4},{:.3},{:.2},{:.3}",
            mean(&|a| a.body.kinds[LEAF] as f64),
            leaf_open,
            k.light / per,
            led,
            mean(&|a| a.body.open_soft as f64),
            pop as f64 / cells_held
        ));
        // e096 (#108): where the bodies stand - the blocks over such a cell and the share of its
        // light they take - and the grass standing on the land they stand on against the grass on
        // the land they leave alone (done-when 4: the grass is not driven out from under them).
        let (mut blocks, mut dark, mut cells_over) = (0u64, 0.0f64, 0u64);
        let (mut gu, mut nu, mut gf, mut nf) = (0.0f64, 0u64, 0.0f64, 0u64);
        for c in 0..g.cells() {
            let over = self.occ.blocks_over(c);
            if over > 0 {
                blocks += over as u64;
                dark += shaded(self.p.shade, over);
                cells_over += 1;
            }
            if depth(&self.w, &self.p, c).is_none() {
                if over > 0 {
                    gu += self.pl.grass[c];
                    nu += 1;
                } else {
        ...
        s.push_str(&format!(
            ",{:.2},{:.3},{:.4},{:.4}",
            blocks as f64 / cells_over.max(1) as f64,
            dark / cells_over.max(1) as f64,
            gu / nu.max(1) as f64,
            gf / nf.max(1) as f64
        ));
    ...
    const AGENT_HEADER: &'static str = "step,id,lineage,age,turns,mass,size,side,density,born_size,born_mass,born_hard,born_muscle,born_sensor,born_digestive,born_leaf,born_bite,\
        hard,muscle,sensor,digestive,leaf,leaf_open,light,bite,bite_any,shell,speed,energy,fat,plant,meat,killed,scavenged,travel,path,worn,pace,place,born_place,temp,moist,height,medium,algae,detritus,water,drank,open_soft,breath,inhaled,breed,share,store,kids,btemp,warmed,cooled,wood,load,invader,cell,crown,seed,fermented,dunged,fiber,fruit,cells";
    ...
    fn agent_rows(&self, out: &mut dyn Write) {
        ...
        for a in &self.agents {
            ...
            writeln!(
                out,
                "{},{},{},{},{},{:.2},{},{},{:.3},{},{:.2},{},{},{},{},{},{},{},{},{},{},{},{},{:.3},{},{},{:.2},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.2},{:.2},{},{:.3},{},{},{:.1},{:.3},{:.0},{},{:.3},{:.3},{:.3},{},{},{:.3},{},{:.4},{:.4},{:.3},{},{:.2},{:.4},{:.4},{:.3},{:.2},{},{},{:.3},{:.3},{:.3},{:.3},{:.4},{:.3},{}",
                self.step,
                a.id,
                a.lineage,
                a.age,
                ...
                b.kinds[DIGESTIVE],
                b.kinds[LEAF],
                a.cells_held().filter(|&q| a.wcells[q] == LEAF as u8).map(|q| a.open[q] as u32).sum::<u32>(),
                a.light,
                b.bite(),
                b.bite_any(),
                b.shell(),
...
fn main() {
    ...
    let mut view = viewer::View::from_env(
        ...
        viewer::Init {
            ...
            blocks: vec!["empty", "hard", "muscle", "sensor", "digestive", "leaf"],
...
mod tests {
    ...
    fn conserved_at(scale: f64, water: f64, dry: f64, breath: f64, history: f64, senses: f64, sets: bool) {
        ...
        if sets {
            ...
            p.fruit_fall = 0.5;
            p.hold = 20.0;
            // e093 (#103): the light, well over the rates the ladder will look at.
            p.light_gain = 0.02;
            // e096 (#108): the shade, over the top of its ladder - the light lost to it is lost, not
            // moved, so the matter of the world must still close.
            p.shade = 4.0;
        }
        ...
        if sets {
            ...
            assert!(sim.k.light > 0.0, "no leaf block took any light");
            assert!(sim.agents.iter().any(|a| a.body.kinds[LEAF] > 0), "no body developed a leaf block");
    ...
    /// e093 (#103): a leaf block takes `light_gain` per face open to the air, times the light on the
    /// cell that face's block stands over, out of that cell's soil - and no more than the soil holds.
    /// With the light off no genome develops a leaf block at all.
    #[test]
    fn a_leaf_block_takes_the_light_out_of_the_soil() {
        // The soil gives what is asked, and when it is short it gives what it has and goes to zero.
        let p0 = small();
        let (w0, mut pl0, _) = settled_world(&p0, None);
        let c = (0..w0.n * w0.n).find(|&c| !w0.sea[c]).unwrap();
        pl0.soil[c] = 0.5;
        assert!((pl0.take_soil(c, 0.2) - 0.2).abs() < 1e-12 && (pl0.soil[c] - 0.3).abs() < 1e-12);
        assert!((pl0.take_soil(c, 10.0) - 0.3).abs() < 1e-12 && pl0.soil[c] == 0.0);
        ...
        let mut sim = Sim::new(p, w, pl, u, 1);
        let g = sim.g;
        // One body, so that nothing shoves it and what the light gave is its own.
        let i = sim
            .agents
            .iter()
            .position(|a| a.body.kinds[LEAF] > 0 && a.cells_held().any(|q| a.wcells[q] == LEAF as u8 && a.open[q] > 0))
            .expect("no body developed a leaf block");
        sim.agents.swap(0, i);
        let rest = sim.agents.split_off(1);
        for a in &rest {
            sim.occ.release(g, a);
        }
        sim.occ.relabel(g, &sim.agents[0], 0);
        // A heavy body takes a turn every few steps (e055), so wait for the first turn that does not
        // fall on a climate update (which would move the light between the reading and the turn).
        let before = sim.matter();
        let (mut last, mut checked) = (0.0f64, false);
        for _ in 0..300 {
            let a = &sim.agents[0];
            let want: f64 = a
                .cells_held()
                .filter(|&q| a.wcells[q] == LEAF as u8)
                .map(|q| {
                    let (sx, sy) = a.sub_at(g, q, NORTH, 0);
                    a.open[q] as f64 * sim.sun[g.wcell(sx, sy)] as f64
                })
                .sum::<f64>()
                * sim.p.light_gain;
            let (turns, world_step) = (a.turns, sim.step % sim.p.tick as u64 == 0);
            sim.step();
            let got = sim.k.light - last;
            last = sim.k.light;
            if sim.agents.is_empty() {
                break;
            }
            if sim.agents[0].turns > turns && !world_step {
                assert!(want > 0.0, "the body stands in the dark");
                assert!((got - want).abs() < 1e-9 * want, "the light gave {got} of {want}");
                checked = true;
                break;
            }
        }
        assert!(checked, "the body never took a turn in the light");
        assert!((sim.matter() - before).abs() < 1e-9 * before, "matter moved");

        // With the light off, no cell of any body is a leaf block and the bodies are e092's.
        let mut p = small();
        p.light_gain = 0.0;
        let (w, pl, u) = settled_world(&p, None);
        let sim = Sim::new(p, w, pl, u, 1);
        assert!(sim.agents.iter().all(|a| a.body.kinds[LEAF] == 0), "a leaf block with the light off");
    }

    /// e096 (#108): a cell's light is one flux. The blocks standing over it darken
    /// `min(1, shade * n / SUB^2)` of it and divide exactly that among themselves, the ground grows
    /// at what is left, and the two add up to the cell's light. A leaf block's income follows its
    /// share, so every block on its cell - its own body's and any other's - cuts it.
    #[test]
    fn the_cell_s_light_is_one_flux() {
        for &shade in &[0.5, 1.0, 2.0, 4.0, 16.0] {
            assert_eq!(block_light(shade, 0), 0.0);
            for n in 1..64u32 {
                let took = n as f64 * block_light(shade, n) / SUB_CELLS as f64;
                let left = 1.0 - shaded(shade, n);
                assert!((took + left - 1.0).abs() < 1e-12, "shade {shade}, {n} blocks: {took} taken, {left} left");
                assert!(n < 2 || block_light(shade, n) <= block_light(shade, n - 1), "the share rose with the crowd");
            }
        }
        // At `shade` 1 a block reads what e093 granted every face until the cell carries the blocks
        // its face holds (SUB^2); past that it falls as 1/n, which is the middle e093 lacked.
        assert!((block_light(1.0, 1) - 1.0).abs() < 1e-12);
        assert!((block_light(1.0, SUB_CELLS as u32) - 1.0).abs() < 1e-12);
        assert!((block_light(1.0, 2 * SUB_CELLS as u32) - 0.5).abs() < 1e-12);
        assert!((shaded(1.0, SUB_CELLS as u32) - 1.0).abs() < 1e-12);

        // The ground grows at what is left: half a cell's light, half its grass and half its algae.
        let p = small();
        let (w0, mut pl0, _) = settled_world(&p, None);
        let (w1, mut pl1, _) = settled_world(&p, None);
        for c in 0..pl1.shade_left.len() {
            pl1.shade_left[c] = 0.5;
        }
        let (f0, f1) = (pl0.update(&w0, &p, 3), pl1.update(&w1, &p, 3));
        assert!(f0.grass > 0.0 && f0.algae > 0.0, "nothing grew to shade");
        assert!((f1.grass - 0.5 * f0.grass).abs() < 1e-9 * f0.grass, "grass {} of {}", f1.grass, f0.grass);
        assert!((f1.algae - 0.5 * f0.algae).abs() < 1e-9 * f0.algae, "algae {} of {}", f1.algae, f0.algae);

        // A leaf block takes its share of its cell's flux, and a body of many blocks shades itself:
        // what it takes is what the formula says, and less than e093's grant of the whole light.
        let mut p = small();
        p.light_gain = 0.02;
        p.shade = 2.0;
        let (w, pl, u) = settled_world(&p, None);
        ...
        for _ in 0..300 {
            let a = &sim.agents[0];
            let (mut want, mut shares) = (0.0f64, false);
            for q in a.cells_held().filter(|&q| a.wcells[q] == LEAF as u8) {
                let (sx, sy) = a.sub_at(g, q, NORTH, 0);
                let c = g.wcell(sx, sy);
                let share = block_light(sim.p.shade, sim.occ.blocks_over(c));
                want += sim.sun[c] as f64 * a.open[q] as f64 * share;
                shares |= share < sim.p.shade; // the cell's flux is spent: the blocks on it divide it
            }
            let want = want * sim.p.light_gain;
            let (turns, world_step) = (a.turns, sim.step % sim.p.tick as u64 == 0);
            sim.step();
            let got = sim.k.light - last;
            last = sim.k.light;
            ...
            if sim.agents[0].turns > turns && !world_step {
                assert!(want > 0.0, "the body stands in the dark");
                assert!((got - want).abs() < 1e-9 * want, "the light gave {got} of {want}");
                assert!(shares, "a body of {} blocks did not shade itself", sim.agents[0].body.size);
                checked = true;
                break;
            }
        }
