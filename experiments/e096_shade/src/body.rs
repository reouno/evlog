...
pub const KIND_MASS: [f32; N_KINDS] = [0.0, 2.0, 1.0, 0.5, 1.0, 1.0]; // empty, hard, muscle, sensor, gut, leaf (e025, e093)
...
impl Laws {
    pub fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        let morph_level = (0..=SIDE_MAX)
            ...
            .collect();
        Laws { morphogen, table, density, side, leaf, leaf_on: false, water, breath, breed, share, store, history: false, store_gene: false, morph_level }
    }
}
...
pub fn develop_genes(genes: &[Gene], laws: &Laws) -> Body {
    ...
    // e093 (#103): with the light off the leaf has no column of its own to score and no cell develops
    // one, so the bodies are e092's to the bit.
    let leaf_col: Vec<f32> = if laws.leaf_on { genes.iter().map(|g| laws.leaf[pattern_index(&g.product)]).collect() } else { Vec::new() };
    let n_kinds = if laws.leaf_on { N_KINDS } else { N_TABLE_KINDS };
...
impl Agent {
    ...
    pub fn new(id: u64, genome: Vec<u8>, keys: Vec<u16>, gene_ids: Vec<u16>, body: Body, facing: u8, energy: f64, lineage: u32) -> Agent {
        let mut a = Agent {
            id, lineage, x: 0, y: 0, energy, age: 0, plant: 0.0, scavenged: 0.0, killed: 0.0, algae: 0.0, detritus: 0.0, light: 0.0, fat: 0.0, born_size: 0, born_mass: 0.0, born_kinds: [0; N_KINDS], born_bite: 0, born_at: (0, 0), born_hab: 0,
...
impl Occ {
    ...
    /// e096 (#108): the blocks of bodies standing over world cell c, in every layer, counted once
    /// each - the unit the shade is cast and divided by. A land body holds both water layers
    /// (`layers`), so on land the floor's count is read alone; in the water the two layers are
    /// different bodies and both stand in the cell's one column of light, as does a crown.
    pub fn blocks_over(&self, c: usize) -> u32 {
        let i = LAYERS * c;
        let below = if self.wet[c] { self.crowd[i + BOTTOM] as u32 } else { 0 };
        self.crowd[i + SURFACE] as u32 + below + self.crowd[i + CROWN] as u32
    }
...
pub fn push(agents: &mut [Agent], i: usize, d: usize, g: Grid, occ: &mut Occ, carrion: &mut [f64], s: f64, flesh: f64, c: &mut Counters) -> Vec<(usize, u8)> {
    let opp = opposite(d);
...
mod tests {
    ...
    fn a_push_meets_face_to_face() {
        ...
        let pressed = push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.0, &mut c);
        ...
        occ.claim(g, &agents[1], 1);
        let mut carrion = vec![0.0; g.cells()];
        push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.0, &mut c);
    ...
    fn a_tear_opens_the_flesh() {
        ...
        let mut c = Counters::default();
        let mut carrion = vec![0.0; g.cells()];
        push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.25, &mut c);
        ...
        let mut carrion = vec![0.0; g.cells()];
        push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.25, &mut c2);
    ...
    fn the_sea_is_a_wall() {
        ...
        let mut c = Counters::default();
        let mut carrion = vec![0.0; g.cells()];
        assert!(push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.0, &mut c).is_empty());
