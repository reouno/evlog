...
impl Food {
    ...
    fn regrow(&mut self, c: usize, own: f32, crown: f32, held: bool, cap: f32, spill: bool, wet: f32, root_cap: f32) -> Regrown {
        ...
        let all = lit + crown;
        let sun = all * wet; // e035: a dry cell uses only part of its light
        let dry = all - sun;
        ...
        Regrown { added, fruit, shaded: own - lit, wasted, barren, rot: rot as f32, dry, stored }
    }
...
fn main() {
    ...
    let prefix = format!("experiments/e040_thirst/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}_seed{seed}", rain.name());
    ...
    for step in 1..=steps {
        ...
        for c in 0..w * h {
            ...
            let r = food.regrow(c, light[c], crown[c], occ.crowd[c] > 0, cap, spill > 0, wet, grew_cap);
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{fill_mean:.4},{:.4},{},{pool_share:.4}", drinking as f64 / decisions, deaths[4]).unwrap();
            sun_acc = 0.0;
...
mod tests {
    ...
    fn a_plant_grows_out_of_the_soil_and_the_dead_rot_into_it() {
        ...
        assert_eq!(food.regrow(0, 0.1, 0.0, false, cap, false, 1.0, 0.0), Regrown { added: 0.05, barren: 0.05, ..Default::default() });
        assert!((food.res[0] - 0.05).abs() < 1e-6 && food.soil[0] < 1e-9);
        // No soil: the whole sun is barren.
        assert_eq!(food.regrow(1, 0.1, 0.0, false, cap, false, 1.0, 0.0), Regrown { barren: 0.1, ..Default::default() });
        // A body on the cell: shaded, nothing moves.
        assert_eq!(food.regrow(2, 0.1, 0.0, true, cap, false, 1.0, 0.0), Regrown { shaded: 0.1, ..Default::default() });
        assert_eq!((food.res[2], food.soil[2]), (0.0, 1.0));
        // At the cap the sun is wasted and the soil stays.
        food.res[2] = cap as f64;
        assert_eq!(food.regrow(2, 0.1, 0.0, false, cap, false, 1.0, 0.0), Regrown { wasted: 0.1, ..Default::default() });
        ...
        food.res[3] = 2.0;
        food.carrion[3] = 2.0;
        let r = food.regrow(3, 0.1, 0.0, false, cap, false, 1.0, 0.0);
    ...
    fn ground_store() {
        ...
        food.res[0] = 8.0;
        let r = food.regrow(0, 0.1, 0.0, false, 8.0, true, 1.0, 0.05);
        ...
        food.res[2] = 8.0;
        let r = food.regrow(2, 0.1, 0.0, false, 8.0, true, 1.0, 0.0);
    ...
    fn a_full_crown_keeps_taking_the_light_and_drops_fruit_around_it() {
        ...
        // out of its soil.
        let r = food.regrow(t, light[t], crown[t], false, 8.0, true, 1.0, 0.0);
        ...
        let r = food.regrow(t, light[t], crown[t], true, 8.0, true, 1.0, 0.0);
        ...
        food.fruit[t] = 1.0;
        let r = food.regrow(t, 0.0, 0.0, false, 8.0, true, 1.0, 0.0);
