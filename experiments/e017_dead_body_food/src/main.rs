//! e017: a dead body is food where it lies. e016's world (space at the resolution of the
//! body, facing, two kinds of place, contact physics, a cell costs what it holds, per-body
//! cost, work = force x distance, a cell held by a body does not regrow) with one law added
//! about matter:
//!
//! - What a body is made of does not vanish when it dies. Every cell of a dead body is
//!   CELL_ENERGY of food on the world cell under it, and the energy the body still held is
//!   split over its cells and lies with them. A child that is never placed (no room, or no
//!   cells) lies where its parent stands. A cell broken by a push that no gut takes lies where
//!   it was. This is the same food the gut cells eat; it is added in full (the cap bounds what
//!   a plant grows to, not what lies on the ground). Nothing else changes: upkeep and the work
//!   of moving are heat and vanish, as before.
//!
//! What a cell is made of, CELL_ENERGY, is an argument (0.02, e010's value, by default): it
//! sets what a body costs to build and what a dead body is worth alike.
//!
//! Everything else is e016 with the strict reading of the shade law. The log gets `dead`
//! (matter laid on the ground per step) and `carrion` (dead matter lying uneaten at the log
//! step); `meat` now counts the dead matter a body ate, next to broken cells.
...
/// Food on the ground: what a cell holds (`res`) and how much of that is dead matter
/// (`carrion`, at most `res`; a measure only, no rule reads it).
struct Food {
    res: Vec<f32>,
    carrion: Vec<f32>,
}
...
fn main() {
    ...
    let prefix = format!("experiments/e017_dead_body_food/results/{size}_sigma{sigma_name}{cell_tag}_seed{seed}");
    ...
    writeln!(places_csv, "step,place,pop,mass,hard,muscle,sensor,digestive,bite,shell,biters,cover,plant_intake,meat_intake,lineages,movers,foot,shell_front,shell_back,dead,carrion").unwrap();

    let mut food = Food { res: vec![cap; w * h], carrion: vec![0.0; w * h] };
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent,shaded,dead,carrion"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        // A plant under a body does not grow: a cell held by a body does not regrow (e016). A
        // plant grows to the cap; dead matter can lie above it, and then nothing grows.
        for (c, (r, &grow)) in food.res.iter_mut().zip(&patches.grow).enumerate() {
            let gr = if occ.crowd[c] == 0 { grow } else { 0.0 };
            shaded += (grow - gr) as f64;
            let added = gr.min((cap - *r).max(0.0));
            *r += added;
        ...
        if step % LONG_INTERVAL == 0 {
            snaps.write_frame(false, step, &food.res, &patches, &agents);
        }
        ...
        if step >= CLIP_START && step < CLIP_START + CLIP_LEN {
            snaps.write_frame(true, step, &food.res, &patches, &agents);
        }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.3},{:.5},{:.2},{:.4},{:.2}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64, shaded / LOG_INTERVAL as f64,
                cc.dead / LOG_INTERVAL as f64, carrion_stock).unwrap();
            for p in 0..=N_PLACES as u8 {
                ...
                writeln!(places_csv, "{step},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{:.3},{:.1},{:.1},{},{movers},{:.2},{:.2},{:.2},{:.4},{:.2}", place_name(p), here.len(),
                    ...
                    cc.dead_at[p as usize] / LOG_INTERVAL as f64, carrion_at[p as usize]).unwrap();
            }
...
mod tests {
    ...
    fn a_dead_body_is_food_where_it_lies() {
        ...
        let mut food = Food { res: vec![0.0; g.cells()], carrion: vec![0.0; g.cells()] };
