//! e024: what flesh is worth (#27). e023's closed world (the canopy, the spill, rain on the
//! mountains, mutation per base, eyes that see far) with one law changed, about what the
//! matter of a body is:
//!
//! - The flesh keeps a share of what the body burns. Of the upkeep a body pays each step, a
//!   share `flesh` (1 by default: nothing a body burns is lost but the work of moving; the
//!   experiment ran 1 and 0.7 and a dose series) is fixed in its flesh (`fat`) instead of breathed to the
//!   air; the body cannot spend it. Whoever breaks a cell of the body gets the cell's matter
//!   (`cell_energy`) plus the cell's share of the body's energy, as before, plus its share of
//!   the fat; a body that dies lays its fat on the ground with the rest. So a cell of an old
//!   body is worth more than the plant it stood on, by what the body lived through, and the
//!   matter stays conserved: the fat is taken from the breath, and returns through the eater
//!   or the soil. `flesh` 0 is e023 byte for byte.
//!
//! Kept from e023:
...
//! range, 8 by default; 0 is e022's law) and `flesh` (the share of the upkeep fixed in the
//! flesh, 1 by default; 0 is e023's law). The log gets `fat_mean` (fat per body),
...
struct Agent {
    ...
    fat: f32, // matter fixed in the flesh from the upkeep paid (the flesh law); yielded to the eater or the ground, never spent
    born_mass: u8,
...
struct Food {
    res: Vec<f32>,
    carrion: Vec<f32>,
    fruit: Vec<f32>, // plant matter lying on the cell, fallen from a column around it (at most `res`, with `carrion`)
    soil: Vec<f64>,
}
...
impl Food {
    fn new(res: Vec<f32>, carrion: Vec<f32>, soil: Vec<f64>) -> Self {
    ...
    fn regrow(&mut self, c: usize, own: f32, crown: f32, held: bool, cap: f32, spill: bool) -> Regrown {
        let rot = (self.carrion[c] + self.fruit[c]) * DECAY;
        self.carrion[c] -= self.carrion[c] * DECAY;
        self.fruit[c] -= self.fruit[c] * DECAY;
        self.res[c] -= rot;
        self.soil[c] += rot as f64;
        let lit = if held { 0.0 } else { own };
        let sun = lit + crown;
        let room = if held { 0.0 } else { (cap - self.res[c]).max(0.0) };
        ...
        self.res[c] += added;
        Regrown { added, fruit, shaded: own - lit, wasted, barren, rot }
    }
    ...
    /// proportions.
    fn take(&mut self, c: usize, e: f32) -> (f32, f32, f32) {
        let r = self.res[c];
...
fn push(agents: &mut [Agent], i: usize, d: usize, g: Grid, occ: &mut Occ, place: &[u8], food: &mut Food, cell_energy: f32, c: &mut Counters) -> Vec<(usize, u8)> {
    ...
    for (victim, pos, eater) in breaks {
        ...
        v.energy -= share;
        let fat = v.fat / v.body.mass as f32;
        v.fat -= fat;
        v.body.cells[bpos] = 0;
        v.body.refresh();
        v.reframe();
        ...
        if e.body.kinds[DIGESTIVE] > 0 {
            let gain = share + fat + cell_energy;
            e.energy += gain;
            e.meat += gain;
            c.meat_intake += gain;
            c.kill_gain += gain;
            ...
            food.lay(under, share + fat + cell_energy, place, c);
        }
    }
    pressed
}
...
fn main() {
    ...
    let prefix = format!("experiments/e024_flesh/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}_seed{seed}", rain.name());
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height,fat").unwrap();
    ...
    let mut food = Food::new(vec![matter0.min(cap); w * h], vec![0.0; w * h], vec![(matter0 - cap).max(0.0) as f64; w * h]);
    ...
    for _ in 0..init_pop {
        ...
        let mut a = Agent {
            id: next_id - 1, lineage: 0, x: 0, y: 0, energy: INIT_ENERGY, age: 0, plant: 0.0, meat: 0.0, fat: 0.0, born_mass: body.mass, born_place: NO_PLACE,
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            for (k, c) in guts.iter().enumerate() {
                ...
                if food.res[c] - food.carrion[c] - food.fruit[c] >= TREE {
                    tree_eaten += e as f64;
                }
            ...
            let fixed = upkeep * flesh;
            a.fat += fixed;
            let burned = upkeep - fixed;
            if breath < 1.0 {
                food.spend(a.here(g), burned * (1.0 - breath));
            }
            ...
            if agents[i].energy >= thr {
                ...
                pending.push((Agent {
                    id: next_id - 1, lineage: a.lineage, x: a.x, y: a.y, energy: a.energy, age: 0, plant: 0.0, meat: 0.0, fat: 0.0, born_mass: 0, born_place: NO_PLACE, alive: true,
        ...
        if step % AGENT_DUMP_INTERVAL == 0 {
            ...
            for a in &agents {
                let k = &a.body.kinds;
                let b = &a.body;
                write!(agents_csv, "{step},{},{},{},{},{},{},{},{:.2},{},{:.3},{},{:.2},{:.2},{:.2},{},{},{},{},{:.2},{:.2},{:.2},{},{},{},{:.1}",
                ...
                write!(agents_csv, ",{:.3}", a.fat).unwrap();
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            let worth = agents.iter().filter(|a| a.body.mass > 0).map(|a| (a.energy.max(0.0) + a.fat) / a.body.mass as f32 + cell_energy).sum::<f32>() / pop;
            ...
            let plant_of = |c: usize| food.res[c] - food.carrion[c] - food.fruit[c];
            ...
            writeln!(log, ",{:.3},{:.5},{:.2},{:.4},{:.2},{soil_stock:.1},{matter:.1},{:.3},{:.4},{:.3},{:.3},{soil_cells:.3},{deep:.3},{air:.1},{:.3},{:.3},{trees},{tree_res:.1},{res_max:.2},{:.3},{:.3},{fruit_stock:.1},{:.3},{:.3},{:.2},{fat_mean:.4},{fat_stock:.1},{worth:.4},{kill_gain:.4}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64, shaded / LOG_INTERVAL as f64,
...
mod tests {
    ...
    fn agent_with(cells: [u8; CELLS], x: usize, y: usize, facing: usize) -> Agent {
        let mut a = Agent {
            id: 0, lineage: 0, x, y, energy: 1.0, age: 0, plant: 0.0, meat: 0.0, fat: 0.0, born_mass: 0, born_place: NO_PLACE, alive: true,
    ...
    fn a_full_crown_keeps_taking_the_light_and_drops_fruit_around_it() {
        ...
        assert!((r.rot - DECAY).abs() < 1e-7 && (food.fruit[t] - (1.0 - DECAY)).abs() < 1e-6 && (food.soil[t] - DECAY as f64).abs() < 1e-7);
    }
    ...
    fn the_flesh_is_worth_what_the_body_lived_through() {
        ...
        assert!((agents[0].energy - (1.0 + gain)).abs() < 1e-6 && (agents[0].meat - gain).abs() < 1e-6);
        assert!((c.kill_gain - gain).abs() < 1e-6);
        assert!((agents[1].fat - 0.2).abs() < 1e-6 && (agents[1].energy - 0.5).abs() < 1e-6);
        assert_eq!(food.res.iter().sum::<f32>(), 0.0);
        ...
        assert!((food.res.iter().sum::<f32>() - (CELL_ENERGY + 0.5 + 0.2)).abs() < 1e-6);
