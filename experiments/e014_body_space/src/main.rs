//! e014: space at the resolution of the body. e013's world (facing, two kinds of place, contact
//! physics, a cell costs what it holds, per-body cost) with its space law rewritten:
//!
//! - A body occupies its own cells. The world is drawn at the resolution of the body cell: a
//!   world cell is 4x4 sub-cells, and a body occupies exactly the sub-cells its grid fills,
//!   turned to its facing. No two bodies share a sub-cell. A move is one sub-cell along the
//!   facing (a second with probability speed). Where a cell of the mover would enter a cell of
//!   another body, the two faces meet: e010's rule, in that line (the softer face breaks if
//!   the mover's muscle in the line exceeds its hardness; the hardness of a face is 3 per
//!   contiguous hard cell behind it, else 1), and the move happens only if the way is clear
//!   afterwards: the other lost the cells in the way, or was shoved one sub-cell by a push whose
//!   muscle exceeds its mass. Food stays per world cell: a digestive cell eats from the world
//!   cell under it. A child is born where its cells find room next to its parent.
//! - Facing stays as in e013 (one number per body; the grid turns with it; only the front pushes;
//!   the policy sees food and bodies ahead, behind, left, right).
//!
//! Everything else is e013. Measures: e013's, with the footprint as the world cells under the
//! body's cells and its extent along and across the facing; cover is the share of sub-cells held.
...
const MOVE_COST: f32 = 0.001; // per block per cell moved (a body cell: one sub-cell of the world)
...
fn main() {
    ...
    let prefix = format!("experiments/e014_body_space/results/{size}_sigma{sigma_name}_seed{seed}");
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            //    attempt costs the move whether or not it happens. Turn: the grid rotates about
            //    its center; the turn happens if the sub-cells it would newly hold are free.
            if action == 1 {
                moves_tried += 1;
                let d = f;
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.3},{:.3},{:.3},{:.3},{:.2},{:.2},{:.2},{:.2},{:.3},{:.2},{:.2},{:.2}",
