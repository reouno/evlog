...
GALLERY = [
    (147, "the constellation, grass", "Six digestive cells at the four corners of the grid (two at the front corners): 7x7 cells across with six of them, each corner in its own world cell. Born at step 14,000, alive at 1,000,000, 5,839 agents at its peak: the body of the run and of every run."),
    (170, "the constellation, mirrored", "The same four corners with the second cell on the other side; 1,578 agents at its peak, 389,000 steps, replaced by lineage 147, its near-clone."),
    (598, "three corners", "Three corners and a fourth cell: 4.2 world cells with six cells, 220,000 steps on the grass."),
    (520, "the front row", "Five cells along the front edge and two below the corners: 3.4 world cells, front-heavy; 792 agents, 195,000 steps early in the run."),
    (214, "the corner wedge", "e010's corner body with a tail: seven cells against the right edge, 3.6 world cells. The shape of the first 100,000 steps, before the corners spread."),
    (1373, "the block, trees", "A 2x2 block of digestive cells at the center of the grid, 100% on the trees: one rich cell feeds it, and it fits in the gap of any constellation. 397,000 steps, the longest tree lineage."),
    (1256, "the front pair, trees", "Two pairs of cells at the front corners, 98% on the trees: the constellation cut to its front row, over two world cells."),
    (171, "corner two, trees", "Two cells in one corner, 98% on the trees, 260,000 steps: the smallest body that lasts, a mouth on a rich cell."),
]
...
TEXT = {
    ...
    "tldr": "e013's space law, rewritten at the resolution of the body: a body holds exactly the sub-cells its grid fills (4x4 per world cell), a small body is small, bodies nest, and contact is the meeting of real cells. The world stands (1,380-2,510 bodies at 128, 290-430 steps/s), births are four times e013's, the narrow places hold three to four times e013's bodies, and still nobody touches anybody: 0.06-0.22 contacts per body per step at every width in every seed (e012: 2-4), no bite, no hard cell, no meat, no hunter lineage, 62-92% of moves blocked. The resolution was not the problem; the motive is. In e012 a move always succeeded and a push into fifty bodies was its free by-product; once bodies take up space, a push is a failed move that costs the mover, a body without a tooth gains nothing from it, and the winning policies stay and turn (forward is 7-15% of decisions). Selection keeps only reach, and reach alone makes plants: the body that wins everywhere is four to eight digestive cells at the four corners of its grid, a constellation lying over four or five world cells that hardly moves. At 256 (eight patches of each kind) it is the same world eight times over: 5,550-7,920 bodies, 0.08-0.19 contacts per body per step, no bite, 2-7 lineages, the trees holding 90-98 bodies per patch (e013: 19-29). Next: charge a move by the distance moved (work is force times distance), folded into ground and friction (#16), so that pushing into a body is free to try and contact can again be a by-product of moving.",
    ...
    "c3": "no", "l3": "No, at every width", "v3": "Trees (width 1), edge (2), shrubs (4): hard cells per body 0.00-0.14, bodies with a bite 0% on the front and 0-0.3% on any side, meat 0.000% of the intake, no hunter lineage, in all twelve runs; the front and the back of a body are equally soft (1.00-1.06). Cells broken: 0-24 per 10,000 steps (one window of 221 in seed 3 with trees, a passing body with muscle). At 256: hard 0.01-0.06 per body, 0% with a bite on the front and 0-0.1% on any side, meat 0.000%, no hunter lineage, on the trees (718-784 bodies over eight patches) and on the edge (2,360-2,440).",
    "c4": "yes", "l4": "Yes", "v4": "No extinction; population 1,380-2,510 at 128, never below 1,250; 290-430 steps per second with twelve runs on one machine (e013: 710-1,550; the sub-cell physics costs about twice per body and there are more bodies). At 256: population 5,550-6,370 with trees (e013: 4,690-5,370), 7,560-7,920 with the edge, never below 4,840; 106-147 steps/s with six runs on one machine, one thread each (the 6-core machine managed 50 and was stopped).",
    ...
    "viewer": "Grass and trees, 256, seed 1. Each grass island is a lawn of one or two colors, thinner than e013's discs since a body holds only its own cells; each tree island a knot of 90-98 small bodies of one color. The zoom shows what a body is now: four dots of one color at the corners of an invisible square, with the dots of other bodies between them, a white edge on the front side of each square pointing every way. The long view shows lineage 147 (the constellation) taking the grass by step 200,000 and holding it to the end while the tree knots change color every hundred thousand steps. Play the clip: a dot moves one sub-cell now and then; most of them do not. ",
