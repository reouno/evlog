...
fn main() {
    ...
    // e052 (#55): the exponent of the body's clock (0: every body takes a turn every step, e051
    // byte for byte). A body of mass m takes (CLOCK_MASS / m)^clock turns a step, at most one.
    ...
    let clock: f32 = args.get(47).and_then(|s| s.parse().ok()).unwrap_or(0.0);
