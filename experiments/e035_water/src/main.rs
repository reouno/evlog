...
impl Food {
    fn new(res: Vec<f64>, carrion: Vec<f64>, soil: Vec<f64>) -> Self {
        ...
        Food { res, carrion, fruit, soil, water }
    }
    ...
    fn regrow(&mut self, c: usize, own: f32, crown: f32, held: bool, cap: f32, spill: bool, wet: f32) -> Regrown {
        ...
        Regrown { added, fruit, shaded: own - lit, wasted, barren, rot: rot as f32, dry }
    }
...
fn main() {
    ...
    let prefix = format!("experiments/e035_water/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"digest\":{digest_law},\"digest_floor\":{DIGEST_FLOOR},\"side\":\"{}\",\"side_max\":{SIDE_MAX},\"store\":{store},\"yolk\":{yolk},\"breed\":{breed_law},\"winter\":\"{}\",\"water\":{water_rate},\"leach\":{leach},\"depth\":{depth},\"mix\":{mix},\"water_rain\":{WATER_RAIN},\"water_evap\":{WATER_EVAP},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), side_law.name(), winter.name(), list(&terrain.height)).unwrap();
    }
    ...
    writeln!(places_csv, "step,place,pop,mass,hard,muscle,sensor,digestive,bite,shell,biters,cover,plant_intake,meat_intake,lineages,movers,foot,shell_front,shell_back,dead,carrion,soil,barren,regrowth,cells,rain,trees,fruit,fruit_intake,water,dry").unwrap();
    ...
    for step in 1..=steps {
        ...
        for c in 0..w * h {
            ...
            let r = food.regrow(c, light[c], crown[c], occ.crowd[c] > 0, cap, spill > 0, wet);
        ...
        if step % AGENT_DUMP_INTERVAL == 0 {
            ...
            writeln!(soil_jsonl, "{{\"step\":{step},\"soil\":[{}],\"plant\":[{}],\"water\":[{}]}}", list(&soil32), list(&res32), list(&water32)).unwrap();
            for a in &agents {
                let k = &a.body.kinds;
                let b = &a.body;
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{side_mean:.2},{side_std:.2},{},{},{},{},{:.4},{:.4},{on_fat:.4},{:.4},{:.4},{:.4},{water_mean:.2},{:.4},{:.4}", sq(0.1), sq(0.5), sq(0.9), sizes.last().copied().unwrap_or(0), fat_spent / LOG_INTERVAL as f64, fat_over / LOG_INTERVAL as f64, breed_decisions as f64 / decisions, breed_denied as f64 / breed_decisions.max(1) as f64, dry / LOG_INTERVAL as f64, leached / LOG_INTERVAL as f64, mixed / LOG_INTERVAL as f64).unwrap();
            sun_acc = 0.0;
            for p in 0..=N_PLACES as u8 {
                ...
                writeln!(places_csv, "{step},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{:.3},{:.1},{:.1},{},{movers},{:.2},{:.2},{:.2},{:.4},{:.2},{:.1},{:.4},{:.4},{},{:.4},{trees_here},{:.4},{:.1},{:.2},{:.4}", place_name(p), here.len(),
                    ...
                    fruit_at[p as usize] / LOG_INTERVAL as f64, fruit_eaten_at[p as usize], water_at[p as usize] / cells_at[p as usize].max(1) as f64, dry_at[p as usize] / LOG_INTERVAL as f64).unwrap();
            }
...
mod tests {
    ...
    fn a_plant_grows_out_of_the_soil_and_the_dead_rot_into_it() {
        ...
        assert_eq!(food.regrow(0, 0.1, 0.0, false, cap, false, 1.0), Regrown { added: 0.05, barren: 0.05, ..Default::default() });
        assert!((food.res[0] - 0.05).abs() < 1e-6 && food.soil[0] < 1e-9);
        // No soil: the whole sun is barren.
        assert_eq!(food.regrow(1, 0.1, 0.0, false, cap, false, 1.0), Regrown { barren: 0.1, ..Default::default() });
        // A body on the cell: shaded, nothing moves.
        assert_eq!(food.regrow(2, 0.1, 0.0, true, cap, false, 1.0), Regrown { shaded: 0.1, ..Default::default() });
        assert_eq!((food.res[2], food.soil[2]), (0.0, 1.0));
        // At the cap the sun is wasted and the soil stays.
        food.res[2] = cap as f64;
        assert_eq!(food.regrow(2, 0.1, 0.0, false, cap, false, 1.0), Regrown { wasted: 0.1, ..Default::default() });
        ...
        food.res[3] = 2.0;
        food.carrion[3] = 2.0;
        let r = food.regrow(3, 0.1, 0.0, false, cap, false, 1.0);
    ...
    fn a_full_crown_keeps_taking_the_light_and_drops_fruit_around_it() {
        ...
        // out of its soil.
        let r = food.regrow(t, light[t], crown[t], false, 8.0, true, 1.0);
        ...
        let r = food.regrow(t, light[t], crown[t], true, 8.0, true, 1.0);
        ...
        food.fruit[t] = 1.0;
        let r = food.regrow(t, 0.0, 0.0, false, 8.0, true, 1.0);
