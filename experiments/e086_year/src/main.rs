//! e086: the year against a life (#93, P1's first layer). e062's producers, unchanged, with one
//! measure added for the last year: `<prefix>_months.bin` holds, for each of 12 months, every cell's
//! mean temperature, its lowest temperature in the month (the nights) and its growing index: on land
//! what a grazed stand of grass regrows at per unit of it (light x warmth x the ground's fill x the
//! light the wood leaves), in water what algae do (light x warmth / (1 + depth / LIGHT_DEPTH)).
//! Every law and rate is e062's; `year` is the only thing the runs change.
//!
//! What follows is e062's description.
//!
...
//!
//! Run: cargo run --release -p e086_year -- <prefix> [key=value ...] [file.params ...]
...
fn main() {
    ...
    // e086: the last year by month (12): mean temperature, lowest temperature, growing index.
    const MONTHS: usize = 12;
    let mut m_temp = vec![0.0f64; MONTHS * cells];
    let mut m_low = vec![f64::INFINITY; MONTHS * cells];
    let mut m_grow = vec![0.0f64; MONTHS * cells];
    let mut m_updates = [0u32; MONTHS];
    ...
    for yr in 1..=years {
        ...
        for _quarter in 0..4 {
            ...
            for _ in 0..per_quarter {
                ...
                q.add(&w, &pl, p.soil);
                if yr == years {
                    let u_in_year = (update_i - first_update) as usize - (years - 1) * per_year;
                    let m = u_in_year * MONTHS / per_year;
                    m_updates[m] += 1;
                    for c in 0..cells {
                        let t = w.temp[c];
                        let warm = ((t - plants::WARM_FROM) / (plants::WARM_FULL - plants::WARM_FROM)).clamp(0.0, 1.0);
                        let grow = match plants::depth(&w, &p, c) {
                            Some(d) => w.light[c] * warm / (1.0 + d / plants::LIGHT_DEPTH),
                            None => {
                                let wd = pl.wood[c];
                                w.light[c] * warm * (w.ground[c] / p.soil).min(1.0) * (1.0 - wd / (wd + plants::WOOD_HALF))
                            }
                        };
                        let i = m * cells + c;
                        m_temp[i] += t;
                        m_low[i] = m_low[i].min(t);
                        m_grow[i] += grow;
                    }
                }
                update_i += 1;
    ...
    // e086: the months of the last year, after the elevation: "E086", n, months, then per month the
    // mean temperature, the lowest temperature and the growing index as f32 maps.
    let mut out: Vec<u8> = Vec::with_capacity(12 + 4 * cells * (1 + 3 * MONTHS));
    out.extend_from_slice(b"E086");
    out.extend_from_slice(&(n as u32).to_le_bytes());
    out.extend_from_slice(&(MONTHS as u32).to_le_bytes());
    for &e in &w.elev {
        out.extend_from_slice(&(e as f32).to_le_bytes());
    }
    for m in 0..MONTHS {
        let u = m_updates[m].max(1) as f64;
        for c in 0..cells {
            let i = m * cells + c;
            for x in [m_temp[i] / u, m_low[i], m_grow[i] / u] {
                out.extend_from_slice(&(x as f32).to_le_bytes());
            }
        }
    }
    std::fs::File::create(format!("{prefix}_months.bin")).unwrap().write_all(&out).unwrap();
    let _ = Params::DOCS;
