//! e006: species. The e005 world, with sexual reproduction under a compatibility limit
//! (only genomes within distance D can mate), lineages detected as groups connected by
//! possible mating, and an event log of lineage births, splits, and extinctions.
//! Mode `asexual` is e005 exactly (control); mode `sexual` adds mating.
...
// Species.
const D_DEFAULT: usize = 6; // two agents can mate if their distance is at most D (4th argument)
...
const RADIUS_DEFAULT: usize = 1; // mate search: cells within this Manhattan distance (5th argument)
...
fn main() {
    ...
    let sexual = match args.get(3).map(String::as_str) {
        None | Some("sexual") => true,
        Some("asexual") => false,
        Some(m) => panic!("unknown mode {m}: use sexual or asexual"),
    };
    let d: usize = args.get(4).and_then(|s| s.parse().ok()).unwrap_or(D_DEFAULT);
    let radius: usize = args.get(5).and_then(|s| s.parse().ok()).unwrap_or(RADIUS_DEFAULT);
    ...
    let prefix = format!("experiments/e006_species/results/{}_d{d}_r{radius}_seed{seed}", if sexual { "sexual" } else { "asexual" });
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{births_with_neighbor},{sexual_births},{},{top_lineage:.3},{no_lineage:.3}", lineages.len()).unwrap();
            births = 0;
            sexual_births = 0;
