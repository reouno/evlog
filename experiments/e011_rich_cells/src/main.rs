//! e011: rich cells. e010's world and physics (contact, a cell costs what it holds, per-body
//! cost) with one change in the world's food law: the same total regrowth falls on fewer cells
//! (patch width PATCH_SIGMA is an argument, 8 in e010), and a cell can hold more than one bite
//! (RES_CAP is an argument, 1 in e010). In e010 a cell regrew 0.10 per step at the center of a
//! patch and a digestive cell takes 0.02, so a gut of five cells took everything a cell could
//! give, and every seed went to the smallest grazer. A cell that regrows faster than a small gut
//! can eat, and that holds a store a small gut cannot finish, is food that a small body cannot
//! take. Nothing about bodies changes: whether a large gut, a tooth or a shell pays is left to
//! selection.
use std::collections::HashMap;
use std::io::Write;

// World (as e001/e003). Width and height are arguments; 64 is e006.
const RES_CAP_DEFAULT: f32 = 1.0; // e010; argument 5
...
const PATCH_SIGMA_DEFAULT: f32 = 8.0; // e010; argument 6
...
impl Patches {
    fn new(g: Grid, seed: u64, sigma: f32) -> Self {
        ...
        let mut p = Patches { centers, grow: vec![0.0; g.cells()], sigma, rng };
        p.field(g);
        p
    }
        ...
        for &(cx, cy) in &self.centers {
            for dy in -r..=r {
                for dx in -r..=r {
                    ...
                    self.grow[g.idx(x, y)] += peak * (-d2 / (2.0 * sigma * sigma)).exp();
                }
            }
        }
    }
...
impl Snapshots {
    fn write_frame(&mut self, clip: bool, step: u64, res: &[f32], cap: f32, agents: &[Agent]) {
        ...
        for (i, r) in res.iter().enumerate() {
            ...
            write!(f, "{}", (r / cap * 15.0).round() as u8).unwrap();
        }
...
fn main() {
    ...
    let cap: f32 = args.get(5).and_then(|s| s.parse().ok()).unwrap_or(RES_CAP_DEFAULT);
    let sigma: f32 = args.get(6).and_then(|s| s.parse().ok()).unwrap_or(PATCH_SIGMA_DEFAULT);
    let sexual = true;
    let d = D;
    ...
    let mut patches = if patchy { Some(Patches::new(g, seed, sigma)) } else { None };
    let init_pop = (INIT_POP_PER_CELL * (w * h) as f32).round() as usize;
    let prefix = format!("experiments/e011_rich_cells/results/{size}_{}_cap{cap}_sigma{sigma}_seed{seed}", if patchy { "patchy" } else { "uniform" });
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage").unwrap();
    ...
    writeln!(lineages_csv, "step,lineage,size,mass,hard,muscle,sensor,digestive,bite,shell,open,bodies,age,plant,meat").unwrap();
    ...
    let mut agents: Vec<Agent> = (0..init_pop)
        .map(|_| {
            ...
            next_id += 1;
            Agent {
                id: next_id - 1, lineage: 0, x: rng.below(w), y: rng.below(h), energy: INIT_ENERGY, age: 0, plant: 0.0, meat: 0.0, born_mass: body.mass,
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         mass_p10,mass_p50,mass_p90,mass_max,occupied_cells,crowd_max,wasted,intake_per_gut"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        match patches.as_mut() {
            None => {
                for r in res.iter_mut() {
                    let added = RES_GROWTH.min(cap - *r);
                    *r += added;
                    regrowth += added as f64;
                    wasted += (RES_GROWTH - added) as f64;
                }
            }
            Some(p) => {
        ...
        for i in 0..agents.len() {
            ...
            if agents[i].energy >= thr {
                ...
                pending.push((Agent {
                    id: next_id - 1, lineage: a.lineage, x: cx, y: cy, energy: a.energy, age: 0, plant: 0.0, meat: 0.0, born_mass: 0, alive: true,
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            for (id, m) in &assigned {
                ...
                writeln!(lineages_csv, "{step},{id},{},{:.1},{:.1},{:.1},{:.1},{:.1},{:.1},{:.2},{:.1},{},{:.0},{:.2},{:.2}", m.len(), mass / sz,
                    kinds[HARD] / sz, kinds[MUSCLE] / sz, kinds[SENSOR] / sz, kinds[DIGESTIVE] / sz, bite / sz, shell / sz, open / sz, bodies.len(), age / sz, plant / sz, meat / sz).unwrap();
            }
        ...
        if step % LONG_INTERVAL == 0 {
            snaps.write_frame(false, step, &res, cap, &agents);
        }
        if step % AGENT_DUMP_INTERVAL == 0 {
            for a in &agents {
                let k = &a.body.kinds;
                writeln!(agents_csv, "{step},{},{},{},{},{},{},{},{:.2},{},{:.3},{},{:.2},{:.2},{:.2},{}",
                    a.body.mass, a.born_mass, k[HARD], k[MUSCLE], k[SENSOR], k[DIGESTIVE], a.body.bite(), a.body.shell(), a.body.open_lines(), a.body.speed(), a.age, a.energy, a.plant, a.meat, a.lineage).unwrap();
            }
        }
        if step >= CLIP_START && step < CLIP_START + CLIP_LEN {
            snaps.write_frame(true, step, &res, cap, &agents);
        }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{},{},{},{},{},{crowd_max},{:.2},{:.4}", q(0.1), q(0.5), q(0.9), masses.last().copied().unwrap_or(0), per_cell.len(),
