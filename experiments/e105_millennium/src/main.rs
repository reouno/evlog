...
//! `_lead.json` (each cell's leading genotype every `lead_every` years), `_row.csv`.
...
fn main() {
    ...
    let prefix = args.first().expect("usage: e105_millennium <prefix> [key=value ...] [file.params ...]").clone();
    let p = parse(&args[1..]);
