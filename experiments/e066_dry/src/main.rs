...
const CAUSES: [&str; 4] = ["hunger", "broken", "wear", "thirst"];
...
params! {
    ...
    // e066's dry air
    dry = 0.0, "water a soft block loses a turn per face open to the air, times the dryness there (1 - the ground's fill), in blocks of water (0: e065)";
...
const BODY_KEYS: [&str; 7] = ["life", "steps", "start", "scale", "water", "dry", "drink"];
...
impl Sim {
    ...
    fn bodies(&mut self) {
        ...
        for i in 0..agents.len() {
            ...
            let action = act(&a.body.policy, &a.body.water_policy, &input, wsee);
            k.actions[action] += 1;
            if a.body.kinds[SENSOR] > 0 {
                k.sense_n += 1;
                k.sense_used += (act(&a.body.policy, &a.body.water_policy, &blind, wsee_blind) != action) as u64;
            }
    ...
    const AGENT_HEADER: &'static str = "step,id,lineage,age,turns,mass,size,side,density,born_size,born_mass,born_hard,born_muscle,born_sensor,born_digestive,born_bite,\
        hard,muscle,sensor,digestive,bite,bite_any,shell,speed,energy,fat,plant,meat,killed,scavenged,travel,path,worn,pace,place,born_place,temp,moist,height,medium,algae,detritus,water,drank,open_soft,cells";
    ...
    fn agent_rows(&self, out: &mut dyn Write) {
        ...
        for a in &self.agents {
            ...
            writeln!(
                out,
                "{},{},{},{},{},{:.2},{},{},{:.3},{},{:.2},{},{},{},{},{},{},{},{},{},{},{},{:.2},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.2},{:.2},{},{:.3},{},{},{:.1},{:.3},{:.0},{},{:.3},{:.3},{:.3},{},{},{}",
                self.step,
                a.id,
                a.lineage,
                a.age,
...
fn main() {
    ...
    let prefix = args.first().expect("usage: e066_dry <prefix> [key=value ...] [file.params ...]").clone();
    let p = parse(&args[1..]);
            ...
            if v.wants(step) {
                ...
                v.frame(step, &refs, &[(TAU * t / p.year).sin() as f32, (t / p.year).fract() as f32, sim.agents.len() as f32], |push| {
                    for a in &sim.agents {
                        ...
                        push(viewer::AgentIn { id: a.id as u32, lineage: a.lineage, x: a.x as u16, y: a.y as u16, facing: a.facing, diet, fill: a.water, energy: a.energy as f32, ripe: a.body.threshold(), fat: (a.fat / (STORE * a.body.mass).max(1e-6) as f64) as f32, age: a.age, born: a.born_size, side: a.body.side, cells: &a.body.cells[..s * s] });
                    }
                });
            }
            v.tick(step);
...
mod tests {
    ...
    /// with the water closed and open, and under dry air, where some die of thirst.
    #[test]
    fn matter_is_conserved_with_bodies() {
        for (scale, water, dry) in [(1.0, 0.0, 0.0), (0.0625, 0.0, 0.0), (0.0625, 1.0, 0.0), (0.0625, 1.0, 0.01)] {
            conserved_at(scale, water, dry);
        }
    }

    fn conserved_at(scale: f64, water: f64, dry: f64) {
        let mut p = small();
        p.scale = scale;
        p.water = water;
        p.dry = dry;
