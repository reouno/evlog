...
impl Laws {
    pub fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        let morph_level = (0..=SIDE_MAX)
            ...
            .collect();
        Laws { morphogen, table, density, side, water, morph_level }
    }
}
...
impl Body {
    pub fn new(cells: [u8; CELLS], side: usize, policy: [f32; N_POLICY], n_genes: u16, density: f32) -> Self {
        let mut b = Body { cells, side: side as u8, size: 0, mass: 0.0, density, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE_MAX]; 4], extent: [0; 2], policy, n_genes, cut: 0, water_policy: [0.0; N_WATER_POLICY], open_soft: 0 };
        b.refresh();
        b
    }
    pub fn empty() -> Self {
...
pub struct Agent {
    ...
    pub n_filled: u16,
    pub bbox: [u8; 4],
    pub open: [u8; CELLS], // e066: the faces of each soft block open to the air, in the world frame
...
impl Agent {
    ...
    pub fn new(id: u64, genome: Vec<u8>, keys: Vec<u16>, gene_ids: Vec<u16>, body: Body, facing: u8, energy: f64, lineage: u32) -> Agent {
        let mut a = Agent {
            ...
            wcells: [0; CELLS], tips: [[Tip::default(); SIDE_MAX]; 4], filled: [0; CELLS], n_filled: 0, bbox: [0; 4], open: [0; CELLS], water: 1.0, drank: 0,
        };
        a.set_body(body);
        a
    }
...
/// `wpolicy`; None without dry air, and the outputs are e065's.
pub fn act(policy: &[f32; N_POLICY], wpolicy: &[f32; N_WATER_POLICY], input: &[f32; N_IN], winput: Option<&[f32; N_IN_WATER]>) -> usize {
    let mut best = 0;
