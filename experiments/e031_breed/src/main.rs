//! e031: the child of the flesh, and breeding as a decision. e030's world (the store, at 5 by
//! default here) with two laws that run alone or together:
//!
...
//! share of the parent's fat a child gets, 0 by default) and `breed` (0 or 1, 0 by default).
...
fn main() {
    ...
    let prefix = format!("experiments/e031_breed/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"digest\":{digest_law},\"digest_floor\":{DIGEST_FLOOR},\"side\":\"{}\",\"side_max\":{SIDE_MAX},\"store\":{store},\"yolk\":{yolk},\"breed\":{breed_law},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), side_law.name(), list(&terrain.height)).unwrap();
    }
    ...
    writeln!(pop_csv, "step,pop,on_fat,fat_stock,age_mean").unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            writeln!(pop_csv, "{step},{},{:.4},{:.1},{:.0}", agents.len(), agents.iter().filter(|a| a.energy <= 0.0).count() as f64 / n, agents.iter().map(|a| a.fat).sum::<f64>(), agents.iter().map(|a| a.age as f64).sum::<f64>() / n).unwrap();
        }
        if step % LINEAGE_INTERVAL == 0 {
