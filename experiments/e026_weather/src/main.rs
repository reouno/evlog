...
fn main() {
    ...
    assert!(amplitude >= 0.0 && (weather != Weather::Season || amplitude <= 1.0), "the amplitude is a sigma (cloud) or a share 0 to 1 (season)");
    ...
    let prefix = format!("experiments/e026_weather/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), list(&terrain.height)).unwrap();
    }
    ...
    for step in 1..=steps {
        ...
        if weather == Weather::Season {
            sun_factor = 1.0 + amplitude * (2.0 * std::f32::consts::PI * step as f32 / SEASON).sin();
            for i in 0..w * h {
                sun_now[i] = patches.grow[i] * sun_factor;
            }
        }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.3},{:.5},{:.2},{:.4},{:.2},{soil_stock:.1},{matter:.1},{:.3},{:.4},{:.3},{:.3},{soil_cells:.3},{deep:.3},{air:.1},{:.3},{:.3},{trees},{tree_res:.1},{res_max:.2},{:.3},{:.3},{fruit_stock:.1},{:.3},{:.3},{:.2},{fat_mean:.4},{fat_stock:.1},{worth:.4},{kill_gain:.4},{size_mean:.2},{density_mean:.3},{density_std:.3},{:.4},{cloud_std:.4}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64, shaded / LOG_INTERVAL as f64,
                ...
                shade_moved / LOG_INTERVAL as f64, tree_eaten / LOG_INTERVAL as f64, fruit_fallen / LOG_INTERVAL as f64, fruit_eaten / LOG_INTERVAL as f64, clones as f64 / children_n, mutations as f64 / children_n, sun_acc / LOG_INTERVAL as f64).unwrap();
            sun_acc = 0.0;
