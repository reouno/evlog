//! e019: matter that flows. e018's closed world (matter does not vanish: it is in the soil of
//! a cell, in the plant on it, lying dead on it, or in a body; a plant grows out of its own
//! cell's soil at most the sun's rate; what a body spends falls to the soil under it; the dead
//! rot into it) with one law added about the ground:
//!
//! - The ground has a height, and soil runs downhill. Every cell has a height (a terrain: smooth
//!   noise drawn from the seed, RELIEF_GRAIN cells in grain, `relief` in height from the lowest
//!   cell to the highest, measured in soil: one unit of soil on a cell raises its surface by
//!   one). The surface of a cell is its height plus its soil. Each step a cell gives a share
//!   `flow` of its soil to the neighbors whose surface is lower, split by the drop to each,
//!   and never more than an eighth of a drop, so that soil levels out where it pools and does
//!   not slosh. Nothing else moves: plants are rooted and the dead lie where they fell.
//!
//! Matter then collects where the ground is low, and the sun can be uniform (a width of 0):
//! the places are the valleys, drawn by the flow, not by a patch. Under the uniform sun the
//! place of a cell is its height band (0: the lowest third of the cells, the valleys; 1: the
//! middle third, the slopes; 2: the highest third, the ridges), so that the per-place log
//! reads by height. With drawn patches the places are the patches, as before.
//!
//! Arguments: steps, seed, size, patch widths (0: the uniform sun), cell energy, matter per
//! cell at the start, `relief` (0: a flat world, e018 with soil that levels) and `flow` (0:
//! e018). The log gets `flow` (soil moved per step), `soil_cells` (share of the cells with at
//! least a step of sun's worth of soil, 0.01) and `deep` (share with a full plant's worth, 8);
//! the agents and lineages get the height under them; the terrain is written once.
...
fn main() {
    ...
    let prefix = format!("experiments/e019_terrain/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_seed{seed}");
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"grain\":{RELIEF_GRAIN},\"height\":[{}],\"band\":[{bands}]}}", list(&terrain.height)).unwrap();
    }
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent,shaded,dead,carrion,soil,matter,barren,rot,spent,flow,soil_cells,deep"
    )
    .unwrap();
