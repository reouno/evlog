...
fn main() {
    ...
    let prefix = format!("experiments/e032_winter/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}_seed{seed}", rain.name());
    ...
    writeln!(pop_csv, "step,pop,on_fat,fat_stock,age_mean,pop0,pop1,pop2,cross0,cross1,cross2").unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            writeln!(pop_csv, "{step},{},{:.4},{:.1},{:.0},{},{},{},{},{},{}", agents.len(), agents.iter().filter(|a| a.energy <= 0.0).count() as f64 / n, agents.iter().map(|a| a.fat).sum::<f64>(), agents.iter().map(|a| a.age as f64).sum::<f64>() / n,
                at[0], at[1], at[2], cross[0], cross[1], cross[2]).unwrap();
        }
        if step % LINEAGE_INTERVAL == 0 {
