...
const BODY_KEYS: [&str; 5] = ["life", "steps", "start", "scale", "water"];
...
impl Sim {
    ...
    fn bodies(&mut self) {
        let Sim { p, w, agents, pl, occ, g, rng, wear_rng, k, hab, laws, cache, threads, next_id, born, dead, .. } = self;
        let g = *g;
    ...
    const AGENT_HEADER: &'static str = "step,id,lineage,age,turns,mass,size,side,density,born_size,born_mass,born_hard,born_muscle,born_sensor,born_digestive,born_bite,\
        hard,muscle,sensor,digestive,bite,bite_any,shell,speed,energy,fat,plant,meat,killed,scavenged,travel,path,worn,pace,place,born_place,temp,moist,height,medium,algae,detritus,cells";
    ...
    fn agent_rows(&self, out: &mut dyn Write) {
        ...
        for a in &self.agents {
            ...
            writeln!(
                out,
                "{},{},{},{},{},{:.2},{},{},{:.3},{},{:.2},{},{},{},{},{},{},{},{},{},{},{},{:.2},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.2},{:.2},{},{:.3},{},{},{:.1},{:.3},{:.0},{},{:.3},{:.3},{}",
                self.step,
                a.id,
                a.lineage,
                a.age,
...
mod tests {
    ...
    /// with the water closed and open.
    #[test]
    fn matter_is_conserved_with_bodies() {
        for (scale, water) in [(1.0, 0.0), (0.0625, 0.0), (0.0625, 1.0)] {
            conserved_at(scale, water);
        }
    }

    fn conserved_at(scale: f64, water: f64) {
        let mut p = small();
        p.scale = scale;
        p.water = water;
