...
impl Sim {
    ...
    fn band_rows(&mut self, step: u64, out: &mut dyn Write) {
        ...
        let n = self.w.n;
        let mut acc = vec![[0.0f64; 6]; BANDS * 3];
        ...
        for (k, r) in acc.iter().enumerate() {
            if r[0] > 0.0 {
                writeln!(out, "{step},{},{},{},{:.4},{:.2},{:.2},{:.2},{}", (k / 3 * 10) as i64 - 60, k % 3, r[0], r[1], r[2], r[3], r[4] / r[0], r[5]).unwrap();
            }
        }
...
fn main() {
    ...
    writeln!(bands_csv, "step,lat,wood_class,cells,grown,grass,wood,temp,bodies").unwrap();
