...
impl Patches {
    fn new(g: Grid, seed: u64, sigmas: Vec<f32>, bands: Vec<u8>, sun: f32) -> Self {
        ...
        let mut p = Patches { centers, sigmas, grow: vec![0.0; g.cells()], place: vec![NO_PLACE; g.cells()], bands, rng, sun };
        p.field(g);
        p
    }
    ...
    fn field(&mut self, g: Grid) {
        ...
        for (k, &(cx, cy)) in self.centers.iter().enumerate() {
            ...
            let peak = RES_GROWTH * self.sun * PATCH_AREA as f32 / (2.0 * std::f32::consts::PI * sigma * sigma);
...
fn main() {
    ...
    let mut patches = Patches::new(g, seed, sigmas.clone(), terrain.band.clone(), sun);
    ...
    let prefix = format!("experiments/e053_bandclock/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}{sat_tag}{hold_tag}{wear_tag}{connect_tag}{yield_tag}{motor_tag}{band_tag}{brain_tag}{clock_tag}_seed{seed}", rain.name());
    ...
    writeln!(places_csv, "step,place,pop,mass,hard,muscle,sensor,digestive,bite,shell,biters,cover,plant_intake,meat_intake,lineages,movers,foot,shell_front,shell_back,dead,carrion,soil,barren,regrowth,cells,rain,trees,fruit,fruit_intake,water,dry,root,root_intake,fill,pool").unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.4},{:.4}", pace_sum / clock_steps.max(1) as f64, clock_turns as f64 / clock_steps.max(1) as f64).unwrap();
            if band > 0 {
                let front = band_front(step, band, w);
            ...
            for p in 0..=N_PLACES as u8 {
                ...
                writeln!(places_csv, ",{:.3},{:.4}", mean(&|a| a.water), pool_here as f64 / cells_at[p as usize].max(1) as f64).unwrap();
            }
