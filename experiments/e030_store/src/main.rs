...
//! line, 2 the sharp curve), `side` (a number up to 16, or grow; 8 by default) and `store`
//! (fat per unit of mass the flesh can hold, 0: e029's law, the fat neither spent nor bounded).
...
fn main() {
    ...
    let store: f32 = args.get(21).and_then(|s| s.parse().ok()).unwrap_or(0.0);
    ...
    let prefix = format!("experiments/e030_store/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"digest\":{digest_law},\"digest_floor\":{DIGEST_FLOOR},\"side\":\"{}\",\"side_max\":{SIDE_MAX},\"store\":{store},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), side_law.name(), list(&terrain.height)).unwrap();
    }
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{side_mean:.2},{side_std:.2},{},{},{},{},{:.4},{:.4},{on_fat:.4}", sq(0.1), sq(0.5), sq(0.9), sizes.last().copied().unwrap_or(0), fat_spent / LOG_INTERVAL as f64, fat_over / LOG_INTERVAL as f64).unwrap();
            sun_acc = 0.0;
