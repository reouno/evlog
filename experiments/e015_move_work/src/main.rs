//! e015: work is force times distance. e014's world (space at the resolution of the body,
//! facing, two kinds of place, contact physics, a cell costs what it holds, per-body cost) with
//! its law for the cost of a move rewritten:
//!
//! - A move costs by what was moved and how far. The mover pays MOVE_COST per cell of mass per
//!   sub-cell it moved, and the same for every body it shoved. A forward action that moves
//!   nothing (the way stays taken) costs nothing: the faces meet, the softer breaks if the
//!   muscle behind the other exceeds its hardness, and that is all. e014 charged a blocked move
//!   as if the body had moved one sub-cell, which made a push a losing action and a free turn
//!   its substitute.
//!
//! Everything else is e014: the world, the materials, the contact rule, facing, mating,
//! lineages and the measures. Two counters are added to the log: `pushes` (forward actions that
//! pressed on at least one body) and `move_spent` (energy paid for moving per body per step).
...
fn main() {
    ...
    let prefix = format!("experiments/e015_move_work/results/{size}_sigma{sigma_name}_seed{seed}");
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.3},{:.5}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64).unwrap();
