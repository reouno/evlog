...
fn main() {
    ...
    // of the step (0: what it cannot pay is dropped, e041 byte for byte).
    let strict: u8 = args.get(38).and_then(|s| s.parse().ok()).unwrap_or(0);
    ...
    let prefix = format!("experiments/e042_strict/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}_seed{seed}", rain.name());
    ...
    writeln!(pop_csv, "step,pop,on_fat,fat_stock,age_mean,pop0,pop1,pop2,cross0,cross1,cross2,soil0,soil1,soil2,root0,root1,root2,fill0,fill1,fill2,drink0,drink1,drink2").unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            writeln!(pop_csv, "{step},{},{:.4},{:.1},{:.0},{},{},{},{},{},{},{:.0},{:.0},{:.0},{:.0},{:.0},{:.0},{:.3},{:.3},{:.3},{},{},{}", agents.len(), agents.iter().filter(|a| a.energy <= 0.0).count() as f64 / n, agents.iter().map(|a| a.fat).sum::<f64>(), agents.iter().map(|a| a.age as f64).sum::<f64>() / n,
                at[0], at[1], at[2], cross[0], cross[1], cross[2], soil_at[0], soil_at[1], soil_at[2], root_at[0], root_at[1], root_at[2],
                fill_at[0] / at[0].max(1) as f64, fill_at[1] / at[1].max(1) as f64, fill_at[2] / at[2].max(1) as f64, drink_at[0], drink_at[1], drink_at[2]).unwrap();
        }
        if step % LINEAGE_INTERVAL == 0 {
