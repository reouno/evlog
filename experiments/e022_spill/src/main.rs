//! e022: the spill. e021's closed world with its canopy (the matter standing on a cell is a
//! column, and a taller column takes, from every cell within its height in cells, a share of
//! that cell's sun; what a body burns rises to the air and rains on the mountains; the ground
//! has a height and soil runs downhill) with the canopy's saturation dropped and one law added
//! about what a column cannot hold:
//!
//! - A full crown keeps taking the light it stands in. The canopy claims at full strength
//!   whatever the column's height (e021 scaled the claim by the column's room, so that a full
//!   crown intercepted nothing), and a column under a body claims too: the light it takes from
//!   around it is the crown's, above the body; only the cell's own sun falls in the body's
//!   shadow (e016).
//! - What a column cannot hold falls as fruit. The growth a column's light and soil would give
//!   it past the cap, or while a body holds it, falls in equal shares on the cells within
//!   `spill` of it (the ring of 8 at 1) as plant matter lying on the ground: eaten by any gut,
//!   rotting into the soil at DECAY if nobody eats it, and a part of the column it lies on.
//!
//! In e021 a tree was silenced by the body that ate it (a held cell neither grew nor claimed),
//! so a tree fed one gut at a time. Now a tree at the cap is a fountain: it takes the sun of
//! the lawn around it (up to its whole reach, 200 suns at rate 2) and lays it as fruit on its
//! ring, where a crowd can eat without standing on the tree - e011's rich cell, closed. The
//! sun and the matter are only moved, never made; a column with no soil takes light and wastes
//! it as before (`barren`).
//!
...
//! probability; 0 is e021's two per child). The log gets `fruit` (fallen per step),
...
fn main() {
    ...
    let prefix = format!("experiments/e022_spill/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}_seed{seed}", rain.name());
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.3},{:.5},{:.2},{:.4},{:.2},{soil_stock:.1},{matter:.1},{:.3},{:.4},{:.3},{:.3},{soil_cells:.3},{deep:.3},{air:.1},{:.3},{:.3},{trees},{tree_res:.1},{res_max:.2},{:.3},{:.3},{fruit_stock:.1},{:.3},{:.3},{:.2}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64, shaded / LOG_INTERVAL as f64,
