...
fn main() {
    ...
    // byte for byte).
    let connect: u8 = args.get(42).and_then(|s| s.parse().ok()).unwrap_or(0);
    ...
    let prefix = format!("experiments/e045_connect/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}{sat_tag}{hold_tag}{wear_tag}{connect_tag}_seed{seed}", rain.name());
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            for (k, c) in guts.iter().enumerate() {
                ...
                let dung = (plant + fruit + dug) * (1.0 - plant_yield) + dead * (1.0 - flesh_yield);
                food.spend(c, dung);
                cc.dung += dung;
                eaten += took + dug - dung;
                dead_eaten += (dead * flesh_yield) as f32;
                plant_at[patches.place[c] as usize] += ((plant + fruit + dug) * plant_yield) as f32;
