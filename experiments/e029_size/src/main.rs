...
//! line, 2 the sharp curve) and `side` (a number up to 16, or grow; 8 by default).
...
impl Laws {
    fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        let morph_level = (0..=SIDE_MAX)
            ...
            .collect();
        Laws { morphogen, table, density, digest, side, morph_level }
    }
}
...
impl Body {
    fn new(cells: [u8; CELLS], side: usize, policy: [f32; N_POLICY], n_genes: u16, density: f32, by_kind: bool) -> Self {
        let mut b = Body { cells, side: side as u8, size: 0, mass: 0.0, density, digest: 0.5, by_kind, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE_MAX]; 4], extent: [0; 2], policy, n_genes };
        b.refresh();
        b
    }
    fn s(&self) -> usize {
...
fn main() {
    ...
    let prefix = format!("experiments/e029_size/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"digest\":{digest_law},\"digest_floor\":{DIGEST_FLOOR},\"side\":\"{}\",\"side_max\":{SIDE_MAX},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), side_law.name(), list(&terrain.height)).unwrap();
    }
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent,shaded,dead,carrion,soil,matter,barren,rot,spent,flow,soil_cells,deep,air,rain,shade,trees,tree_res,res_max,tree_eaten,fruit,fruit_stock,fruit_eaten,clones,mutations,fat_mean,fat_stock,worth,kill_gain,size_mean,density_mean,density_std,sun,cloud_std,digest_mean,digest_std,flesh_guts,dung,side_mean,side_std,size_p10,size_p50,size_p90,size_max"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{side_mean:.2},{side_std:.2},{},{},{},{}", sq(0.1), sq(0.5), sq(0.9), sizes.last().copied().unwrap_or(0)).unwrap();
            sun_acc = 0.0;
