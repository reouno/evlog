...
//!   (`Body::bite_any`, e010's rule applied to a plant); `plants.rs` serves it.
//! - **C, the fat weighs**: `Agent::load` is the mass the fat adds, and `Agent::mass` is what the clock,
//!   the work of a move and a shove read. The fat the flesh holds per unit of mass is read from the
//!   genome (e069's `store` alone) when `Laws::store_gene` is on.
//! - **D, the sea does not quench** (`dry_turn`): a block drinks what the cell under it gives
//!   (`drink_at`), which is all of `drink` on a pool, a share of it on wet ground and none in the sea.
//! - **E, light** (`sense`): a sensor block sees its reach times the light where the body stands.
//! - **F, height** and **G, the runoff's soil** are laws of a move and of the climate (`main.rs`,
//!   `climate.rs`).
...
pub const KIND_MASS: [f32; N_KINDS] = [0.0, 2.0, 1.0, 0.5, 1.0, 1.0, 1.0]; // empty, hard, muscle, sensor, gut, leaf, leg (e025, e093, e095)
...
pub const LEAF: usize = 5; // e093 (#103): a block that takes matter from the soil by its open faces and the light
pub const LEG: usize = 6; // e095 (#107): a block that adds to the motor force by its faces open to the air
// e093: the law table keeps the four older kinds and the policy in the columns it always had, and the
// leaf's column is drawn from a stream of its own (as the density and the side are, e059). So a world
// with the light off develops exactly the bodies e092's did, and the six-seed control ladder stands.
const N_TABLE_KINDS: usize = 5;
...
const K: usize = N_TABLE_KINDS + N_POLICY;
...
/// e095 (#107): is the block at `pos` a spike along the line that runs `into` the body - a hard
/// block at the front of its line with nothing beside it? Nothing of the body may stand ahead of it
/// or to either side across the line (off the grid counts as nothing). A hard block inside a flat
/// armoured face is not one, and neither is a soft block. The same plate is a spike seen end on: a
/// point is a point of the shape and of the direction it presses.
fn is_spike(cells: &[u8; CELLS], pos: usize, into: usize, s: usize) -> bool {
    if cells[pos] != HARD as u8 {
        return false;
    }
    let across = if into == NORTH || into == SOUTH { [EAST, WEST] } else { [NORTH, SOUTH] };
    let clear = |d: usize| neighbor(pos, d, s).is_none_or(|q| cells[q] == 0);
    clear(opposite(into)) && across.iter().all(|&d| clear(d))
}

/// e095 (#107): how many of a body's blocks are a spike toward at least one of the four sides - the
/// shape the law pays for, read off the grid a genome develops and so heritable.
pub fn spikes_of(b: &Body) -> usize {
    let s = b.s();
    (0..s * s).filter(|&p| DIRS.iter().any(|&d| is_spike(&b.cells, p, d, s))).count()
}
...
pub struct Laws {
    ...
    density: Vec<f32>,
    side: Vec<f32>,
    leaf: Vec<f32>, // e093: the leaf block's column, from its own stream
    pub leaf_on: bool, // e093: off, no cell develops a leaf block and the body is e092's
    leg: Vec<f32>, // e095 (#107): the leg block's column, from a stream of its own
    pub leg_on: bool, // e095: off, no cell develops a leg block and the body is e092's
    ...
    pub store_gene: bool, // e072 (set C): read the store alone from the genome
...
impl Laws {
    pub fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        let mut rng7 = Rng(seed.wrapping_mul(0xD6E8FEB86659FD93).wrapping_add(0xA3B195354A39B70D) | 1);
        let leaf = (0..256).map(|_| rng7.f32() * 2.0 - 1.0).collect();
        let mut rng8 = Rng(seed.wrapping_mul(0xC3A5C85C97CB3127).wrapping_add(0xB492B66FBE98F273) | 1);
        let leg = (0..256).map(|_| rng8.f32() * 2.0 - 1.0).collect();
        ...
        let morph_level = (0..=SIDE_MAX)
            ...
            .collect();
        Laws { morphogen, table, density, side, leaf, leaf_on: false, leg, leg_on: false, water, breath, breed, share, store, history: false, store_gene: false, morph_level }
    }
}
...
pub struct Body {
    ...
    pub leg_open: u16, // e095 (#107): faces of leg blocks open to the air - the only ones that pull
...
impl Body {
    pub fn new(cells: [u8; CELLS], side: usize, policy: [f32; N_POLICY], n_genes: u16, density: f32) -> Self {
        let mut b = Body { cells, side: side as u8, size: 0, mass: 0.0, density, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE_MAX]; 4], extent: [0; 2], policy, n_genes, cut: 0, water_policy: [0.0; N_WATER_POLICY], open_soft: 0, leg_open: 0, breath_policy: [0.0; N_OUT], breed: BREED, share: SHARE, store: STORE, sight: [0; 4] };
        b.refresh();
        b
    }
    pub fn empty() -> Self {
    ...
    pub fn refresh(&mut self) {
        ...
        let open = open_faces(&self.cells, self.s());
        self.open_soft = open.iter().map(|&f| f as u16).sum();
        // e095 (#107): a leg pulls only where it touches the outside. Turning a body permutes its
        // grid, so what is open in the birth frame is open in the world's.
        self.leg_open = (0..self.s() * self.s()).filter(|&p| self.cells[p] == LEG as u8).map(|p| open[p] as u16).sum();
        self.sight = sight_of(&self.cells, self.s());
    }
    /// Muscle over mass (e048), and e095's legs (#107): a leg block adds `leg` to the motor for each
    /// of its faces open to the air and nothing where it is walled in, and the sum is divided by the
    /// mass as the muscle is. At `leg` 0 this is e048's own number to the bit.
    pub fn speed(&self, leg: f32) -> f32 {
        if self.mass <= 0.0 {
            return 0.0;
        }
        let motor = self.kinds[MUSCLE] as f32 + leg * self.leg_open as f32;
        motor / self.mass
    }
...
pub fn develop_genes(genes: &[Gene], laws: &Laws) -> Body {
    ...
    // e093 (#103), e095 (#107): a kind added after the table has a column of its own, drawn from a
    // stream of its own. With its law off it is not scored and no cell develops one, so the bodies
    // are e092's to the bit - and the control ladder is the one on disk.
    let leaf_col: Vec<f32> = if laws.leaf_on { genes.iter().map(|g| laws.leaf[pattern_index(&g.product)]).collect() } else { Vec::new() };
    let leg_col: Vec<f32> = if laws.leg_on { genes.iter().map(|g| laws.leg[pattern_index(&g.product)]).collect() } else { Vec::new() };
    for (c, cell) in cells.iter_mut().enumerate().take(ncell) {
        let mut score = [0.0f32; N_KINDS];
        for (i, row) in rows.iter().enumerate() {
            let lv = level[i * ncell + c];
            for k in 0..N_TABLE_KINDS {
                score[k] += row[k] * lv;
            }
            if laws.leaf_on {
                score[LEAF] += leaf_col[i] * lv;
            }
            if laws.leg_on {
                score[LEG] += leg_col[i] * lv;
            }
        }
        let mut best = 0;
        for k in 1..N_TABLE_KINDS {
            if score[k] > score[best] {
                best = k;
            }
        }
        for (k, on) in [(LEAF, laws.leaf_on), (LEG, laws.leg_on)] {
            if on && score[k] > score[best] {
                best = k;
            }
        }
        *cell = best as u8;
    ...
    for (i, row) in rows.iter().enumerate() {
        for k in 0..N_POLICY {
            policy[k] += row[N_TABLE_KINDS + k] * free[i];
        }
    }
    ...
    // The values of a life (e069), each from its own column as the density is. e072's set C reads the
    // store alone that way, and leaves the energy to breed and the child's share constants.
    ...
    if laws.history || laws.store_gene {
...
pub struct Agent {
    ...
    pub light: f32, // e093 (#103): lifetime intake its leaf blocks took from the soil by the light
    pub sharp: f32, // e095 (#107): lifetime intake it took through a spike
    pub fat: f64,
    pub born_size: u16,
    pub born_mass: f32,
    ...
    pub open_hard: [u8; CELLS], // e072 (set A): the same for the hard blocks, which pass a share of the heat
    ...
    pub temp: f32, // e072 (set A): the body's temperature, C
    pub warmed: f32, // e072 (set A): energy it has paid to warm itself
    pub cooled: f32, // e072 (set A): water it has paid to cool itself, in fills
    pub cold_short: bool, // e072 (set A): this turn's warming was not paid in full, the upkeep was
    pub load: f32, // e072 (set C): the mass its fat adds
    pub wood: f32, // e072 (set B): lifetime intake from wood
    pub invader: u8, // e074 (#72): 1 for a body the injection put into the world, and for its descendants
    pub held: f64, // e081 (W2): the water it holds as the land's ledger last booked it, in blocks of water
    pub drunk: f32, // e081 (W1): blocks of water it has taken from the ground
    pub layer: usize,   // e091 (#101 C1): the layer it holds, SURFACE or BOTTOM on the floor (e065), CROWN in a crown
    pub fruit: f32,     // e091 (#101 C2): of `plant`, the crown's fruit it took
    pub seed: f32,      // e089 (#99 S): of `plant`, the seed it took
    pub fiber: f64,     // e089 (#99 Fb): the fiber in its gut, not yet fermented or passed
    pub fermented: f32, // e089: lifetime energy from its gut's fiber
    pub dunged: f32,    // e089: lifetime fiber passed as dung
}
...
impl Agent {
    ...
    pub fn new(id: u64, genome: Vec<u8>, keys: Vec<u16>, gene_ids: Vec<u16>, body: Body, facing: u8, energy: f64, lineage: u32) -> Agent {
        let mut a = Agent {
            id, lineage, x: 0, y: 0, energy, age: 0, plant: 0.0, scavenged: 0.0, killed: 0.0, algae: 0.0, detritus: 0.0, light: 0.0, sharp: 0.0, fat: 0.0, born_size: 0, born_mass: 0.0, born_kinds: [0; N_KINDS], born_bite: 0, born_at: (0, 0), born_hab: 0,
            turns: 0, phase: 0.0, path: 0, alive: false, short: false, worn: 0, worn_out: false, genome, keys, gene_ids, body: Body::empty(), facing,
            wcells: [0; CELLS], tips: [[Tip::default(); SIDE_MAX]; 4], filled: [0; CELLS], n_filled: 0, bbox: [0; 4], open: [0; CELLS], open_hard: [0; CELLS], water: 1.0, drank: 0, breath: 1.0, inhaled: 0, kids: 0,
            temp: 0.0, warmed: 0.0, cooled: 0.0, cold_short: false, load: 0.0, wood: 0.0, invader: 0, held: 0.0, drunk: 0.0, layer: SURFACE, fruit: 0.0, seed: 0.0, fiber: 0.0, fermented: 0.0, dunged: 0.0,
        };
        a.set_body(body);
        a
    }
    ...
    pub fn set_body(&mut self, body: Body) {
        ...
        self.body = body;
        self.layer = self.body.layer(); // e091: the floor it is born on; a crown takes it in `layer_for`
        self.reframe();
    }
    ...
    pub fn reframe(&mut self) {
        ...
        self.open_hard = hard_faces(&self.wcells, s);
    }
    /// What the body weighs with the fat it carries (e072, set C): the clock, the work of a move and a
    /// shove read this. With `fat_weight` 0 the load stays 0 and this is the body's own mass (e070).
    pub fn mass(&self) -> f32 {
        self.body.mass + self.load
    }
    ...
    pub fn here(&self, g: Grid) -> usize {
        self.here_at(g, NORTH, 0)
    }
    /// The same, moved k sub-cells in direction d (e091: the cell whose crown it would stand in).
    pub fn here_at(&self, g: Grid, d: usize, k: usize) -> usize {
        let [r0, r1, c0, c1] = self.bbox;
        let (sx, sy) = self.sub_at(g, (r0 + r1) as usize / 2 * self.body.s() + (c0 + c1) as usize / 2, d, k);
        g.wcell(sx, sy)
    }
    /// Whether every cell of the body, moved k sub-cells in direction d, lands on a free sub-cell
    /// or on one of its own, in the layer it would stand in there (e091: the crown of the cell it
    /// would be over, whose sub-cells are a wall where no crown stands).
    pub fn fits(&self, g: Grid, occ: &Occ, me: u32, d: usize, k: usize) -> bool {
        self.fits_in(g, occ, me, occ.layer_for(g, self, d, k), d, k)
    }
    /// The same in a layer given (e091: what a body tests before it climbs or comes down).
    pub fn fits_in(&self, g: Grid, occ: &Occ, me: u32, layer: usize, d: usize, k: usize) -> bool {
...
/// e091 (#101 C1): the crown over a stand of wood, a third place a body stands in. A body in a
/// crown holds this layer and nothing of the floor; a body on the floor holds the other two, as
/// it always did. Where no crown stands the layer is a wall, so a crown's body cannot step off it.
pub const CROWN: usize = 2;
pub const LAYERS: usize = 3;
/// Land, the water's surface, its bottom, and (e091) the crown: what `Occ::medium` reads.
pub const N_MEDIA: usize = 4;

/// Occupancy, per layer: the body holding each sub-cell (FREE, WALL, or its index) at
/// `LAYERS * sidx + layer`, and per world cell the sub-cells bodies hold (the crowd there, what a
/// body sees) at `LAYERS * cell + layer`. A land sub-cell holds its body in the water's two layers;
/// e091 (#101 C1) adds the crown, which a body holds alone.
pub struct Occ {
    pub sub: Vec<u32>,
    pub crowd: Vec<u16>,
    wet: Vec<bool>, // per world cell: water, where the layers are apart
    lift: Vec<f32>, // e091 (C3): the mass a cell's crown holds (0: no crown there)
}

impl Occ {
    /// `open`: bodies may live in the water (e065); else the sea is a wall. The crown's layer is a
    /// wall until `set_crowns` opens it (e091).
    pub fn new(g: Grid, sea: &[bool], open: bool) -> Self {
        let mut sub = vec![FREE; LAYERS * g.sw * g.sh];
        for sy in 0..g.sh {
            for sx in 0..g.sw {
                let i = LAYERS * g.sidx(sx, sy);
                sub[i + CROWN] = WALL;
                if !open && sea[g.wcell(sx, sy)] {
                    sub[i + SURFACE] = WALL;
                    sub[i + BOTTOM] = WALL;
                }
            }
        ...
        Occ { sub, crowd: vec![0; LAYERS * g.cells()], wet, lift: vec![0.0; g.cells()] }
    }
    /// e091 (#101 C1, C3): a land cell whose wood is `at` or more carries a crown, and it holds a
    /// body of at most `hold` x its wood. Called after every producers' update: a crown that comes
    /// opens its layer, one that goes walls the sub-cells nothing stands on, and the bodies still in
    /// it come down at their turn (`Occ::layer_for`).
    pub fn set_crowns(&mut self, g: Grid, wood: &[f64], at: f64, hold: f64) {
        for c in 0..g.cells() {
            let has = !self.wet[c] && wood[c] >= at;
            let was = self.lift[c] > 0.0;
            self.lift[c] = if has { (hold * wood[c]) as f32 } else { 0.0 };
            if has == was {
                continue;
            }
            let (x0, y0) = (c % g.w * SUB, c / g.w * SUB);
            for dy in 0..SUB {
                for dx in 0..SUB {
                    let i = LAYERS * g.sidx(x0 + dx, y0 + dy) + CROWN;
                    if has && self.sub[i] == WALL {
                        self.sub[i] = FREE;
                    } else if !has && self.sub[i] == FREE {
                        self.sub[i] = WALL;
                    }
                }
            }
    ...
    /// e091: whether cell c carries a crown, and the mass it holds.
    pub fn crowned(&self, c: usize) -> bool {
        self.lift[c] > 0.0
    }
    pub fn lift(&self, c: usize) -> f32 {
        self.lift[c]
    }
    /// e091 (C1, C3): the layer the body belongs in where it stands, moved k sub-cells in direction
    /// d: the crown of the cell under its middle when that cell carries one and holds its mass,
    /// else the layer of the water it would swim in (e065), which is the floor on land.
    pub fn layer_for(&self, g: Grid, a: &Agent, d: usize, k: usize) -> usize {
        let c = a.here_at(g, d, k);
        if self.lift[c] > 0.0 && a.mass() <= self.lift[c] {
            CROWN
        } else {
            a.body.layer()
        }
    ...
    fn layers(&self, c: usize, layer: usize) -> std::ops::Range<usize> {
        if layer == CROWN || self.wet[c] { layer..layer + 1 } else { 0..2 }
    }
    /// 0 on land, 1 at the surface, 2 on the bottom, 3 in a crown (e091): the medium the body stands in.
    pub fn medium(&self, g: Grid, a: &Agent) -> usize {
        if a.layer == CROWN {
            3
        } else if self.wet[a.here(g)] {
            1 + a.layer
        } else {
            0
        }
    ...
    fn put(&mut self, g: Grid, a: &Agent, pos: usize, v: u32) {
        ...
        for l in self.layers(c, a.layer) {
            self.sub[i + l] = v;
            if v == FREE {
...
pub struct Counters {
    ...
    pub kill_gain: f64, // what guts gained from the blocks they broke, the flesh of the tear with it
    pub flesh_gain: f64, // e075 (#90): of that, what the tear itself gave
    pub kill_by: [f64; N_MEDIA], // e065: the same, by the eater's medium (`Occ::medium`); e091 adds the crown
    pub dead: f64, // matter laid on the ground by the dead
    pub cut_break: u64,
    pub cut_wear: u64,
    pub sharp_gain: f64, // e095 (#107): what gut blocks took through a spike
    pub sharp_breaks: u64, // e095: blocks broken off by a spike
    pub cut_bodies: u64,
    pub born_cut: u64,
    pub not_built: u64,
}
...
/// breaker with a gut, else to the ground; `flesh` (e075, #90) is the share of what the torn body
/// still holds that goes with it. A body the breaks cut in parts keeps its largest part.
/// Returns, per body pressed, the force against it. The sea presses nothing.
pub fn push(agents: &mut [Agent], i: usize, d: usize, g: Grid, occ: &mut Occ, carrion: &mut [f64], s: f64, flesh: f64, spike: f32, c: &mut Counters) -> Vec<(usize, u8)> {
    let opp = opposite(d);
    let mut pressed: Vec<(usize, u8)> = Vec::new();
    let mut breaks: Vec<(usize, u8, usize, bool)> = Vec::new(); // (victim, world-frame cell, eater, by a spike)
    let layer = agents[i].layer;
    for p in agents[i].cells_held() {
        ...
        // e095 (#107): a spike - a hard block with nothing beside it across the line - concentrates
        // what is behind it: the line's force counts 1 + `spike` times over for the break. What it
        // shoves is unchanged (that is the whole body's weight against another's), and so is its own
        // face, which is as hard as the run of hard blocks behind it and no harder.
        let sharp = spike > 0.0 && is_spike(&agents[i].wcells, p, opp, agents[i].body.s());
        let force = if sharp { force as f32 * (1.0 + spike) } else { force as f32 };
        if hb < ha && force > hb {
            breaks.push((j, q as u8, i, sharp));
        } else if ha < hb && force > ha {
            breaks.push((i, p as u8, j, false));
        }
    }
    c.contacts += pressed.len() as u64;
    let mut victims: Vec<usize> = Vec::new();
    for &(v, _, _, _) in &breaks {
        if !victims.contains(&v) {
            victims.push(v);
        }
    }
    for (victim, pos, eater, sharp) in breaks {
        let gut = agents[eater].body.kinds[DIGESTIVE] > 0;
        ...
        v.reframe();
        c.cells_broken += 1;
        c.sharp_breaks += sharp as u64;
        if v.body.size == 0 && v.alive {
            v.alive = false;
            c.kills += 1;
        }
        // e075 (#90): the tear opens the flesh. A gut that breaks a block takes with it a share of
        // what the body still holds, its energy and its fat; a body with no gut only breaks the block.
        let torn = if gut && flesh > 0.0 {
            let (from_energy, from_fat) = (flesh * v.energy.max(0.0), flesh * v.fat);
            v.energy -= from_energy;
            v.fat -= from_fat;
            from_energy + from_fat
        } else {
            0.0
        };
        let taken = share + fat + matter + torn;
        let m = occ.medium(g, &agents[eater]);
        let e = &mut agents[eater];
        if gut {
            e.energy += taken;
            e.killed += taken as f32;
            // e095 (#107): what came in through a spike, of this body's life and of the world's.
            if sharp {
                e.sharp += taken as f32;
                c.sharp_gain += taken;
            }
            c.kill_gain += taken;
            c.flesh_gain += torn;
...
pub fn look<F: Fn(usize) -> f64, W: Fn(usize) -> f64>(a: &Agent, g: Grid, d: usize, k: usize, food: &F, water: &W, crowd: &[u16]) -> (f32, f32, f32) {
    ...
    for c in a.under(g, d, k * SUB).iter() {
        ...
        f += food(c) as f32;
        others += crowd[LAYERS * c + layer] as f32 / SUB_CELLS as f32;
        wt += water(c) as f32;
    }
    (f, others, wt)
}
...
/// e072 (set E): `lit` is the share of its reach the light where the body stands allows; 1 is e070.
pub fn sense<F: Fn(usize) -> f64, W: Fn(usize) -> f64>(a: &Agent, g: Grid, food: &F, water: &W, crowd: &[u16], senses: bool, lit: f32) -> Readings {
    ...
    for (j, &d) in dirs.iter().enumerate() {
        let full = if senses { a.body.reach(j) } else { a.body.range() };
        let reach = (full as f32 * lit).round() as usize;
        if reach == 0 {
            continue;
        }
...
/// The same for the hard blocks (e072, set A): a hard face is closed to the air and the water but it
/// still passes heat, at a share of what a soft face passes.
pub fn hard_faces(cells: &[u8; CELLS], s: usize) -> [u8; CELLS] {
    let mut out = [0u8; CELLS];
    for pos in 0..s * s {
        if cells[pos] != HARD as u8 {
            continue;
...
/// e072 (set D) puts what a cell gives to drink in `drink_at`: 1 over any water when the sea quenches
/// (e066), else 1 over a pool, a share over wet ground and 0 over the sea.
pub fn dry_turn(a: &mut Agent, g: Grid, wet: &[bool], dryness: &[f32], drink_at: &[f32], dry: f32, drink: f32) -> bool {
    ...
    a.water = (a.water + (drink * drunk - dry * lost) / a.body.size.max(1) as f32).min(1.0);
    drunk > 0.0
}

/// e081 (W1, W2): `dry_turn` with a body's water part of the land's. A block drinks what the cell under
/// it gives, as today, but takes it out of that cell's ground (a pool's standing water is ground above
/// the soil's capacity), and only as much as brings the body to full: the turn's offer is cut in
/// proportion over the blocks, then each takes what its cell holds. What the soft faces lose goes to
/// the air over the cell each loses it in, by its share of the loss. `unit` is the mm of a cell a block
/// of water holds; `a.held` must be the body's water as booked before the turn (`settle`). Returns
/// whether a block was over water, and the mm taken from the ground and given to the air.
#[allow(clippy::too_many_arguments)]
pub fn dry_turn_w(a: &mut Agent, g: Grid, wet: &[bool], dryness: &[f32], drink_at: &[f32], dry: f32, drink: f32, unit: f64, ground: &mut [f64], vapor: &mut [f64]) -> (bool, f64, f64) {
    let (mut lost, mut offer, mut drunk) = (0.0f32, 0.0f32, 0.0f32);
    for p in a.cells_held() {
        let (sx, sy) = a.sub_at(g, p, NORTH, 0);
        let c = g.wcell(sx, sy);
        drunk += drink_at[c];
        offer += drink * drink_at[c];
        if !wet[c] {
            lost += dry * dryness[c] * a.open[p] as f32;
        }
    }
    let size = a.body.size.max(1) as f32;
    let room = (1.0 - a.water) * size + lost;
    let frac = if offer > 0.0 { (room / offer).clamp(0.0, 1.0) as f64 } else { 0.0 };
    let (mut got, mut given) = (0.0f64, 0.0f64);
    if frac > 0.0 {
        for p in a.cells_held() {
            let (sx, sy) = a.sub_at(g, p, NORTH, 0);
            let c = g.wcell(sx, sy);
            let want = frac * (drink * drink_at[c]) as f64 * unit;
            let take = want.min(ground[c].max(0.0));
            ground[c] -= take;
            got += take;
        }
    }
    a.water = (a.water + ((got / unit) as f32 - lost) / size).min(1.0);
    let after = a.water.max(0.0) as f64 * a.body.size as f64;
    let out = (a.held + got / unit - after).max(0.0) * unit;
    if out > 0.0 {
        for p in a.cells_held() {
            let (sx, sy) = a.sub_at(g, p, NORTH, 0);
            let c = g.wcell(sx, sy);
            if !wet[c] && lost > 0.0 {
                let mm = out * (dry * dryness[c] * a.open[p] as f32 / lost) as f64;
                vapor[c] += mm;
                given += mm;
            }
        }
    }
    // What the shares do not place (a loss of zero, the rounding) stays in the booking.
    a.held += got / unit - given / unit;
    a.drunk += (got / unit) as f32;
    (drunk > 0.0, got, given)
}
...
mod tests {
    ...
    fn a_push_meets_face_to_face() {
        ...
        let pressed = push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.0, 0.0, &mut c);
        ...
        occ.claim(g, &agents[1], 1);
        let mut carrion = vec![0.0; g.cells()];
        push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.0, 0.0, &mut c);
        assert!((agents[0].killed as f64 - (CELL_ENERGY + 0.5) as f64).abs() < 1e-6);
        assert_eq!(carrion.iter().sum::<f64>(), 0.0);
    }

    /// e075 (#90): the tear. A gut that breaks a block takes a share of what the torn body still
    /// holds besides the block, and what is torn leaves the victim: nothing is made or lost.
    #[test]
    fn a_tear_opens_the_flesh() {
        let g = Grid::new(8, 8);
        let mut gutted = tooth();
        gutted[3 * SIDE + 2] = DIGESTIVE as u8;
        let mut soft = [0u8; CELLS];
        ...
        let mut agents = vec![agent_with(gutted, 0, 8, EAST), agent_with(soft, 8, 10, NORTH)];
        agents[1].fat = 2.0;
        let (energy, fat) = (agents[1].energy, agents[1].fat);
        let mut occ = land(g);
        occ.claim(g, &agents[0], 0);
        occ.claim(g, &agents[1], 1);
        let mut c = Counters::default();
        let mut carrion = vec![0.0; g.cells()];
        push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.25, 0.0, &mut c);
        // The block is half the prey's energy and its matter; the tear is a quarter of what the
        // prey holds after that break: (energy / 2 + fat / 2) / 4.
        let block = CELL_ENERGY as f64 + energy / 2.0 + fat / 2.0;
        let torn = 0.25 * (energy / 2.0 + fat / 2.0);
        assert!((c.flesh_gain - torn).abs() < 1e-9, "torn {} against {torn}", c.flesh_gain);
        assert!((agents[0].killed as f64 - (block + torn)).abs() < 1e-6);
        assert!((agents[1].energy + agents[1].fat - (energy + fat) / 2.0 + torn).abs() < 1e-9);
        assert_eq!(carrion.iter().sum::<f64>(), 0.0);
        // A breaker with no gut only breaks the block: nothing is torn out of the prey.
        let mut agents = vec![agent_with(tooth(), 0, 8, EAST), agent_with(soft, 8, 10, NORTH)];
        agents[1].fat = 2.0;
        let mut occ = land(g);
        occ.claim(g, &agents[0], 0);
        occ.claim(g, &agents[1], 1);
        ...
        let mut carrion = vec![0.0; g.cells()];
        push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.25, 0.0, &mut c2);
        assert_eq!(c2.flesh_gain, 0.0);
        assert!((agents[1].energy + agents[1].fat - (energy + fat) / 2.0).abs() < 1e-9);
    }
    ...
    fn the_sea_is_a_wall() {
        ...
        let mut c = Counters::default();
        let mut carrion = vec![0.0; g.cells()];
        assert!(push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.0, 0.0, &mut c).is_empty());
    ...
    fn the_water_has_two_layers() {
        ...
        // by its medium.
        let layer_sum = |occ: &Occ, l: usize| occ.crowd.iter().skip(l).step_by(LAYERS).map(|&n| n as u32).sum::<u32>();
        let mut occ = Occ::new(g, &sea, true);
    ...
    /// e091 (#101 C1, C3): a land cell whose wood is `crown_at` or more carries a crown. A body
    /// light enough for it stands there and holds nothing of the floor, so a heavy body walks
    /// under it; the crown's layer is a wall where no crown stands, so nothing hangs over the
    /// lawn; and when the wood goes, so does the crown.
    #[test]
    fn a_crown_is_a_place_over_the_floor() {
        let g = Grid::new(8, 8);
        let sea = vec![false; g.cells()];
        let mut occ = Occ::new(g, &sea, true);
        // Cells 9 and 10 (the second row) stand wood; the rest of the world is lawn.
        let mut wood = vec![0.0; g.cells()];
        wood[9] = 4.0;
        wood[10] = 4.0;
        occ.set_crowns(g, &wood, 1.0, 0.25); // a crown holds a body of 1 there
        let mut soft = [0u8; CELLS];
        soft[0] = MUSCLE as u8;
        // A body over cell 9 (sub-cells 4-7 both ways): light enough to be held, and a heavy one.
        let light = agent_of(soft, 5, 5, NORTH, 0.5);
        let heavy = agent_of(soft, 5, 5, NORTH, 1.5);
        assert!(light.mass() <= occ.lift(9) && heavy.mass() > occ.lift(9), "{} {}", light.mass(), heavy.mass());
        assert_eq!(occ.layer_for(g, &light, NORTH, 0), CROWN);
        assert_eq!(occ.layer_for(g, &heavy, NORTH, 0), BOTTOM);
        // They stand on the same sub-cells without meeting: the crown is not the floor.
        let mut agents = vec![light, heavy];
        agents[0].layer = CROWN;
        occ.claim(g, &agents[0], 0);
        assert!(agents[1].fits(g, &occ, 1, NORTH, 0));
        occ.claim(g, &agents[1], 1);
        assert_eq!((occ.medium(g, &agents[0]), occ.medium(g, &agents[1])), (3, 0));
        assert_eq!((occ.at(g, 5, 5, CROWN), occ.at(g, 5, 5, SURFACE), occ.at(g, 5, 5, BOTTOM)), (0, 1, 1));
        // Where no crown stands the layer is a wall, so nothing in a crown lies over the lawn: a
        // body whose middle moves off the stand comes down to the floor, one whose middle stays in
        // the crown while a block of it hangs over the lawn cannot move at all.
        assert_eq!(occ.at(g, 3, 5, CROWN), WALL);
        assert_eq!(occ.layer_for(g, &agents[0], WEST, 4), SURFACE);
        assert_eq!(occ.layer_for(g, &agents[0], EAST, 4), CROWN);
        assert!(agents[0].fits(g, &occ, 0, EAST, 4)); // cell 10 carries a crown too
        // The stand falls: the crown goes, and the body left in it comes down when it fits.
        wood[9] = 0.5;
        occ.set_crowns(g, &wood, 1.0, 0.25);
        assert!(!occ.crowned(9) && occ.crowned(10));
        assert_eq!(occ.layer_for(g, &agents[0], NORTH, 0), SURFACE);
        assert!(!agents[0].fits_in(g, &occ, 0, SURFACE, NORTH, 0), "the floor is taken");
        occ.release(g, &agents[1]);
        assert!(agents[0].fits_in(g, &occ, 0, SURFACE, NORTH, 0));
    }
    ...
    fn soft_faces_dry_in_the_air_and_blocks_in_water_drink() {
        ...
        let drink_at: Vec<f32> = wet.iter().map(|&w| w as u8 as f32).collect(); // e066's map: any water quenches
        ...
        assert!(!dry_turn(&mut a, g, &wet, &dryness, &drink_at, 0.1, 0.1));
        ...
        dry_turn(&mut a, g, &wet, &dryness, &drink_at, 0.1, 0.1);
        ...
        dry_turn(&mut b, g, &wet, &dryness, &drink_at, 0.1, 0.1);
    ...
    /// e072 (set A): the faces of a body pass heat by the difference, a hard face passes a share of
    /// what a soft face passes, and the fat stops it.
    #[test]
    fn heat_crosses_the_open_faces_and_the_fat_stops_it() {
        // World columns 0-3 are at -10 C (sub-cells x < 16), the rest at 30 C.
        let g = Grid::new(8, 8);
        let temp: Vec<f64> = (0..g.cells()).map(|c| if c % g.w < 4 { -10.0 } else { 30.0 }).collect();
        let mut pair = [0u8; CELLS];
        pair[0] = MUSCLE as u8;
        pair[1] = MUSCLE as u8;
        let mut a = agent_with(pair, 0, 0, EAST);
    ...
    #[test]
    fn the_fat_weighs_and_slows_the_clock() {
        let mut pair = [0u8; CELLS];
        pair[0] = MUSCLE as u8;
        pair[1] = MUSCLE as u8;
        let mut a = agent_with(pair, 0, 0, NORTH);
        assert_eq!(a.mass(), a.body.mass);
        a.fat = 20.0;
        a.load = 0.2 * a.fat as f32; // as the turn sets it, at fat_weight 0.2
        assert!((a.mass() - (a.body.mass + 4.0)).abs() < 1e-6);
        // A body of 16 muscles with a full store carries as much again, and its clock slows by it.
        let mut block = [0u8; CELLS];
        for y in 0..4 {
            for x in 0..4 {
                block[y * SIDE + x] = MUSCLE as u8;
            }
        }
        let mut b = agent_with(block, 0, 0, NORTH);
        b.fat = (b.body.store * b.body.mass) as f64;
        b.load = 0.2 * b.fat as f32;
        assert!((b.mass() - 2.0 * b.body.mass).abs() < 1e-5, "{} {}", b.mass(), b.body.mass);
        assert!(pace(b.mass()) < pace(b.body.mass), "{} {}", pace(b.mass()), pace(b.body.mass));
    }

    /// e072 (set D): a block drinks what the cell under it gives, and the dry air is unchanged.
    #[test]
    fn a_body_drinks_only_what_the_cell_gives() {
        // Columns 0-3 are land (sub-cells x < 16), the rest the sea, which no longer quenches.
        let g = Grid::new(8, 8);
        let wet: Vec<bool> = (0..g.cells()).map(|c| c % g.w >= 4).collect();
        let dryness = vec![0.5f32; g.cells()];
        let drink_at: Vec<f32> = (0..g.cells()).map(|c| if c % g.w >= 4 { 0.0 } else { 0.3 }).collect();
        let mut pair = [0u8; CELLS];
        pair[0] = MUSCLE as u8;
        pair[1] = MUSCLE as u8;
        let mut a = agent_with(pair, 16, 0, EAST);
        a.water = 0.5;
        // In the sea it neither dries nor drinks.
        assert!(!dry_turn(&mut a, g, &wet, &dryness, &drink_at, 0.1, 0.1));
        assert_eq!(a.water, 0.5);
        // On land it drinks 0.3 of the rate under each of its two blocks, and dries by its open faces.
        a.x = 0;
        assert!(dry_turn(&mut a, g, &wet, &dryness, &drink_at, 0.1, 0.1));
        assert!((a.water - (0.5 + (0.1 * 0.6 - 0.1 * 0.5 * 6.0) / 2.0)).abs() < 1e-6, "{}", a.water);
    }

    /// e081 (W1, W2): a body drinks out of the ground under it, only up to full, and what it loses goes
    /// to the air over it; the ground, the air and the body hold together what they held.
    #[test]
    fn a_body_drinks_out_of_the_ground_and_dries_into_the_air() {
        let g = Grid::new(8, 8);
        let wet = vec![false; g.cells()];
        let dryness = vec![0.5f32; g.cells()];
        let drink_at = vec![0.3f32; g.cells()];
        let mut pair = [0u8; CELLS];
        pair[0] = MUSCLE as u8;
        pair[1] = MUSCLE as u8;
        let mut a = agent_with(pair, 0, 0, EAST);
        let unit = 10.0;
        let total = |a: &Agent, ground: &[f64], vapor: &[f64]| a.held * unit + ground.iter().sum::<f64>() + vapor.iter().sum::<f64>();
        let turn = |a: &mut Agent, ground: &mut Vec<f64>, vapor: &mut Vec<f64>| dry_turn_w(a, g, &wet, &dryness, &drink_at, 0.01, 0.1, unit, ground, vapor);
        // Half full on wet ground: it takes the whole offer, 0.1 x 0.3 under each block, and loses its
        // open faces' 0.01 x 0.5 x 6, to the air.
        let (mut ground, mut vapor) = (vec![1000.0; g.cells()], vec![0.0; g.cells()]);
        (a.water, a.held) = (0.5, 1.0);
        let before = total(&a, &ground, &vapor);
        let (drank, got, given) = turn(&mut a, &mut ground, &mut vapor);
        assert!(drank);
        assert!((got - 0.06 * unit).abs() < 1e-6 && (given - 0.03 * unit).abs() < 1e-6, "took {got}, gave {given}");
        assert!((a.water - (0.5 + (0.06 - 0.03) / 2.0)).abs() < 1e-6, "{}", a.water);
        assert!((total(&a, &ground, &vapor) - before).abs() < 1e-9, "the ledger moved");
        // Nearly full it takes only what brings it to full: its room and what it loses, 0.02 + 0.03.
        (a.water, a.held) = (0.99, 1.98);
        let before = total(&a, &ground, &vapor);
        let (_, got, _) = turn(&mut a, &mut ground, &mut vapor);
        assert!((got - 0.05 * unit).abs() < 1e-4, "took {got}");
        assert_eq!(a.water, 1.0);
        assert!((total(&a, &ground, &vapor) - before).abs() < 1e-9, "the ledger moved");
        // On dry ground it takes nothing and only loses.
        let (mut dry_ground, mut vapor) = (vec![0.0; g.cells()], vec![0.0; g.cells()]);
        (a.water, a.held) = (0.5, 1.0);
        let (_, got, given) = turn(&mut a, &mut dry_ground, &mut vapor);
        assert_eq!(got, 0.0);
        assert!((a.water - (0.5 - 0.03 / 2.0)).abs() < 1e-6 && (given - 0.03 * unit).abs() < 1e-6);
        // A crowd draws its ground down: turn after turn on a cell of 1 mm, the ground runs out.
        let mut thin = vec![1.0; g.cells()];
        for _ in 0..40 {
            (a.water, a.held) = (0.5, 1.0);
            turn(&mut a, &mut thin, &mut vapor);
        }
        assert!(thin.iter().all(|&v| v >= 0.0) && thin.iter().any(|&v| v < 1e-9), "{:?}", &thin[..2]);
    }

    /// e072 (set E): a sensor block's reach follows the light, and in the dark it sees nothing.
    #[test]
    fn a_sensor_block_sees_as_far_as_the_light() {
        let g = Grid::new(8, 8);
        let mut occ = land(g);
        let mut cells = [0u8; CELLS];
        cells[0] = SENSOR as u8;
        cells[1] = SENSOR as u8;
        cells[SIDE] = MUSCLE as u8;
        let a = agent_with(cells, 8, 8, NORTH);
        occ.claim(g, &a, 0);
        let food = |c: usize| (c % 7) as f64;
        let water = |_: usize| 0.0;
        let full = sense(&a, g, &food, &water, &occ.crowd, true, 1.0);
        let half = sense(&a, g, &food, &water, &occ.crowd, true, 0.5);
        let dark = sense(&a, g, &food, &water, &occ.crowd, true, 0.0);
        assert!(a.body.reach(0) >= 2, "reach {}", a.body.reach(0));
        assert!(full.input[1] != half.input[1], "the light did not shorten the reach");
        assert_eq!(dark.input[1..9], [0.0; 8], "a body in the dark still sees around it");
        assert_eq!(dark.input[0], full.input[0], "the food under its sensor blocks is touched, not seen");
    }
    ...
    fn a_step_takes_a_motor_and_a_block_weighs_by_its_kind() {
        ...
        assert_eq!((b.size, b.mass, b.speed(0.0)), (2, 3.0, 0.0));
        cells[2] = MUSCLE as u8;
        assert!((Body::new(cells, SIDE, [0.0; N_POLICY], 0, 1.0).speed(0.0) - 0.25).abs() < 1e-6);
    ...
    /// e095 (#107): a leg pulls only where it touches the outside. A leg walled in by the body adds
    /// nothing and still weighs; at `leg` 0 the motor is e048's muscle over mass to the bit.
    #[test]
    fn a_leg_pulls_where_it_touches_the_outside() {
        // A plus: a leg in the middle of four blocks has no face open to the air.
        let mut cells = [0u8; CELLS];
        let s = SIDE;
        let mid = 2 * s + 2;
        cells[mid] = LEG as u8;
        for d in DIRS {
            cells[neighbor(mid, d, s).unwrap()] = DIGESTIVE as u8;
        }
        let walled = Body::new(cells, s, [0.0; N_POLICY], 0, 1.0);
        assert_eq!((walled.leg_open, walled.kinds[LEG]), (0, 1));
        assert_eq!(walled.speed(2.0), walled.speed(0.0), "a walled leg adds nothing");
        assert_eq!(walled.mass, 5.0, "and it still weighs");
        // The same body with the leg moved to the end of an arm: three faces open, three times the
        // rate over the mass.
        let mut out = [0u8; CELLS];
        out[mid] = DIGESTIVE as u8;
        out[mid + 1] = DIGESTIVE as u8;
        out[mid + 2] = LEG as u8;
        let legged = Body::new(out, s, [0.0; N_POLICY], 0, 1.0);
        assert_eq!((legged.leg_open, legged.mass), (3, 3.0));
        assert_eq!(legged.speed(0.0), 0.0, "no muscle, and the law off");
        assert!((legged.speed(2.0) - 2.0).abs() < 1e-6, "3 faces x 2 over a mass of 3");
    }

    /// e095 (#107): a spike is a hard block with nothing beside it across the line it presses on,
    /// and the force behind it counts 1 + `spike` times over. The same block inside a flat armoured
    /// face counts as it does today, and with `spike` 0 nothing changes at all.
    #[test]
    fn a_spike_is_a_tip_with_nothing_beside_it() {
        let s = SIDE;
        // A hard tip with one muscle behind it, and the same face made flat by two hard blocks beside
        // the tip. Only the first is a spike.
        let mut point = [0u8; CELLS];
        point[2] = HARD as u8;
        point[s + 2] = MUSCLE as u8;
        let mut flat = point;
        flat[1] = HARD as u8;
        flat[3] = HARD as u8;
        assert_eq!(spikes_of(&Body::new(point, s, [0.0; N_POLICY], 0, 1.0)), 1);
        assert!(is_spike(&point, 2, SOUTH, s), "a tip with nothing beside it");
        assert!(!is_spike(&flat, 2, SOUTH, s), "the same block inside a face is not");

        // A force of 1 against a soft face of 1 breaks nothing (e010 wants more than the face).
        // Through a spike at 1 it counts 2 and the block comes off - and what the gut takes through
        // it is counted apart.
        let g = Grid::new(8, 8);
        let mut soft = [0u8; CELLS];
        soft[0] = MUSCLE as u8;
        soft[1] = MUSCLE as u8;
        let press = |tip: [u8; CELLS], spike: f32| {
            let mut gutted = tip;
            gutted[3 * s + 2] = DIGESTIVE as u8;
            let mut agents = vec![agent_with(gutted, 0, 8, EAST), agent_with(soft, 8, 10, NORTH)];
            let mut occ = land(g);
            occ.claim(g, &agents[0], 0);
            occ.claim(g, &agents[1], 1);
            let mut c = Counters::default();
            let mut carrion = vec![0.0; g.cells()];
            push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, 1.0, 0.0, spike, &mut c);
            (c.cells_broken, c.sharp_breaks, c.sharp_gain, agents[0].sharp)
        };
        assert_eq!(press(point, 0.0), (0, 0, 0.0, 0.0), "a force of 1 breaks no soft face");
        let (broke, sharp, gain, got) = press(point, 1.0);
        assert_eq!((broke, sharp), (1, 1));
        assert!(gain > 0.0 && (got as f64 - gain).abs() < 1e-6, "what came in through the spike");
        assert_eq!(press(flat, 1.0), (0, 0, 0.0, 0.0), "the same tip inside a flat face is no spike");
    }
    ...
    fn a_body_senses_only_through_its_sensor_blocks() {
        ...
        for facing in [NORTH, EAST] {
            ...
            (a.water, a.breath) = (0.5, 0.75);
            let (off, on) = (sense(&a, g, &food, &water, &occ.crowd, false, 1.0), sense(&a, g, &food, &water, &occ.crowd, true, 1.0));
            if facing == NORTH {
