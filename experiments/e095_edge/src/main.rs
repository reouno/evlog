...
params! {
    ...
    light_gain = 0.0, "e093 (#103): matter a leaf block takes a step per face open to the air, times the light where it stands (the sun's height less the water's depth and the wood's shade), out of the soil of its cell (0: no body develops a leaf block and the world is e092's)";
    // e095's edge (#107, P3 step 2), both 0 being e092's control world
    spike = 0.0, "e095 (#107): what a spike adds to the force behind it, as a share: a hard block with nothing beside it across the line it presses on breaks with 1 + spike times its line's muscle (0: a tip counts as it does today)";
    leg = 0.0, "e095 (#107): what a leg block adds to the motor for each of its faces open to the air, in the units of a muscle block; walled in it adds nothing and still weighs (0: no body develops a leg block and the world is e092's)";
}
...
const BODY_KEYS: [&str; 54] = [
    ...
    // e093's (#103): the light a leaf block lives by
    "light_gain",
    // e095's (#107): the two materials that work only at a body's edge
    "spike", "leg",
];
...
impl Sim {
    ...
    fn with_pools(p: Params, w: World, pl: Plants, update_i: u64, threads: usize, pools: Pools) -> Sim {
        ...
        laws.leg_on = p.leg > 0.0; // e095 (#107): off, no cell develops a leg block
    ...
    fn bodies(&mut self) {
        ...
        for i in 0..agents.len() {
            ...
            // light on the cell it stands over, straight out of that cell's soil - the same store a
            // producer grows out of, so the ledger closes and the body competes with the grass under
            // it. The soil gives what it has; the faces that gain are the faces that lose water to
            // dry air (e067), pass heat (e072) and are what a tooth breaks (e075).
            ...
            if light_gain > 0.0 {
                for p in a.cells_held() {
                    ...
                    let (sx, sy) = a.sub_at(g, p, NORTH, 0);
                    let c = g.wcell(sx, sy);
                    let want = light_gain * a.open[p] as f64 * sun[c] as f64 * s;
            ...
            if action == 1 {
                ...
                k.tried_by[medium] += 1;
                let d = f;
                let pressed = push(agents, i, d, g, occ, &mut pl.carrion, s, p.flesh_bite, p.spike as f32, &mut k.cc);
                if agents[i].alive {
                    let mut work = 0.0f32;
                    ...
                    if agents[i].fits(g, occ, i as u32, d, 1) {
                        if stroke(agents[i].body.speed(p.leg as f32), rng) {
                            occ.release(g, &agents[i]);
                            let (nx, ny) = g.sstep(agents[i].x, agents[i].y, d, 1);
                            agents[i].x = nx;
                            agents[i].y = ny;
                            moved = 1;
                            if rng.f32() < agents[i].body.speed(p.leg as f32) && agents[i].fits(g, occ, i as u32, d, 1) {
                ...
                if room && stroke(agents[i].body.speed(p.leg as f32), rng) {
    ...
    const LOG_HEADER: &'static str = "step,pop,grown,births,children,sexual,no_room,empty,deaths_hunger,deaths_broken,deaths_wear,cells_broken,contacts,\
        ...
        leg_mean,leg_open,leg_led,spike_mean,spiked,sharp_share,sharp_broke";
    ...
    fn log_row(&mut self, steps: u64, matter0: f64, wall: f64) -> (String, Point) {
        ...
        s.push_str(&format!(
            ...
            mean(&|a| a.body.speed(self.p.leg as f32) as f64),
        ...
        // e095 (#107): the two materials of the edge. The leg blocks a body holds and the faces of
        // theirs that pull (walled in, a leg is dead weight); the share of grown bodies whose legs
        // give more motor than their muscle. The spikes a body's grid carries (a hard block with
        // nothing beside it across a line, counted once however many sides it points), the share of
        // grown bodies with one, and what came in through a spike - of the flesh taken and of the
        // blocks broken - over the interval.
        let leg = self.p.leg as f32;
        let leg_led = grown.iter().filter(|a| leg * a.body.leg_open as f32 > a.body.kinds[MUSCLE] as f32).count() as f64 / grown.len().max(1) as f64;
        let spike_mean = mean(&|a| spikes_of(&a.body) as f64);
        let spiked = grown.iter().filter(|a| spikes_of(&a.body) > 0).count() as f64 / grown.len().max(1) as f64;
        s.push_str(&format!(
            ",{:.2},{:.2},{:.3},{:.2},{:.3},{:.3},{:.3}",
            mean(&|a| a.body.kinds[LEG] as f64),
            mean(&|a| a.body.leg_open as f64),
            leg_led,
            spike_mean,
            spiked,
            k.cc.sharp_gain / k.cc.kill_gain.max(1e-12),
            k.cc.sharp_breaks as f64 / k.cc.cells_broken.max(1) as f64
        ));
        let point = Point { step: self.step, pop, plant: k.plant, kills: k.cc.kill_gain, scavenged: k.scavenged, ms, err, pop_by };
        (s, point)
    }

    const AGENT_HEADER: &'static str = "step,id,lineage,age,turns,mass,size,side,density,born_size,born_mass,born_hard,born_muscle,born_sensor,born_digestive,born_leaf,born_leg,born_bite,\
        hard,muscle,sensor,digestive,leaf,leg,leaf_open,leg_open,light,sharp,bite,bite_any,shell,speed,energy,fat,plant,meat,killed,scavenged,travel,path,worn,pace,place,born_place,temp,moist,height,medium,algae,detritus,water,drank,open_soft,breath,inhaled,breed,share,store,kids,btemp,warmed,cooled,wood,load,invader,cell,crown,seed,fermented,dunged,fiber,fruit,cells";
    ...
    fn agent_rows(&self, out: &mut dyn Write) {
        ...
        for a in &self.agents {
            ...
            writeln!(
                out,
                "{},{},{},{},{},{:.2},{},{},{:.3},{},{:.2},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{:.3},{:.3},{},{},{:.2},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.2},{:.2},{},{:.3},{},{},{:.1},{:.3},{:.0},{},{:.3},{:.3},{:.3},{},{},{:.3},{},{:.4},{:.4},{:.3},{},{:.2},{:.4},{:.4},{:.3},{:.2},{},{},{:.3},{:.3},{:.3},{:.3},{:.4},{:.3},{}",
                self.step,
                a.id,
                a.lineage,
                a.age,
...
fn main() {
    ...
    let mut view = viewer::View::from_env(
        ...
        viewer::Init {
            ...
            blocks: vec!["empty", "hard", "muscle", "sensor", "digestive", "leaf", "leg"],
...
mod tests {
    ...
    fn conserved_at(scale: f64, water: f64, dry: f64, breath: f64, history: f64, senses: f64, sets: bool) {
        ...
        if sets {
            ...
            p.light_gain = 0.02;
            // e095 (#107): the spike and the leg, each well over the rates the ladders will look at.
            p.spike = 4.0;
            p.leg = 2.0;
        }
        ...
        if sets {
            ...
            // e095 (#107): both materials of the edge were built and both laws engaged.
            assert!(sim.agents.iter().any(|a| a.body.kinds[LEG] > 0), "no body developed a leg block");
            assert!(sim.agents.iter().any(|a| a.body.leg_open > 0), "no leg block touches the outside");
            // The spike has something to act on: bodies pack (#52), so a lone hard tip is rare - in
            // this small world 1-2% of the bodies carry one and 3,000 steps pass without one
            // breaking anything. That the force counts through it is `a_spike_is_a_tip_with_nothing
            // _beside_it` in `body.rs`; what the runs must show is whether the shape is ever kept.
            assert!(sim.agents.iter().any(|a| spikes_of(&a.body) > 0), "no body carries a spike");
