//! e016: a plant under a body does not grow. e015's world (space at the resolution of the
//! body, facing, two kinds of place, contact physics, a cell costs what it holds, per-body
//! cost, work = force x distance) with one law added to the regrowth of food:
//!
//! - A cell held by a body does not regrow. Food is per world cell; a world cell is SUB x SUB
//!   sub-cells and a body holds sub-cells. With `shade` = "any" (the default) a cell regrows
//!   only while no body holds any of its sub-cells; with "free" it regrows by its free
//!   sub-cells (regrowth times the free share). e015's world regrew under the body that ate
//!   it, so the best body stood still and reached; here standing still exhausts the spot and
//!   the cells a body leaves recover.
//!
//! Everything else is e015: the world, the materials, the contact rule, facing, mating,
//! lineages and the measures. One counter is added to the log: `shaded` (regrowth lost to
//! bodies standing on cells, per step, next to `wasted`, the regrowth lost to the cap).
...
fn push(agents: &mut [Agent], i: usize, d: usize, g: Grid, occ: &mut Occ, place: &[u8], c: &mut Counters) -> Vec<(usize, u8)> {
    let opp = opposite(d);
...
fn main() {
    ...
    // How a held cell regrows: "any" (a cell held by any body does not regrow) or "free" (by
    // its free sub-cells).
    let shade = args.get(5).map(String::as_str).unwrap_or("any");
    let shade_any = match shade { "any" => true, "free" => false, _ => panic!("shade: any or free") };
    ...
    let shade_tag = if shade_any { "" } else { "-free" };
    let prefix = format!("experiments/e016_plant_under_body/results/{size}_sigma{sigma_name}{shade_tag}_seed{seed}");
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent,shaded"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        // A plant under a body does not grow: a cell regrows only by the sub-cells no body
        // holds ("free"), or not at all while a body holds any ("any").
        for (c, (r, &grow)) in res.iter_mut().zip(&patches.grow).enumerate() {
            let held = occ.crowd[c] as usize;
            let gr = if held == 0 { grow } else if shade_any { 0.0 } else { grow * (SUB_CELLS - held) as f32 / SUB_CELLS as f32 };
        ...
        for i in 0..agents.len() {
            ...
            // 1. Eat plants: each digestive cell eats from the world cell under it.
            let a = &mut agents[i];
            a.age += 1;
            ...
            for (k, c) in guts.iter().enumerate() {
                let e = res[c].min(BITE * gut_n[k] as f32);
                res[c] -= e;
                eaten += e;
            ...
            let here: f32 = a.under(g, NORTH, 0).iter().map(|c| res[c]).sum();
            let mut input = [0.0f32; N_IN];
            let mut blind = [0.0f32; N_IN];
            input[0] = here;
            blind[0] = here;
            for (k, &d) in dirs.iter().enumerate() {
                let (food1, others1) = look(a, d, 1, &res, &occ);
                let (food2, others2) = look(a, d, 2, &res, &occ);
            ...
            if action == 1 {
                moves_tried += 1;
                let d = f;
                let pressed = push(&mut agents, i, d, g, &mut occ, &patches.place, &mut cc);
                if agents[i].alive {
                    if !pressed.is_empty() {
                        pushes += 1;
                    }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.3},{:.5},{:.2}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64, shaded / LOG_INTERVAL as f64).unwrap();
            for p in 0..=N_PLACES as u8 {
                ...
                writeln!(places_csv, "{step},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{:.3},{:.1},{:.1},{},{movers},{:.2},{:.2},{:.2}", place_name(p), here.len(),
                    ...
                    mean(&|a| a.foot_n(g) as f32), mean(&|a| a.body.shell_side(NORTH)), mean(&|a| a.body.shell_side(SOUTH))).unwrap();
            }
...
mod tests {
    ...
    fn a_push_meets_face_to_face() {
        ...
        let pressed = push(&mut agents, 0, EAST, g, &mut occ, &place, &mut c);
        // One body pressed with force 2 (two muscle cells in the line); the soft cell broke and
        // was eaten by nobody (the hunter has no digestive cell); the prey lost a cell.
