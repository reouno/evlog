#!/usr/bin/env python3
"""Read e104's runs against e103's five hypotheses (#116, rung 2); e103's reader, copied.

Run from the repo root: `uv run python experiments/e104_generations/measure.py [results_dir run ...]` (a minute).
...
def read(run):
    ...
    out["H5"] = abs(out["et_change"]) >= EVAP and out["ab_corr_e102"] < AB_CORR
...
def main():
    ...
    prov = [{"reading": "measure", "run": o["run"], "census_years": o["years"], "thresholds":
             ...
             f"{CHANGES}; new share {NEW_SHARE}; evaporation {EVAP}; A:B corr {AB_CORR}; places as e102 (river {RIVER} mm, "
