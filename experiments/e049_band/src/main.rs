...
impl Laws {
    fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        let morph_level = (0..=SIDE_MAX)
            ...
            .collect();
        Laws { morphogen, table, density, digest, side, breed, water, morph_level }
    }
}
...
impl Body {
    fn new(cells: [u8; CELLS], side: usize, policy: [f32; N_POLICY], n_genes: u16, density: f32, by_kind: bool) -> Self {
        let mut b = Body { cells, side: side as u8, size: 0, mass: 0.0, density, digest: 0.5, by_kind, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE_MAX]; 4], extent: [0; 2], policy, breed: [0.0; N_IN + 1], water_policy: [0.0; N_WATER_POLICY], n_genes, cut: 0 };
        b.refresh();
        b
    }
    fn s(&self) -> usize {
...
fn main() {
    ...
    let prefix = format!("experiments/e049_band/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}{sat_tag}{hold_tag}{wear_tag}{connect_tag}{yield_tag}{motor_tag}{band_tag}_seed{seed}", rain.name());
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height,fat,size,density,digest,side,water,worn,travel,drift").unwrap();
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            let (mut action, best_v) = act_value(&a.body.policy, &a.body.water_policy, &input, wsee);
            ...
            if a.body.kinds[SENSOR] > 0 {
                sense_decisions += 1;
                if act(&a.body.policy, &a.body.water_policy, &blind, wsee_blind) != action {
                    sense_used += 1;
                }
            }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.4},{:.5}", band_bodies[..band_w].iter().sum::<u64>() as f64 / band_steps.max(1) as f64, west as f64 / LOG_INTERVAL as f64 / pop as f64).unwrap();
            if band > 0 {
                let front = band_front(step, band, w);
