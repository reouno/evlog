...
/// empty). Returns the labels and the number of parts.
...
fn keep_largest(cells: &mut [u8; CELLS], s: usize, by_kind: bool) -> u16 {
...
fn develop_genes(genes: &[Gene], laws: &Laws, weight: Weight, side_law: Side, connect: bool) -> Body {
    let n = genes.len();
    ...
    let cut = if connect { keep_largest(&mut cells, side, weight.kind) } else { 0 };
...
fn push(agents: &mut [Agent], i: usize, d: usize, g: Grid, occ: &mut Occ, place: &[u8], food: &mut Food, cell_energy: f32, digest_law: u8, connect: bool, c: &mut Counters) -> Vec<(usize, u8)> {
    let opp = opposite(d);
    ...
    for v in victims {
        let lost = cut_loose(&mut agents[v], g, occ, food, place, cell_energy, c);
...
fn wear_blocks(a: &mut Agent, h: f64, rng: &mut Rng, g: Grid, occ: &mut Occ, food: &mut Food, place: &[u8], cell_energy: f32, connect: bool, c: &mut Counters) -> u16 {
    ...
    if connect && !failed.is_empty() {
        let lost = cut_loose(a, g, occ, food, place, cell_energy, c);
...
fn cut_loose(a: &mut Agent, g: Grid, occ: &mut Occ, food: &mut Food, place: &[u8], cell_energy: f32, c: &mut Counters) -> u16 {
    let s = a.body.s();
    let (label, parts) = parts_of(&a.body.cells, s);
    if parts < 2 {
        return 0;
    }
...
fn main() {
    ...
    assert!(connect <= 1, "connect is 0 or 1");
    let connect_tag = if connect == 1 { "_connect" } else { "" };
    ...
    for _ in 0..init_pop {
        ...
        let body = develop_genes(&genes, &laws, weight, side_law, connect == 1);
        next_id += 1;
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            if wear > 0.0 {
                ...
                worn += wear_blocks(a, h, &mut wear_rng, g, &mut occ, &mut food, &patches.place, cell_energy, connect == 1, &mut cc) as u64;
                if !a.alive {
                    continue;
                }
            }
            ...
            if action == 1 {
                moves_tried += 1;
                let d = f;
                let pressed = push(&mut agents, i, d, g, &mut occ, &patches.place, &mut food, cell_energy, digest_law, connect == 1, &mut cc);
                if agents[i].alive {
                    if !pressed.is_empty() {
                        pushes += 1;
                    }
        ...
        if n_threads < 2 {
            developed.extend(jobs.iter().map(|(_, g)| develop_genes(g, &laws, weight, side_law, connect == 1)));
        } else {
            let chunk = jobs.len().div_ceil(n_threads);
            let laws = &laws;
            let parts: Vec<Vec<Body>> = std::thread::scope(|sc| {
                let handles: Vec<_> = jobs.chunks(chunk).map(|c| sc.spawn(move || c.iter().map(|(_, g)| develop_genes(g, laws, weight, side_law, connect == 1)).collect::<Vec<Body>>())).collect();
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            let (mut split, mut outside, mut blocks) = (0usize, 0usize, 0usize);
            for a in &agents {
                let s = a.body.s();
                let (label, parts) = parts_of(&a.body.cells, s);
            ...
            writeln!(log, ",{:.4},{:.4},{:.4},{:.3},{:.4},{:.4},{:.4}", split as f64 / pop as f64, outside as f64 / blocks.max(1) as f64, cc.born_cut as f64 / births.max(1) as f64,
                cc.not_built as f64 / births.max(1) as f64, per_step(cc.cut_break), per_step(cc.cut_wear), per_step(cc.cut_bodies)).unwrap();
...
mod tests {
    ...
    fn a_gut_digests_by_its_axis_and_the_rest_is_dung() {
        ...
        for (law, d, yield_) in [(1, 1.0, 1.0), (1, 0.0, DIGEST_FLOOR), (1, 0.5, (1.0 + DIGEST_FLOOR) / 2.0), (0, 0.0, 1.0), (2, 0.0, DIGEST_FLOOR), (2, 0.5, 1.0 - (1.0 - DIGEST_FLOOR) * 0.5f32.sqrt())] {
            ...
            push(&mut agents, 0, EAST, g, &mut occ, &place, &mut food, CELL_ENERGY, law, false, &mut c);
    ...
    fn the_flesh_is_worth_what_the_body_lived_through() {
        ...
        occ.claim(g, &agents[0], 0);
        occ.claim(g, &agents[1], 1);
        push(&mut agents, 0, EAST, g, &mut occ, &place, &mut food, CELL_ENERGY, 0, false, &mut c);
    ...
    fn a_push_meets_face_to_face() {
        ...
        let mut c = Counters::default();
        let mut food = Food::new(vec![0.0; g.cells()], vec![0.0; g.cells()], vec![0.0f64; g.cells()]);
        let pressed = push(&mut agents, 0, EAST, g, &mut occ, &place, &mut food, CELL_ENERGY, 0, false, &mut c);
    ...
    fn wear_takes_half_the_blocks_by_the_median_life() {
        ...
        let mut rng = Rng(1);
        assert_eq!(wear_blocks(&mut a, 0.0, &mut rng, g, &mut occ, &mut food, &place, CELL_ENERGY, false, &mut c), 0);
        assert_eq!((a.body.size, a.alive), (3, true));
        assert_eq!(wear_blocks(&mut a, 1.0, &mut rng, g, &mut occ, &mut food, &place, CELL_ENERGY, false, &mut c), 3);
    ...
    fn a_body_is_what_holds_together() {
        ...
        assert_eq!(parts_of(&cells, SIDE).1, 3);
        let mut born = cells;
        assert_eq!(keep_largest(&mut born, SIDE, true), 3);
        ...
        keep_largest(&mut by_kind, SIDE, true);
        assert_eq!((by_kind[0], by_kind[5]), (0, HARD as u8));
        let mut plain = pair;
        keep_largest(&mut plain, SIDE, false);
        ...
        for (connect, kept) in [(false, 3u16), (true, 2)] {
            ...
            assert_eq!((c.cut_break, c.cut_bodies), if connect { (1, 1) } else { (0, 0) });
            ...
            assert_eq!(occ.sub[g.sidx(9, 10)] == u32::MAX, connect);
