#!/bin/bash
# A run of e082 (#95): `run.sh <world> <steps> <life> <name> [key=value ...]` puts stage C's default
...
# `fresh=0.2` overrides the default world's 0.05). The crate is e081's unchanged.
#
# From the repo root:
#   (nohup bash experiments/e082_fresh/run.sh c1225 60000 9 u9.4_f0.2 unit=9.4 fresh=0.2 \
#     > experiments/e082_fresh/results/c1225_life9_u9.4_f0.2.log 2>&1 < /dev/null &)
set -e
...
out=${EVLOG_OUT:-experiments/e082_fresh/results}
mkdir -p $out
EVLOG_THREADS=1 ./target/release/e082_fresh $out/${world}_life${life}_$name $params $d11 $sets $kept life=$life steps=$steps "$@"
