...
fn main() {
    ...
    // e047 (#50): 2 lets blocks that touch at a corner join too (8 neighbors).
    let connect: u8 = args.get(42).and_then(|s| s.parse().ok()).unwrap_or(1);
    ...
    let prefix = format!("experiments/e047_corner/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}{sat_tag}{hold_tag}{wear_tag}{connect_tag}{yield_tag}_seed{seed}", rain.name());
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height,fat,size,density,digest,side,water,worn").unwrap();
    ...
    for _ in 0..init_pop {
        ...
        let mut a = Agent {
            id: next_id - 1, lineage: 0, x: 0, y: 0, energy: INIT_ENERGY as f64, age: 0, plant: 0.0, meat: 0.0, fat: 0.0, water: 1.0, born_size: body.size, breed_now: false, born_place: NO_PLACE, short: false, worn: 0, worn_out: false,
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            if agents[i].energy >= thr as f64 && (breed_law == 0 || agents[i].breed_now) {
                ...
                pending.push((Agent {
                    id: next_id - 1, lineage: a.lineage, x: a.x, y: a.y, energy: a.energy, age: 0, plant: 0.0, meat: 0.0, fat: yolk_fat, water: a.water, born_size: 0, breed_now: false, born_place: NO_PLACE, short: false, worn: 0, worn_out: false, alive: true,
        ...
        if step % AGENT_DUMP_INTERVAL == 0 {
            ...
            for a in &agents {
                ...
                write!(agents_csv, ",{:.3},{},{:.3},{:.3},{},{:.3},{}", a.fat, b.size, b.density, b.digest, b.side, a.water, a.worn).unwrap();
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.4},{:.4},{:.4},{:.3},{:.4},{:.4},{:.4},{:.4}", split as f64 / pop as f64, outside as f64 / blocks.max(1) as f64, cc.born_cut as f64 / births.max(1) as f64,
...
mod tests {
    ...
    fn agent_with(cells: [u8; CELLS], x: usize, y: usize, facing: usize) -> Agent {
        let mut a = Agent {
            id: 0, lineage: 0, x, y, energy: 1.0, age: 0, plant: 0.0, meat: 0.0, fat: 0.0, water: 1.0, born_size: 0, breed_now: false, born_place: NO_PLACE, short: false, worn: 0, worn_out: false, alive: true,
