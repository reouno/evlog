//! e020: the breath rises and the rain falls on the mountains. e019's closed world (matter is
//! in the soil of a cell, in the plant on it, lying dead on it, or in a body; a plant grows out
//! of its own cell's soil at most the sun's rate; the ground has a height and soil runs
//! downhill, so it pools level in the low ground) with one law changed about where what a body
//! burns goes:
//!
//! - What a body burns goes to the air, and the air rains on the ground. e018 and e019 let the
//!   upkeep of a body and the work of its moving fall to the soil under it; here it rises to
//!   the air (one pool for the whole world). Each step the air rains on every cell: at most
//!   the sun's worth per step (RES_GROWTH) times the cell's height over the relief, so that the
//!   rain falls on the mountains and hardly at all on the lake floor (`high`); or the same on
//!   every cell (`flat`, a control); what the air holds beyond what can fall stays in the air.
//!   Rain lands in the soil of the cell, where the sun draws it into the plant or the flow
//!   carries it downhill. The dead still lie where they fell and rot into the soil there.
//!   `soil` is e019: nothing goes to the air.
//!
//! The high ground then has an income that does not come from the soil, which ran off it, and
//! the world's idle store moves from the lake's soil to the air. Everything else is e019: the
//! uniform sun, the terrain, the flow, the height bands as places.
//!
//! Arguments: steps, seed, size, patch widths (0: the uniform sun), cell energy, matter per
//! cell at the start, `relief`, `flow`, `rain` (high, flat or soil) and `breath` (the share of
//! what a body burns that goes to the air, 1 by default; the rest falls to the soil under it). The log gets `air`
//! (matter in the air) and `rain` (rain fallen per step); the per-place log gets `rain`.
...
fn main() {
    ...
    let prefix = format!("experiments/e020_rain/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"grain\":{RELIEF_GRAIN},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), list(&terrain.height)).unwrap();
    }
    ...
    writeln!(places_csv, "step,place,pop,mass,hard,muscle,sensor,digestive,bite,shell,biters,cover,plant_intake,meat_intake,lineages,movers,foot,shell_front,shell_back,dead,carrion,soil,barren,regrowth,cells,rain").unwrap();
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent,shaded,dead,carrion,soil,matter,barren,rot,spent,flow,soil_cells,deep,air,rain"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.3},{:.5},{:.2},{:.4},{:.2},{soil_stock:.1},{matter:.1},{:.3},{:.4},{:.3},{:.3},{soil_cells:.3},{deep:.3},{air:.1},{:.3}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64, shaded / LOG_INTERVAL as f64,
                cc.dead / LOG_INTERVAL as f64, carrion_stock, barren / LOG_INTERVAL as f64, rot / LOG_INTERVAL as f64, spent / LOG_INTERVAL as f64, flowed / LOG_INTERVAL as f64, rained / LOG_INTERVAL as f64).unwrap();
            for p in 0..=N_PLACES as u8 {
                ...
                writeln!(places_csv, "{step},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{:.3},{:.1},{:.1},{},{movers},{:.2},{:.2},{:.2},{:.4},{:.2},{:.1},{:.4},{:.4},{},{:.4}", place_name(p), here.len(),
