//! e012: two kinds of place. e011's world and physics (contact, a cell costs what it holds,
//! per-body cost, food patches whose width is a law of the world) with one change in the food
//! law: the patches of one world need not all have the same width. Patch k has the width
//! sigmas[k % sigmas.len()], so "8,1" makes every other patch wide (e010's grass, where the
//! smallest grazer wins) and the rest narrow (e011's trees, where the crowd makes tortoises and
//! hunters). Every patch carries the same total regrowth whatever its width. Nothing about
//! bodies changes; whether both kinds of body exist in one world, and whether a lineage moves
//! between places, is left to selection. What a cell can hold is a constant now (e011: no effect).
//!
//! Measures added: each cell of the world belongs to the patch that gives it the most regrowth
//! (its place; cells beyond every patch belong to none). The log reports per place, and each
//! agent remembers the place it was born in, so a body found in the other kind of place is a
//! crossing that can be counted.
...
// Sides of a body, and the lines that meet a neighbor on that side.
const NORTH: usize = 0;
const SOUTH: usize = 1;
const EAST: usize = 2;
const WEST: usize = 3;
...
struct Body {
    ...
    mass: u8,
    kinds: [u8; N_KINDS],
    tips: [[Tip; SIDE]; 4], // per side, per line
    policy: [f32; N_POLICY],
    n_genes: u16,
}
...
impl Body {
    fn new(cells: [u8; CELLS], policy: [f32; N_POLICY], n_genes: u16) -> Self {
        let mut b = Body { cells, mass: 0, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE]; 4], policy, n_genes };
        b.refresh();
        b
    }
    /// Recompute what follows from the cells: mass, counts per kind, and the tips of every line
    /// on every side. Called at birth and whenever a cell breaks.
    fn refresh(&mut self) {
        ...
        for side in 0..4 {
            for line in 0..SIDE {
                ...
                let force = (0..SIDE).filter(|&k| self.cells[at(k)] == MUSCLE as u8).count() as u8;
                if let Some(k0) = (0..SIDE).find(|&k| self.cells[at(k)] != 0) {
                    let hardness = if self.cells[at(k0)] == HARD as u8 {
                        HARDNESS * (k0..SIDE).take_while(|&k| self.cells[at(k)] == HARD as u8).count() as u8
                    } else {
                        1
                    };
                    tip = Tip { pos: at(k0) as u8, hardness, force };
                }
                self.tips[side][line] = tip;
            }
        }
    }
    fn speed(&self) -> f32 {
    ...
    /// a hard tip. Shell: the mean hardness of the tips that can be touched. Open lines: lines
    /// with nothing to touch.
    fn bite(&self) -> u8 {
        self.tips.iter().flatten().filter(|t| t.hardness > 1).map(|t| t.force).max().unwrap_or(0)
    }
    fn shell(&self) -> f32 {
        let touchable: Vec<u8> = self.tips.iter().flatten().filter(|t| t.hardness > 0).map(|t| t.hardness).collect();
...
fn main() {
    ...
    let prefix = format!("experiments/e012_two_places/results/{size}_sigma{sigma_name}_seed{seed}");
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place").unwrap();
    ...
    writeln!(lineages_csv, "step,lineage,size,mass,hard,muscle,sensor,digestive,bite,shell,open,bodies,age,plant,meat,p0,p1,pnone").unwrap();
    ...
    // the intake eaten there, the lineages present, and the movers (born in the other kind of place).
    let mut places_csv = open("places.csv");
    writeln!(places_csv, "step,place,pop,mass,hard,muscle,sensor,digestive,bite,shell,biters,crowd_max,plant_intake,meat_intake,lineages,movers").unwrap();
    ...
    let mut agents: Vec<Agent> = (0..init_pop)
        .map(|_| {
            ...
            Agent {
                id: next_id - 1, lineage: 0, x, y, energy: INIT_ENERGY, age: 0, plant: 0.0, meat: 0.0, born_mass: body.mass, born_place: patches.place[g.idx(x, y)],
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         mass_p10,mass_p50,mass_p90,mass_max,occupied_cells,crowd_max,wasted,intake_per_gut,crossers,pop_none"
    )
    .unwrap();
    ...
    let mut prey_age = 0u64; // summed over the deaths by damage of the log interval
    let mut kills_young = 0u64; // deaths by damage before YOUNG steps
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            a.plant += eaten;
            plant_intake += eaten;
            plant_at[patches.place[here] as usize] += eaten;
            ...
            // Moving toward a cell that holds other bodies pushes into them along the facing
            // side, line by line, with the force of the muscle in each line. In a line where
            // both have a cell, the softer tip breaks if the push exceeds its hardness (equal
            // hardness: nothing). A broken cell is gone; its share of the body's energy and the
            // energy of its matter go to the pusher if the pusher has digestive cells.
            if action != 0 {
                let (c1, side_i, side_j) = match action {
                    1 => (idx(x, yn), NORTH, SOUTH),
                    2 => (idx(x, yp), SOUTH, NORTH),
                    3 => (idx(xp, y), EAST, WEST),
                    _ => (idx(xn, y), WEST, EAST),
                };
                let mut j = head[c1];
                while j != u32::MAX {
                    let j_us = j as usize;
                    if j_us != i && agents[j_us].alive && agents[i].alive {
                        contacts += 1;
                        let mut breaks: Vec<(usize, u8)> = Vec::new(); // (victim, cell)
                        {
                            let (a, b) = (&agents[i].body, &agents[j_us].body);
                            for line in 0..SIDE {
                                let (ta, tb) = (a.tips[side_i][line], b.tips[side_j][line]);
                                if ta.hardness == 0 || tb.hardness == 0 {
                                    continue;
                                }
                                if tb.hardness < ta.hardness && ta.force > tb.hardness {
                                    breaks.push((j_us, tb.pos));
                        ...
                        for (victim, pos) in breaks {
                            let eater = if victim == i { j_us } else { i };
                            let v = &mut agents[victim];
                            if v.body.cells[pos as usize] == 0 {
                                continue;
                            }
                            let share = v.energy.max(0.0) / v.body.mass as f32;
                            v.energy -= share;
                            v.body.cells[pos as usize] = 0;
                            v.body.refresh();
                            cells_broken += 1;
                            if v.body.mass == 0 && v.alive {
                                v.alive = false;
                                deaths[2] += 1;
                                prey_age += v.age as u64;
                                if v.age < YOUNG {
                                    kills_young += 1;
                                }
                            }
                            ...
                            if e.body.kinds[DIGESTIVE] > 0 {
                                ...
                                e.meat += gain;
                                meat_intake += gain;
                                meat_at[patches.place[idx(e.x, e.y)] as usize] += gain;
                            }
                        }
                    }
                    j = next[j_us];
            ...
            if agents[i].energy >= thr {
                ...
                pending.push((Agent {
                    id: next_id - 1, lineage: a.lineage, x: cx, y: cy, energy: a.energy, age: 0, plant: 0.0, meat: 0.0, born_mass: 0, born_place: patches.place[idx(cx, cy)], alive: true,
                    keys, gene_ids, genome, body: body.unwrap_or_else(|| Body::new([0; CELLS], [0.0; N_POLICY], 0)),
                }, todo, i));
            }
        }
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            for (id, m) in &assigned {
                ...
                for &i in m {
                    ...
                    mass += agents[i].body.mass as f32;
                    bite += agents[i].body.bite() as f32;
                    shell += agents[i].body.shell();
                    open += agents[i].body.open_lines() as f32;
                    age += agents[i].age as f32;
                    plant += agents[i].plant;
                    meat += agents[i].meat;
                    *bodies.entry(&agents[i].body.cells).or_insert(0) += 1;
                }
                writeln!(lineages_csv, "{step},{id},{},{:.1},{:.1},{:.1},{:.1},{:.1},{:.1},{:.2},{:.1},{},{:.0},{:.2},{:.2},{},{},{}", m.len(), mass / sz,
        ...
        if step % AGENT_DUMP_INTERVAL == 0 {
            for a in &agents {
                let k = &a.body.kinds;
                writeln!(agents_csv, "{step},{},{},{},{},{},{},{},{:.2},{},{:.3},{},{:.2},{:.2},{:.2},{},{},{}",
                    a.body.mass, a.born_mass, k[HARD], k[MUSCLE], k[SENSOR], k[DIGESTIVE], a.body.bite(), a.body.shell(), a.body.open_lines(), a.body.speed(), a.age, a.energy, a.plant, a.meat, a.lineage,
                    place_name(patches.place[idx(a.x, a.y)]), place_name(a.born_place)).unwrap();
            }
        }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            write!(
                log,
                "{step},{},{mean_energy:.3},{mean_res:.3},{mean_genes:.2},{births},{},{},{},{},{cells_broken},{plant_intake:.1},{meat_intake:.1}",
                agents.len(),
                deaths[0],
                deaths[1],
                deaths[2],
            ...
            let kills = deaths[2].max(1) as f64;
            ...
            write!(log, ",{:.0},{:.3},{:.3},{meat_majority:.3},{contacts},{shell_mean:.2},{open_mean:.2},{full:.3},{damaged:.3},{biters:.3}",
                prey_age as f64 / kills, meat_intake as f64 / cells_broken.max(1) as f64, kills_young as f64 / kills).unwrap();
            // Size distribution (quantiles of mass), crowding (cells with a body, most bodies in
            // one cell), regrowth lost to the cap, and plant intake per digestive cell per step.
            ...
            let mut per_cell: HashMap<usize, u32> = HashMap::new();
            for a in &agents {
                *per_cell.entry(idx(a.x, a.y)).or_default() += 1;
            }
            let crowd_max = per_cell.values().max().copied().unwrap_or(0);
            ...
            writeln!(log, ",{},{},{},{},{},{crowd_max},{:.2},{:.4},{crossers:.3},{pop_none}", q(0.1), q(0.5), q(0.9), masses.last().copied().unwrap_or(0), per_cell.len(),
                wasted / LOG_INTERVAL as f64, plant_intake as f64 / LOG_INTERVAL as f64 / guts).unwrap();
            for p in 0..=N_PLACES as u8 {
                ...
                let mut crowd: HashMap<usize, u32> = HashMap::new();
                for a in &here {
                    *crowd.entry(idx(a.x, a.y)).or_default() += 1;
                }
                ...
                writeln!(places_csv, "{step},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{},{:.1},{:.1},{},{movers}", place_name(p), here.len(),
                    ...
                    crowd.values().max().copied().unwrap_or(0), plant_at[p as usize], meat_at[p as usize], present.len()).unwrap();
            }
            plant_at = [0.0; N_PLACES + 1];
            meat_at = [0.0; N_PLACES + 1];
            births = 0;
            sexual_births = 0;
