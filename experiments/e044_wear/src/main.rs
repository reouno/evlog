...
impl Body {
    fn new(cells: [u8; CELLS], side: usize, policy: [f32; N_POLICY], n_genes: u16, density: f32, by_kind: bool) -> Self {
        let mut b = Body { cells, side: side as u8, size: 0, mass: 0.0, density, digest: 0.5, by_kind, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE_MAX]; 4], extent: [0; 2], policy, breed: [0.0; N_IN + 1], water_policy: [0.0; N_WATER_POLICY], n_genes };
        b.refresh();
        b
    }
    fn s(&self) -> usize {
...
fn wear_blocks(a: &mut Agent, h: f64, rng: &mut Rng, g: Grid, occ: &mut Occ, food: &mut Food, place: &[u8], cell_energy: f32, c: &mut Counters) -> u16 {
...
fn main() {
    ...
    // MAX_AGE, e043 byte for byte). With wear on, MAX_AGE does not apply.
    let wear: f64 = args.get(41).and_then(|s| s.parse().ok()).unwrap_or(0.0);
    ...
    let prefix = format!("experiments/e044_wear/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}{sat_tag}{hold_tag}{wear_tag}_seed{seed}", rain.name());
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            if wear > 0.0 {
                ...
                worn += wear_blocks(a, h, &mut wear_rng, g, &mut occ, &mut food, &patches.place, cell_energy, &mut cc) as u64;
                if !a.alive {
                    continue;
                }
            }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{},{:.4},{:.4},{:.4},{},{},{}", deaths[5], worn as f64 / LOG_INTERVAL as f64, agents.iter().filter(|a| a.worn > 0).count() as f64 / pop as f64,
...
mod tests {
    ...
    fn wear_takes_half_the_blocks_by_the_median_life() {
        ...
        let mut rng = Rng(1);
        assert_eq!(wear_blocks(&mut a, 0.0, &mut rng, g, &mut occ, &mut food, &place, CELL_ENERGY, &mut c), 0);
        assert_eq!((a.body.size, a.alive), (3, true));
        assert_eq!(wear_blocks(&mut a, 1.0, &mut rng, g, &mut occ, &mut food, &place, CELL_ENERGY, &mut c), 3);
