//! e021: the tall plant takes the light. e020's closed world (matter is in the soil of a cell,
//! in the plant on it, lying dead on it, in a body, or in the air; a plant grows out of its own
//! cell's soil at most the sun's rate; what a body burns rises to the air and rains on the
//! mountains; the ground has a height and soil runs downhill) with one law added about the
//! light:
//!
//! - A taller plant shades a shorter one, as far as it is tall, and takes only what it can
//!   use. The matter standing on a cell (its plant, and the dead lying on it) is a column;
//!   each step, before anything grows, a column claims from every cell around it a share of
//!   that cell's sun equal to the difference of the columns less the distance walked, over the
//!   cap, times its own room over the cap (`shade` scales it; the reach of a shadow is at most
//!   the cap in cells; a full crown intercepts nothing; a column under a body claims nothing;
//!   claims past a cell's whole sun share it in proportion), and the light moves to the column
//!   that took it. A plant that stands ungrazed rises above the grazed lawn around it, drawing
//!   light from as far as it stands tall, fastest when it has been bitten deep - a well that
//!   refills hardest when most drawn - and goes quiet at the cap, a standing larder. The sun
//!   is only moved, never made: the world's light is the same as e020's.
//!
//! A cell in a closed world could never hold more than one bite's worth of growth (no cell
//! grows faster than the sun, e020); now an ungrazed cell can gather the sun of its neighbors
//! and stand as a tree, a store of hundreds of bites in one place. Everything else is e020: the
//! uniform sun, the terrain, the flow, the breath to the air, the rain by height, the height
//! bands as places. The terrain's mean height is normalized to half the relief, so that every
//! seed's rain caps add up to the same income (in e020 they spread 164 x 0.44 to 0.56 with the
//! seed's geography).
//!
//! Arguments: steps, seed, size, patch widths (0: the uniform sun), cell energy, matter per
//! cell at the start, `relief`, `flow`, `rain` (high, flat or soil), `breath` (the share of
//! what a body burns that goes to the air) and `shade` (the rate of the canopy law, 1 by
//! default; 0 is e020). The log gets `shade` (sun moved between cells per step), `trees` (cells
//! holding at least TREE of matter), `tree_res` (the matter standing in them), `res_max` and
//! `tree_eaten` (intake from cells at TREE or more); the per-place log gets `trees`.
...
impl Food {
    ...
    fn shade(&self, g: Grid, grow: &[f32], rate: f32, cap: f32, crowd: &[u16], claims: &mut [f32], light: &mut [f32]) -> f32 {
        ...
        for y in 0..g.h {
            for x in 0..g.w {
                let t = g.idx(x, y);
                let hc = self.res[t];
                let hunger = rate * (cap - hc).max(0.0) / cap;
                if hc <= 0.0 || hunger <= 0.0 || crowd[t] > 0 {
                    continue;
                }
                for d in 1..=(hc.ceil() as isize).min(reach_max) {
                    ring(x, y, d, Box::new(|n: usize| {
                        let s = hunger * (hc - self.res[n] - (d - 1) as f32) / cap;
                        if s > 0.0 {
                            claims[n] += s;
                        }
                    }));
...
fn main() {
    ...
    let prefix = format!("experiments/e021_canopy/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"grain\":{RELIEF_GRAIN},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), list(&terrain.height)).unwrap();
    }
    // The air: what bodies burn, until it rains. The most that can fall on each cell per step,
    // and the sum of it.
    let mut air = 0.0f64;
    let rain_caps = rain.caps(&terrain.height, relief);
    let rain_total: f64 = rain_caps.iter().map(|&c| c as f64).sum();
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height").unwrap();
    ...
    writeln!(lineages_csv, "step,lineage,size,mass,hard,muscle,sensor,digestive,bite,shell,open,bodies,age,plant,meat,p0,p1,pnone,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height").unwrap();
    ...
    writeln!(places_csv, "step,place,pop,mass,hard,muscle,sensor,digestive,bite,shell,biters,cover,plant_intake,meat_intake,lineages,movers,foot,shell_front,shell_back,dead,carrion,soil,barren,regrowth,cells,rain,trees").unwrap();
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent,shaded,dead,carrion,soil,matter,barren,rot,spent,flow,soil_cells,deep,air,rain,shade,trees,tree_res,res_max,tree_eaten"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        // light it has left.
        if shade_rate > 0.0 {
            shade_moved += food.shade(g, &patches.grow, shade_rate, cap, &occ.crowd, &mut claims, &mut light) as f64;
        } else {
            light.copy_from_slice(&patches.grow);
        }
        for c in 0..w * h {
            let r = food.regrow(c, light[c], occ.crowd[c] > 0, cap);
        ...
        for i in 0..agents.len() {
            ...
            if breath < 1.0 {
                food.spend(a.here(g), upkeep * (1.0 - breath));
            }
            air += (upkeep * breath) as f64;
            spent += upkeep as f64;
            ...
            if action == 1 {
                ...
                if agents[i].alive {
                    ...
                    if breath < 1.0 {
                        food.spend(agents[i].here(g), paid * (1.0 - breath));
                    }
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            for (id, m) in &assigned {
                ...
                writeln!(lineages_csv, "{step},{id},{},{:.1},{:.1},{:.1},{:.1},{:.1},{:.1},{:.2},{:.1},{},{:.0},{:.2},{:.2},{},{},{},{:.1},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.1}", m.len(), mass / sz,
                    ...
                    at[0], at[1], at[N_PLACES], bite_any / sz, s_front / sz, s_back / sz, s_side / sz, foot / sz, long / sz, wide / sz, height / sz).unwrap();
            }
        ...
        if step % AGENT_DUMP_INTERVAL == 0 {
            ...
            for a in &agents {
                let k = &a.body.kinds;
                let b = &a.body;
                writeln!(agents_csv, "{step},{},{},{},{},{},{},{},{:.2},{},{:.3},{},{:.2},{:.2},{:.2},{},{},{},{},{:.2},{:.2},{:.2},{},{},{},{:.1}",
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            let trees = food.res.iter().filter(|&&r| r >= TREE).count();
            let tree_res = food.res.iter().filter(|&&r| r >= TREE).map(|&r| r as f64).sum::<f64>();
            let res_max = food.res.iter().copied().fold(0.0f32, f32::max);
            writeln!(log, ",{:.3},{:.5},{:.2},{:.4},{:.2},{soil_stock:.1},{matter:.1},{:.3},{:.4},{:.3},{:.3},{soil_cells:.3},{deep:.3},{air:.1},{:.3},{:.3},{trees},{tree_res:.1},{res_max:.2},{:.3}", pushes as f64 / tried, move_spent / LOG_INTERVAL as f64 / pop as f64, shaded / LOG_INTERVAL as f64,
                ...
                shade_moved / LOG_INTERVAL as f64, tree_eaten / LOG_INTERVAL as f64).unwrap();
            for p in 0..=N_PLACES as u8 {
                ...
                let trees_here = patches.place.iter().enumerate().filter(|&(c, &q)| q == p && food.res[c] >= TREE).count();
                writeln!(places_csv, "{step},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{:.3},{:.1},{:.1},{},{movers},{:.2},{:.2},{:.2},{:.4},{:.2},{:.1},{:.4},{:.4},{},{:.4},{trees_here}", place_name(p), here.len(),
                    ...
                    regrowth_at[p as usize] / LOG_INTERVAL as f64, cells_at[p as usize], rain_at[p as usize] / LOG_INTERVAL as f64).unwrap();
            }
...
mod tests {
    ...
    fn the_air_rains_on_the_mountains_at_most_the_sun_per_step() {
        ...
        let mut food = Food { res: vec![0.0; 3], carrion: vec![0.0; 3], soil: vec![0.0f64; 3] };
        let mut air = 1.0f64;
    ...
    fn the_tall_plant_takes_the_light() {
        ...
        let bare = |mut res: Vec<f32>, put: &[(usize, usize, f32)]| {
            for &(x, y, h) in put {
                res[g.idx(x, y)] = h;
            }
            res
        ...
        let food = Food { res: bare(vec![0.0; n], &[(16, 16, 4.0)]), carrion: vec![0.0; n], soil: vec![0.0f64; n] };
        let moved = food.shade(g, &grow, 1.0, 8.0, &crowd, &mut claims, &mut light);
        assert!((moved - 0.10).abs() < 1e-5, "{moved}");
        assert!((light[g.idx(16, 16)] - 0.11).abs() < 1e-5);
        ...
        assert!((light.iter().map(|&l| l as f64).sum::<f64>() - 0.01 * n as f64).abs() < 1e-4);
        // A full crown intercepts nothing: the tree at the cap is a standing larder.
        let food = Food { res: bare(vec![0.0; n], &[(16, 16, 8.0)]), carrion: vec![0.0; n], soil: vec![0.0f64; n] };
        assert_eq!(food.shade(g, &grow, 1.0, 8.0, &crowd, &mut claims, &mut light), 0.0);
        ...
        let food = Food { res: vec![8.0; n], carrion: vec![0.0; n], soil: vec![0.0f64; n] };
        assert_eq!(food.shade(g, &grow, 1.0, 8.0, &crowd, &mut claims, &mut light), 0.0);
        // A column under a body claims nothing; its own sun can still be claimed.
        let food = Food { res: bare(vec![0.0; n], &[(16, 16, 4.0), (17, 16, 2.0)]), carrion: vec![0.0; n], soil: vec![0.0f64; n] };
        let mut held = vec![0u16; n];
        held[g.idx(17, 16)] = 1;
        food.shade(g, &grow, 1.0, 8.0, &held, &mut claims, &mut light);
        ...
        let food = Food { res: bare(vec![0.0; n], &[(15, 16, 4.0), (17, 16, 4.0)]), carrion: vec![0.0; n], soil: vec![0.0f64; n] };
        food.shade(g, &grow, 1.0, 8.0, &crowd, &mut claims, &mut light);
        assert!((light[g.idx(15, 16)] - light[g.idx(17, 16)]).abs() < 1e-6);
        assert!(light[g.idx(16, 16)] < 0.01);
        // Rate 0 is e020: nothing moves.
        let moved = food.shade(g, &grow, 0.0, 8.0, &crowd, &mut claims, &mut light);
        assert_eq!(moved, 0.0);
        assert!(light.iter().all(|&l| (l - 0.01).abs() < 1e-9));
    }
