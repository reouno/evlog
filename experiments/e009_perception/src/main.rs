//! e009: perception. The e007 world (patchy food, sexual mode, e007's kill value) with one
//! change in what the policy sees. Mode `counts` is e007: the neighbor inputs are counts of
//! agents in four directions. Mode `who` adds eight inputs, two per direction: how many of those
//! neighbors can eat me, and how many I can eat, by the predation rule itself, weighted by sense
//! (sensor blocks / 8), so that an agent without sensors sees nothing new. Mode `blind` is the
//! knockout: sense forced to 0 for everyone (the eight inputs and the distance-2 inputs are 0;
//! sensor blocks still cost upkeep). `counts` reproduces e007/e008 byte for byte; `who` shares
//! its laws and initial population, the weights of the new inputs come from a separate stream.
...
const YOUNG: u32 = 50; // a kill of prey younger than this counts as a kill of a newborn
...
// Policy: 10 inputs (e007) plus 8 `who` inputs -> 5 actions, read from the same table (no
// position input). The table columns are: kinds, e007's weights and biases (as e007, from the
// world's rng), then the `who` weights (from their own rng stream).
const N_IN: usize = 10;
const N_WHO: usize = 8;
const N_OUT: usize = 5;
const N_BASE: usize = N_IN * N_OUT + N_OUT;
const N_POLICY: usize = N_BASE + N_WHO * N_OUT;
const K: usize = N_KINDS + N_POLICY;
const K_BASE: usize = N_KINDS + N_BASE;
...
impl Laws {
    fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        let mut table: Vec<[f32; K]> = (0..256)
            .map(|_| {
                let mut row = [0.0; K];
                for v in row[..K_BASE].iter_mut() {
                    *v = rng.f32() * 2.0 - 1.0;
                }
                row
            })
            .collect();
        let mut who_rng = Rng(seed.wrapping_mul(0xA0761D6478BD642F) | 1);
        for row in table.iter_mut() {
            for v in row[K_BASE..].iter_mut() {
                *v = who_rng.f32() * 2.0 - 1.0;
            }
        }
...
impl Body {
    ...
    /// The predation rule, without the escape roll: my attack beats its defense, its mass fits
    /// my gut, and it is not my own body.
    fn can_eat(&self, prey: &Body) -> bool {
        self.attack as f32 > prey.defense() && prey.mass as f32 <= self.gut() && self.cells != prey.cells
    }
}
...
/// `who` inputs are added after e007's sum; when they are all 0 the result is e007's, bit for bit.
fn act(policy: &[f32; N_POLICY], input: &[f32; N_IN], who: &[f32; N_WHO]) -> usize {
    let mut best = 0;
...
fn main() {
    ...
    let mode = match args.get(5).map(String::as_str) {
        None | Some("counts") => Mode::Counts,
        Some("who") => Mode::Who,
        Some("blind") => Mode::Blind,
        Some(m) => panic!("unknown mode {m}: use counts, who or blind"),
    };
    let mode_name = match mode { Mode::Counts => "counts", Mode::Who => "who", Mode::Blind => "blind" };
    let sexual = true;
    let d = D;
    ...
    let prefix = format!("experiments/e009_perception/results/{size}_{}_{mode_name}_seed{seed}", if patchy { "patchy" } else { "uniform" });
    ...
    writeln!(lineages_csv, "step,lineage,size,hard,muscle,sensor,digestive,attack,bodies,age,plant,meat").unwrap();
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_eaten,deaths_body,escapes,plant_intake,meat_intake,\
         ...
         prey_age_mean,gain_mean,kills_young_share,meat_majority,eaten_with_sensor,kills_by_sensor,who_decisions,who_used"
    )
    .unwrap();
    ...
    let mut prey_age = 0u64; // summed over the kills of the log interval
    let mut kills_young = 0u64; // kills of prey younger than YOUNG steps
    let mut eaten_with_sensor = 0u64; // prey killed that had at least one sensor block
    let mut kills_by_sensor = 0u64; // kills made by an eater with at least one sensor block
    let mut who_decisions = 0u64; // moves decided by an agent with sensors that saw a threat or prey (a who input above 0)
    let mut who_used = 0u64; // of those, moves that differ from the choice with the who inputs zeroed
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            if attack > 0.0 {
                ...
                if let Some(j) = prey {
                    let p = &agents[j];
                    let gain = MEAT_ENERGY * p.energy.max(0.0) + MEAT_MASS * p.body.mass as f32;
                    prey_age += p.age as u64;
                    if p.age < YOUNG {
                        kills_young += 1;
                    }
                    if p.body.kinds[SENSOR] > 0 {
                        eaten_with_sensor += 1;
                    }
                    if agents[i].body.kinds[SENSOR] > 0 {
                        kills_by_sensor += 1;
                    }
            ...
            // 3. Move.
            let a = &agents[i];
            let s = if mode == Mode::Blind { 0.0 } else { a.body.sense() };
            let thr = a.body.threshold();
            let others = |c1: usize, c2: usize| count_at(c1, &head, &next) + s * count_at(c2, &head, &next);
            // `who`: per direction, neighbors (distance 1 and 2) that can eat me, and that I can
            // eat, by the predation rule, times sense. Only computed for agents with sensors.
            let mut who = [0.0f32; N_WHO];
            if mode == Mode::Who && s > 0.0 {
                for (d, (c1, c2)) in [(idx(x, yn), idx(x, yn2)), (idx(x, yp), idx(x, yp2)), (idx(xp, y), idx(xp2, y)), (idx(xn, y), idx(xn2, y))].into_iter().enumerate() {
                    let (mut threat, mut prey) = (0.0, 0.0);
                    for c in [c1, c2] {
                        let mut j = head[c];
                        while j != u32::MAX {
                            let n = &agents[j as usize];
                            if j as usize != i && n.alive {
                                if n.body.can_eat(&a.body) {
                                    threat += 1.0;
                                }
                                if a.body.can_eat(&n.body) {
                                    prey += 1.0;
                                }
                            }
                            j = next[j as usize];
            ...
            let action = act(&a.body.policy, &input, &who);
            actions[action] += 1;
            if s > 0.0 {
                sense_decisions += 1;
                let blind = [
                ...
                if act(&a.body.policy, &blind, &[0.0; N_WHO]) != action {
                    sense_used += 1;
                }
                if who.iter().any(|&w| w > 0.0) {
                    who_decisions += 1;
                    if act(&a.body.policy, &input, &[0.0; N_WHO]) != action {
                        who_used += 1;
                    }
                }
            }
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            for (id, m) in &assigned {
                ...
                writeln!(lineages_csv, "{step},{id},{},{:.1},{:.1},{:.1},{:.1},{:.1},{},{:.0},{:.2},{:.2}", m.len(),
                    kinds[HARD] / sz, kinds[MUSCLE] / sz, kinds[SENSOR] / sz, kinds[DIGESTIVE] / sz, attack / sz, bodies.len(), age / sz, plant / sz, meat / sz).unwrap();
            }
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            // Kills in this window: mean age of the prey, mean gain per kill, share of kills of
            // newborns. And the share of agents that got most of their food from prey.
            let kills = deaths[2].max(1) as f64;
            let meat_majority = agents.iter().filter(|a| a.meat > a.plant && a.meat > 0.0).count() as f32 / pop;
            writeln!(log, ",{:.0},{:.2},{:.3},{meat_majority:.3},{eaten_with_sensor},{kills_by_sensor},{who_decisions},{:.3}", prey_age as f64 / kills, meat_intake as f64 / kills, kills_young as f64 / kills,
                if who_decisions > 0 { who_used as f64 / who_decisions as f64 } else { 0.0 }).unwrap();
            births = 0;
            sexual_births = 0;
