...
impl Body {
    pub fn new(cells: [u8; CELLS], side: usize, policy: [f32; N_POLICY], n_genes: u16, density: f32) -> Self {
        let mut b = Body { cells, side: side as u8, size: 0, mass: 0.0, density, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE_MAX]; 4], extent: [0; 2], policy, n_genes, cut: 0, water_policy: [0.0; N_WATER_POLICY], open_soft: 0, breath_policy: [0.0; N_OUT], breed: BREED, share: SHARE, store: STORE };
        b.refresh();
        b
    }
    pub fn empty() -> Self {
...
mod tests {
    ...
    /// before any selection (`cargo test --release -p e069_history start_spread -- --ignored --nocapture`).
    #[test]
    #[ignore]
    fn start_spread() {
