...
impl Plants {
    ...
    pub fn matter(&self) -> f64 {
        let cells: f64 = (0..self.soil.len()).map(|c| self.soil[c] + self.grass[c] + self.wood[c] + self.algae[c] + self.litter[c] + self.carrion[c]).sum();
        cells + self.air
    }
    ...
    /// e072 (set B): a gut on land in a body with a tooth hard enough for wood takes at most `bite`
    /// from the grass, the carrion and `share` of the wood of cell `c`, in their proportions. Returns
    /// (grass, dead, wood) taken.
    pub fn take_with_wood(&mut self, c: usize, bite: f64, share: f64) -> (f64, f64, f64) {
        let (g, d, w) = (self.grass[c].max(0.0), self.carrion[c].max(0.0), (self.wood[c] * share).max(0.0));
        let all = g + d + w;
        if all <= 0.0 {
            return (0.0, 0.0, 0.0);
        }
        let k = (bite / all).min(1.0);
        let (tg, td, tw) = (g * k, d * k, w * k);
        self.grass[c] -= tg;
        self.carrion[c] -= td;
        self.wood[c] -= tw;
        (tg, td, tw)
    ...
    pub fn update(&mut self, w: &World, p: &Params, year: u32) -> Flow {
        ...
        for c in 0..cells {
            ...
            if p.water > 0.0 {
                ...
                self.soil[c] += rot - (grow_g + grow_w + grow_a);
            } else {
                self.litter[c] = lit + die_g + die_w - rot;
                self.soil[c] += die_a + rot - (grow_g + grow_w + grow_a);
            }
            f.grass += grow_g;
            f.wood += grow_w;
            f.algae += grow_a;
