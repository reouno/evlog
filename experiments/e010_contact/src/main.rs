//! e010: contact physics. The e007 world (patchy food, sexual mode, lineages) with the trait
//! read-outs removed. There is no attack, defense, gut, escape roll or kin exclusion. A block
//! is a material: hard resists (hardness 3 per contiguous hard cell from the tip), muscle pushes
//! (force = muscle cells in its line, in the direction the body moves) and moves (speed =
//! muscle / mass), digestive turns matter into energy (plants and broken cells), sensor gives
//! inputs, empty is nothing. A cell costs the same energy to build as it holds when eaten. When
//! a body moves toward a cell that holds other bodies, it pushes into them along the facing
//! side, line by line: the softer of the two tips breaks if the push exceeds its hardness (a
//! soft tip crushes against armor; armor breaks under a strong push). A broken cell is gone
//! from that body; its energy goes to the pusher if it has digestive cells. A body with no
//! cells left dies. Everything a creature can do (bite, armor, hide, hurt its kin) is a
//! consequence of shape, material, and which way it moves.
...
fn main() {
    ...
    let prefix = format!("experiments/e010_contact/results/{size}_{}_seed{seed}", if patchy { "patchy" } else { "uniform" });
    ...
    for step in 1..=steps {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.0},{:.3},{:.3},{meat_majority:.3},{contacts},{shell_mean:.2},{open_mean:.2},{full:.3},{damaged:.3},{biters:.3}",
