#!/usr/bin/env python3
"""Read e081's runs (#94): does a crowd pay for its water when drinking takes from the ground?

Run from the repo root:
`uv run python experiments/e081_drink/sweep.py [--every 10000] <dir or run prefix> ...`
...
def main():
    ...
    for arg in args:
        for pre in prefixes(arg):
            ...
            out = {"run": name, "unit": float(row.get("unit", 0))}
            ...
            rows.append(out)
    print(f"{'run':>10} {'unit':>5} {'land':>5} {'thirst':>6} {'home':>5} {'lawn/st aut':>11} {'spr':>5}"
    ...
    for r in rows:
        ...
        print(f"{r['run']:>10} {r['unit']:>5g} {r['pop_land']:>5.0f} {r['d_thirst']:>6.1%} {r['home']:>5.2f} {r['lawn_autumn_share']:>11.2f} {r['lawn_spring_share']:>5.2f}"
