//! e036: a store in the ground (#36). One law about the world, off by default (`root` 0 is
//! e035 byte for byte):
//!
...
//!   on (`grew`, round 1) or of the cell the fruit falls on (`fell`, round 2). Round 1 showed
//!   the surplus is made on the hundred-odd cells under the crowns, so a store laid where it
//!   grew is a larder the crowd stands on. What is dug is plant matter (digested as plant).
//!
...
fn main() {
    ...
    let prefix = format!("experiments/e036_ground/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"digest\":{digest_law},\"digest_floor\":{DIGEST_FLOOR},\"side\":\"{}\",\"side_max\":{SIDE_MAX},\"store\":{store},\"yolk\":{yolk},\"breed\":{breed_law},\"winter\":\"{}\",\"water\":{water_rate},\"leach\":{leach},\"depth\":{depth},\"mix\":{mix},\"root\":{root_cap},\"dig\":{dig},\"root_at\":\"{}\",\"water_rain\":{WATER_RAIN},\"water_evap\":{WATER_EVAP},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), side_law.name(), winter.name(), root_at.name(), list(&terrain.height)).unwrap();
    }
