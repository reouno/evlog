...
const PLACES: &str = "experiments/e087_torpor/results"; // e087: the maps of the places by month
...
params! {
    ...
    // e087's pace (#93, Q), 0 being e082
    q10 = 0.0, "e087 (Q): a body takes clock x q10^((T - warm_lo) / 10) turns a step, at most its clock; it exchanges heat by the step and makes it by the turn, and neither pays to warm nor dies of cold (0: set A as e072 built it)";
    q_floor = 0.1, "e087 (Q): the least share of its clock a cold body keeps";
    torpid = 0.5, "e087: a body is counted torpid while its pace is at most this share of its clock (a measure)";
}

/// e079: the settled world's keys added after e078; at 0 they leave its hash, so e078's world is read.
const NEW_WORLD_KEYS: [&str; 2] = ["crown_wet", "wood_rest"];

/// The parameters of the bodies; every other one makes the settled world.
const BODY_KEYS: [&str; 48] = [
    ...
    "census", "census_from", "shade_heat", "shade_dry", "cell_map", "crown_cool", "unit", "q10", "q_floor", "torpid",
];
...
fn world_hash(p: &Params) -> u64 {
    ...
    for (name, v) in Params::NAMES.iter().zip(p.values()) {
        if !BODY_KEYS.contains(name) && !(NEW_WORLD_KEYS.contains(name) && v == 0.0) {
...
fn settled_world(p: &Params, dir: Option<&str>) -> (World, Plants, u64) {
    ...
    for yr in 1..=years {
        ...
        eprintln!("settling year {yr}: wood {:.3} a land cell, {stands} stand cells, {mid} of them at 20-50 degrees", wood / land.max(1) as f64);
    }
...
struct Sim {
    ...
    places: Option<Places>, // e087: the places by month and the winters lived through (a measure)
...
impl Sim {
    ...
    fn with_pools(p: Params, w: World, pl: Plants, update_i: u64, threads: usize, pools: Pools) -> Sim {
        ...
        let mut sim = Sim {
            ...
            fill_sum: vec![0.0; n * n], rain_sum: vec![0.0; n * n], season: None, places: None, wsea: 0.0, wclim: 0.0, w0: 0.0,
        };
    ...
    fn bodies(&mut self) {
        // e087: a new month is read before the bodies move (the climate's month, as e086 counts it).
        let month = ((self.update_i % self.per_year) * 12 / self.per_year) as u32;
        let step = self.step;
        if let Some(pc) = self.places.as_mut() {
            pc.month(month, step, (self.p.year / 12.0) as u64, &self.agents, self.g, GROWN);
        }
        let Sim { p, w, agents, pl, occ, g, rng, wear_rng, k, hab, wet: wet_cells, dryness, drink_at, vis, laws, cache, threads, next_id, born, dead, wsea, places, .. } = self;
        let g = *g;
        ...
        let pace_on = heat_on && p.q10 > 0.0; // e087 (Q)
        let q_seen = if p.q10 > 0.0 { p.q10 } else { 2.5 }; // e087: what "torpid" reads without Q
        ...
        for i in 0..agents.len() {
            ...
            a.age += 1;
            k.body_steps += 1;
            // e087 (Q): the flesh's pace follows its warmth. The body exchanges heat with its cells every
            // step (the share of the way set A takes in a turn), and makes it only in its turns (below).
            let mut warm = 1.0;
            if pace_on {
                let (pass, seen) = heat_flux(a, g, &w.temp_read, p.heat_hard, p.heat_fat);
                if pass > 0.0 {
                    let r = (p.heat * pass / a.mass().max(1e-6) as f64).min(1.0);
                    a.temp += (r * (seen - a.temp as f64)) as f32;
                }
                warm = warmth(a.temp as f64, p.warm_lo, p.q10, p.q_floor);
            }
            if let Some(pc) = places.as_mut() {
                pc.body_step(a, g, warmth(a.temp as f64, p.warm_lo, q_seen, p.q_floor) <= p.torpid, GROWN);
            }
            a.phase += pace(a.mass()) * warm as f32;
            if a.phase < 1.0 {
                continue;
            }
            a.phase -= 1.0;
            ...
            if heat_on {
                ...
                if pass > 0.0 {
                    ...
                    if pace_on {
                        // e087 (Q): the turn makes the heat its upkeep burns; the exchange was the step's.
                        a.temp += (r * p.heat_make * full / pass) as f32;
                    } else {
                        let target = seen + p.heat_make * full / pass;
                        a.temp += (r * (target - a.temp as f64)) as f32;
                    }
                }
                // e087 (Q): a cold body slows instead of paying to warm, and does not die of cold.
                if !pace_on && (a.temp as f64) < p.warm_lo {
    ...
    fn band_rows(&mut self, step: u64, out: &mut dyn Write) {
        ...
        let n = self.w.n;
        let mut acc = vec![[0.0f64; 11]; BANDS * 3];
        ...
        for (k, r) in acc.iter().enumerate() {
            if r[0] > 0.0 {
                writeln!(out, "{step},{},{},{},{:.4},{:.2},{:.2},{:.2},{},{:.2},{:.3},{:.3},{:.4},{:.3}", (k / 3 * 10) as i64 - 60, k % 3, r[0], r[1], r[2], r[3], r[4] / r[0], r[5], r[6] / r[0], r[7] / r[0], r[8] / r[0], r[9] / r[0], r[10] / r[0]).unwrap();
            }
        }
    ...
    fn cell_map(&self) -> Option<Vec<u8>> {
        ...
        let n = self.w.n;
        let mut out = Vec::from(&b"E079CELL"[..]);
...
fn main() {
    ...
    // e087: the places by month, when e086's map of this year has been written (`places.py`).
    sim.places = Places::load(&format!("{PLACES}/places_y{}.bin", p.year as u64), sim.g.w, &prefix);
    ...
    writeln!(bands_csv, "step,lat,wood_class,cells,grown,grass,wood,temp,bodies,felt,over,under,fill,rain").unwrap();
...
mod tests {
    ...
    /// e087 (Q): in a world colder than the band a body slows instead of paying to warm, and none dies
    /// of cold; the matter is still conserved. Without Q the same world pays to warm and has cold deaths.
    /// In a world hotter than the band a body under Q sweats as under set A.
    #[test]
    fn a_cold_body_slows_and_does_not_pay_to_warm() {
        let run = |q10: f64, lo: f64| {
            let mut p = small();
            p.heat = 0.05;
            p.warm_lo = lo;
            p.warm_hi = lo + 10.0;
            ...
            let m = sim.matter();
            assert!((m - start).abs() < 1e-9 * start, "matter {start} -> {m}");
            (sim.k.turned as f64 / sim.k.body_steps as f64, sim.k.warmed_by.iter().sum::<f64>(), sim.k.cold, sim.k.cooled_by.iter().sum::<f64>())
        };
        // Every cell under the band: under Q the bodies slow, without it they pay and die of cold.
        let (slow, warmed, cold, _) = run(2.5, 40.0);
        assert_eq!((warmed, cold), (0.0, 0));
        let (pace, warmed, cold, _) = run(0.0, 40.0);
        assert!(slow < 0.5 * pace, "turns a step {slow} against {pace}");
        assert!(warmed > 0.0 && cold > 0, "warmed {warmed}, cold deaths {cold}");
        // Every cell over the band: under Q a body still sweats.
        let (_, _, _, cooled) = run(2.5, -60.0);
        assert!(cooled > 0.0, "no body sweated under Q");
    }
