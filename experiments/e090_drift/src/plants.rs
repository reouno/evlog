...
impl Plants {
    ...
    pub fn matter(&self) -> f64 {
        let cells: f64 = (0..self.soil.len()).map(|c| self.soil[c] + self.grass[c] + self.wood[c] + self.browse[c] + self.algae[c] + self.litter[c] + self.carrion[c] + self.seed[c]).sum();
        cells + self.air
    }
    ...
    pub fn update(&mut self, w: &World, p: &Params, year: u32) -> Flow {
        ...
        for c in 0..cells {
            ...
            if p.water > 0.0 {
                ...
                self.soil[c] += rot + rot_b + rot_s - (grow_g + grow_w + grow_a + drop_b);
            } else {
                self.litter[c] = lit + die_g + die_w - rot;
                self.soil[c] += die_a + rot + rot_b + rot_s - (grow_g + grow_w + grow_a + drop_b);
            }
            f.grass += leaf;
            f.seed += set;
            f.sprout += sprout;
