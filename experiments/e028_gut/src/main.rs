...
//! the season; 1 by default), `digest` (0, 1 or 2; 1 by default: the line; 2 the sharp curve).
...
impl Laws {
    fn new(rng: &mut Rng, seed: u64) -> Self {
        ...
        Laws { morphogen, table, density, digest, morph_level }
    }
}
...
impl Body {
    fn new(cells: [u8; CELLS], policy: [f32; N_POLICY], n_genes: u16, density: f32, by_kind: bool) -> Self {
        let mut b = Body { cells, size: 0, mass: 0.0, density, digest: 0.5, by_kind, kinds: [0; N_KINDS], tips: [[Tip::default(); SIDE]; 4], extent: [0; 2], policy, n_genes };
        b.refresh();
        b
    }
...
fn develop_genes(genes: &[Gene], laws: &Laws, weight: Weight) -> Body {
    ...
    let mut body = Body::new(cells, policy, n as u16, density, weight.kind);
    // The digestion axis (#32): read like the density, a sigmoid of the sum.
    let mut d = 0.0f32;
    for (i, g) in genes.iter().enumerate() {
        d += laws.digest[pattern_index(&g.product)] * level[i * CTX + CELLS];
    }
    body.digest = sigmoid(d);
    body
}
...
fn main() {
    ...
    let digest_law: u8 = args.get(19).and_then(|s| s.parse().ok()).unwrap_or(1);
    ...
    let prefix = format!("experiments/e028_gut/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"digest\":{digest_law},\"digest_floor\":{DIGEST_FLOOR},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), list(&terrain.height)).unwrap();
    }
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height,fat,size,density,digest").unwrap();
    ...
    writeln!(lineages_csv, "step,lineage,size,mass,hard,muscle,sensor,digestive,bite,shell,open,bodies,age,plant,meat,p0,p1,pnone,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height,density,digest").unwrap();
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_broken,deaths_body,cells_broken,plant_intake,meat_intake,\
         ...
         blocked,shoves,turns_blocked,births_no_room,foot_mean,len_fwd,len_side,bite_any_mean,biters_any_share,shell_front,shell_back,shell_side,pushes,move_spent,shaded,dead,carrion,soil,matter,barren,rot,spent,flow,soil_cells,deep,air,rain,shade,trees,tree_res,res_max,tree_eaten,fruit,fruit_stock,fruit_eaten,clones,mutations,fat_mean,fat_stock,worth,kill_gain,size_mean,density_mean,density_std,sun,cloud_std,digest_mean,digest_std,flesh_guts,dung"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            for (id, m) in &assigned {
                ...
                let (mut density, mut digest) = (0.0f32, 0.0f32);
                ...
                writeln!(lineages_csv, "{step},{id},{},{:.1},{:.1},{:.1},{:.1},{:.1},{:.1},{:.2},{:.1},{},{:.0},{:.2},{:.2},{},{},{},{:.1},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.1},{:.3},{:.3}", m.len(), mass / sz,
                    ...
                    at[0], at[1], at[N_PLACES], bite_any / sz, s_front / sz, s_back / sz, s_side / sz, foot / sz, long / sz, wide / sz, height / sz, density / sz, digest / sz).unwrap();
            }
        ...
        if step % AGENT_DUMP_INTERVAL == 0 {
            ...
            for a in &agents {
                ...
                write!(agents_csv, ",{:.3},{},{:.3},{:.3}", a.fat, b.size, b.density, b.digest).unwrap();
...
mod tests {
    ...
    fn a_gut_digests_by_its_axis_and_the_rest_is_dung() {
        ...
        let mut b = Body::new([0; CELLS], [0.0; N_POLICY], 0, 1.0, true);
