...
//!
//! Run: cargo run --release -p e063_bodies -- <prefix> [key=value ...] [file.params ...]
...
const WORLDS: &str = "experiments/e063_bodies/results/worlds";
...
const BODY_KEYS: [&str; 3] = ["life", "steps", "start"];
...
impl Sim {
    ...
    fn matter(&self) -> f64 {
        self.pl.matter() + self.agents.iter().map(|a| a.energy.max(0.0) + a.fat + a.body.matter()).sum::<f64>()
    }
    ...
    fn bodies(&mut self) {
        let Sim { agents, pl, occ, g, rng, wear_rng, k, hab, laws, cache, threads, next_id, born, dead, .. } = self;
        let g = *g;
        ...
        for i in 0..agents.len() {
            ...
            a.turns += 1;
            k.turned += 1;
            k.worn += wear_blocks(a, wear_chance(a.turns), wear_rng, g, occ, &mut pl.carrion, &mut k.cc) as u64;
            if !a.alive {
                continue;
            }
            ...
            for (j, c) in guts.iter().enumerate() {
                let (pg, dg) = pl.take(c, (BITE * gut_n[j] as f32) as f64);
                plant += pg;
                scavenged += dg;
            }
            ...
            pl.soil[a.here(g)] += from_fat + over;
            k.upkept += 1;
            ...
            let food = |c: usize| pl.grass[c] + pl.carrion[c];
            ...
            if action == 1 {
                k.tried += 1;
                let d = f;
                let pressed = push(agents, i, d, g, occ, &mut pl.carrion, &mut k.cc);
                if agents[i].alive {
                    let mut work = 0.0f32;
                    ...
                    pl.soil[a.here(g)] += paid + from_fat;
                }
            } else if action >= 2 {
        ...
        for (mut a, genes, parent) in pending.drain(..) {
            ...
            match spot {
                ...
                _ => {
                    k.no_room += 1;
                    lay(&mut pl.carrion, g.wcell(px, py), a.energy + a.fat, &mut k.cc);
                }
            }
        }
        ...
        for a in agents.iter_mut() {
            ...
            if let Some(cause) = cause {
                ...
                occ.release(g, a);
                a.alive = false;
                lay_body(a, g, &mut pl.carrion, &mut k.cc);
            }
        }
    ...
    const LOG_HEADER: &'static str = "step,pop,grown,births,children,sexual,no_room,empty,deaths_hunger,deaths_broken,deaths_wear,cells_broken,contacts,\
        ...
        grass,wood,algae,litter,carrion,air,soil_land,spent,matter,matter_err,burnt,land_temp,develops,ms_step,ms_world,ms_bodies,ms_lineage,ms_wall";
    ...
    fn log_row(&mut self, steps: u64, matter0: f64, wall: f64) -> (String, Point) {
        ...
        s.push_str(&format!(
            ",{travel_p50:.2},{path_mean:.2},{:.0},{:.0},{:.0},{:.0},{:.1},{:.1},{soil_land:.0},{:.3},{matter:.3},{err:.2e},{:.5},{land_temp:.2},{},{:.3},{:.3},{:.3},{:.3},{:.3}",
            sum(&pl.grass),
            sum(&pl.wood),
            sum(&pl.algae),
            sum(&pl.litter),
...
fn main() {
    ...
    let prefix = args.first().expect("usage: e063_bodies <prefix> [key=value ...] [file.params ...]").clone();
    let p = parse(&args[1..]);
...
mod tests {
    ...
    /// and the world's matter neither grows nor shrinks.
    #[test]
