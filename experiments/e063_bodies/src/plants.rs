...
fn depth(w: &World, p: &Params, c: usize) -> Option<f64> {
    if w.sea[c] {
        Some(-w.elev[c])
    } else {
...
impl Plants {
    ...
    /// A gut takes at most `bite` from cell `c`: the grass and the carrion in their proportions.
    /// Returns (plant, dead) taken.
    pub fn take(&mut self, c: usize, bite: f64) -> (f64, f64) {
        let (g, d) = (self.grass[c], self.carrion[c]);
        let all = g + d;
        if all <= 0.0 {
            return (0.0, 0.0);
        }
        ...
        let plant = bite * g / all;
        let dead = bite - plant;
        self.grass[c] = g - plant;
        self.carrion[c] = d - dead;
        (plant, dead)
    }
