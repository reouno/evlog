...
/// Dead matter `e` lies on world cell `c`.
pub fn lay(carrion: &mut [f64], c: usize, e: f64, cc: &mut Counters) {
    if e <= 0.0 {
        return;
    }
    carrion[c] += e;
...
pub fn lay_body(a: &Agent, g: Grid, carrion: &mut [f64], cc: &mut Counters) {
    let energy = a.energy.max(0.0) + a.fat;
    if a.n_filled == 0 {
        lay(carrion, g.wcell(a.x, a.y), energy, cc);
        return;
    }
    let share = energy / a.n_filled as f64;
    for p in a.cells_held() {
        let (sx, sy) = a.sub_at(g, p, NORTH, 0);
        lay(carrion, g.wcell(sx, sy), a.body.block_matter(a.wcells[p]) + share, cc);
    }
}
...
pub fn push(agents: &mut [Agent], i: usize, d: usize, g: Grid, occ: &mut Occ, carrion: &mut [f64], c: &mut Counters) -> Vec<(usize, u8)> {
    let opp = opposite(d);
    ...
    for (victim, pos, eater) in breaks {
        ...
        if e.body.kinds[DIGESTIVE] > 0 {
            ...
            lay(carrion, under, taken, c);
        }
    }
    for v in victims {
        let lost = cut_loose(&mut agents[v], g, occ, carrion, c);
...
pub fn wear_blocks(a: &mut Agent, h: f64, rng: &mut Rng, g: Grid, occ: &mut Occ, carrion: &mut [f64], c: &mut Counters) -> u16 {
    ...
    for &pos in &failed {
        ...
        a.body.refresh();
        a.reframe();
        lay(carrion, g.wcell(sx, sy), share + fat + matter, c);
    }
    a.worn += failed.len() as u16;
    if !failed.is_empty() {
        let lost = cut_loose(a, g, occ, carrion, c);
...
fn cut_loose(a: &mut Agent, g: Grid, occ: &mut Occ, carrion: &mut [f64], c: &mut Counters) -> u16 {
    let s = a.body.s();
    let (label, parts) = parts_of(&a.body.cells, s);
    if parts < 2 {
        return 0;
    }
    let keep = kept_part(&a.body.cells, s, &label, parts);
...
mod tests {
    ...
    fn a_push_meets_face_to_face() {
        ...
        let pressed = push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, &mut c);
        ...
        occ.claim(g, &agents[1], 1);
        let mut carrion = vec![0.0; g.cells()];
        push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, &mut c);
    ...
    fn the_sea_is_a_wall() {
        ...
        let mut c = Counters::default();
        let mut carrion = vec![0.0; g.cells()];
        assert!(push(&mut agents, 0, EAST, g, &mut occ, &mut carrion, &mut c).is_empty());
    ...
    fn wear_takes_half_the_blocks_by_the_median_life() {
        ...
        let mut rng = Rng(1);
        assert_eq!(wear_blocks(&mut a, 0.0, &mut rng, g, &mut occ, &mut carrion, &mut c), 0);
        assert_eq!(wear_blocks(&mut a, 1.0, &mut rng, g, &mut occ, &mut carrion, &mut c), 3);
