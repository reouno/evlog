...
fn main() {
    ...
    let prefix = format!("experiments/e051_leave/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}{thirst_tag}{stock_tag}{strict_tag}{sat_tag}{hold_tag}{wear_tag}{connect_tag}{yield_tag}{motor_tag}{band_tag}{brain_tag}_seed{seed}", rain.name());
    ...
    writeln!(agents_csv, "step,mass,born_mass,hard,muscle,sensor,digestive,bite,shell,open,speed,age,energy,plant,meat,lineage,place,born_place,bite_any,shell_front,shell_back,shell_side,foot,len_fwd,len_side,height,fat,size,density,digest,side,water,worn,travel,drift,hidden,eta,beta,wchange").unwrap();
    ...
    writeln!(deaths_csv, "step,cause,age,size,n,worn").unwrap();
    ...
    for _ in 0..init_pop {
        ...
        let mut a = Agent {
            id: next_id - 1, lineage: 0, x: 0, y: 0, energy: INIT_ENERGY as f64, age: 0, plant: 0.0, meat: 0.0, fat: 0.0, water: 1.0, born_size: body.size, breed_now: false, born_place: NO_PLACE, born_at: (0, 0), short: false, worn: 0, worn_out: false,
    ...
    let mut death_table: std::collections::BTreeMap<(u8, u32, u16), (u64, u64)> = std::collections::BTreeMap::new();
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            let a = &mut agents[i];
            a.age += 1;
            // e044 (#47): wear. Every block may fail this step, by the body's age; a body left
            // with no block is dead of wear.
            if wear > 0.0 {
                let h = wear_chance(wear, a.age);
            ...
            if agents[i].energy >= thr as f64 && (breed_law == 0 || agents[i].breed_now) {
                ...
                pending.push((Agent {
                    id: next_id - 1, lineage: a.lineage, x: a.x, y: a.y, energy: a.energy, age: 0, plant: 0.0, meat: 0.0, fat: yolk_fat, water: a.water, born_size: 0, breed_now: false, born_place: NO_PLACE, born_at: (0, 0), short: false, worn: 0, worn_out: false, alive: true,
        ...
        for a in agents.iter_mut() {
            ...
            if let Some(cause) = dead {
                ...
                if a.born_size > 0 {
                    ...
                    let e = death_table.entry((cause, a.age / DEATH_BIN * DEATH_BIN, a.born_size / 4 * 4)).or_insert((0, 0));
                    e.0 += 1;
                    e.1 += (a.worn > 0) as u64;
                }
                occ.release(g, a);
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            west = 0;
            for (&(cause, age, size), &(n, wn)) in &death_table {
                writeln!(deaths_csv, "{step},{},{age},{size},{n},{wn}", CAUSES[cause as usize]).unwrap();
            }
            sun_acc = 0.0;
...
mod tests {
    ...
    fn agent_with(cells: [u8; CELLS], x: usize, y: usize, facing: usize) -> Agent {
        let mut a = Agent {
            id: 0, lineage: 0, x, y, energy: 1.0, age: 0, plant: 0.0, meat: 0.0, fat: 0.0, water: 1.0, born_size: 0, breed_now: false, born_place: NO_PLACE, born_at: (0, 0), short: false, worn: 0, worn_out: false, alive: true,
