...
fn main() {
    ...
    let prefix = format!("experiments/e039_reach/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}{flesh_tag}{weight_tag}{weather_tag}{digest_tag}{side_tag}{store_tag}{yolk_tag}{breed_tag}{winter_tag}{water_tag}{root_tag}{k_tag}{sun_tag}{reach_tag}_seed{seed}", rain.name());
    ...
    // The terrain, once: the height and the band of every cell.
    {
        ...
        writeln!(f, "{{\"relief\":{relief},\"flow\":{flow_rate},\"rain\":\"{}\",\"breath\":{breath},\"shade\":{shade_rate},\"spill\":{spill},\"mutation\":{mutation},\"grain\":{RELIEF_GRAIN},\"weather\":\"{}\",\"amplitude\":{amplitude},\"weather_grain\":{WEATHER_GRAIN},\"weather_span\":{WEATHER_SPAN},\"wind\":{WIND},\"season\":{SEASON},\"digest\":{digest_law},\"digest_floor\":{DIGEST_FLOOR},\"side\":\"{}\",\"side_max\":{SIDE_MAX},\"store\":{store},\"yolk\":{yolk},\"breed\":{breed_law},\"winter\":\"{}\",\"water\":{water_rate},\"leach\":{leach},\"depth\":{depth},\"mix\":{mix},\"root\":{root_cap},\"dig\":{dig},\"root_at\":\"{}\",\"k\":{k},\"reach\":{reach},\"reachfix\":{reach_fix},\"water_rain\":{WATER_RAIN},\"water_evap\":{WATER_EVAP},\"height\":[{}],\"band\":[{bands}]}}", rain.name(), weather.name(), side_law.name(), winter.name(), root_at.name(), list(&terrain.height)).unwrap();
    }
    ...
    writeln!(pop_csv, "step,pop,on_fat,fat_stock,age_mean,pop0,pop1,pop2,cross0,cross1,cross2,soil0,soil1,soil2,root0,root1,root2").unwrap();
    ...
    writeln!(places_csv, "step,place,pop,mass,hard,muscle,sensor,digestive,bite,shell,biters,cover,plant_intake,meat_intake,lineages,movers,foot,shell_front,shell_back,dead,carrion,soil,barren,regrowth,cells,rain,trees,fruit,fruit_intake,water,dry,root,root_intake").unwrap();
    ...
    for step in 1..=steps {
        ...
        if step % LINEAGE_INTERVAL == 0 {
            ...
            writeln!(pop_csv, "{step},{},{:.4},{:.1},{:.0},{},{},{},{},{},{},{:.0},{:.0},{:.0},{:.0},{:.0},{:.0}", agents.len(), agents.iter().filter(|a| a.energy <= 0.0).count() as f64 / n, agents.iter().map(|a| a.fat).sum::<f64>(), agents.iter().map(|a| a.age as f64).sum::<f64>() / n,
                at[0], at[1], at[2], cross[0], cross[1], cross[2], soil_at[0], soil_at[1], soil_at[2], root_at[0], root_at[1], root_at[2]).unwrap();
        }
        if step % LINEAGE_INTERVAL == 0 {
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            for p in 0..=N_PLACES as u8 {
                ...
                writeln!(places_csv, "{step},{},{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2},{:.3},{:.3},{:.1},{:.1},{},{movers},{:.2},{:.2},{:.2},{:.4},{:.2},{:.1},{:.4},{:.4},{},{:.4},{trees_here},{:.4},{:.1},{:.2},{:.4},{:.1},{:.4}", place_name(p), here.len(),
