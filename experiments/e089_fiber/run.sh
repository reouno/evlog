#!/bin/bash
# A run of e089 (#99), e082's line: `run.sh <world> <steps> <life> <name> [key=value ...]` puts stage C's default
...
# `fresh=0.2` overrides the default world's 0.05). The crate is e082's with e088's seed (seed_share, sprout).
#
# From the repo root:
#   (nohup bash experiments/e089_fiber/run.sh c1225 60000 9 u9.4_f0.2 unit=9.4 fresh=0.2 \
#     > experiments/e089_fiber/results/c1225_life9_u9.4_f0.2.log 2>&1 < /dev/null &)
set -e
...
out=${EVLOG_OUT:-experiments/e089_fiber/results}
mkdir -p $out
EVLOG_THREADS=1 ./target/release/e089_fiber $out/${world}_life${life}_$name $params $d11 $sets $kept life=$life steps=$steps "$@"
