//! e008: prey worth eating. The e007 world (patchy food, sexual mode) with one change: what a
//! kill is worth grows with what the prey's body cost to keep. The eater gets, on top of half
//! the prey's energy and 0.02 per block, `keep` times the upkeep the prey has paid over its life
//! (UPKEEP * mass * age). `keep` is an argument; 0 reproduces e007 byte for byte. The question is
//! whether carnivores appear once an old grazer is a prize.
...
fn main() {
    ...
    let keep: f32 = args.get(5).and_then(|s| s.parse().ok()).unwrap_or(0.0);
    let sexual = true;
    let d = D;
    ...
    let prefix = format!("experiments/e008_prey_worth/results/{size}_{}_keep{keep}_seed{seed}", if patchy { "patchy" } else { "uniform" });
    ...
    writeln!(
        ...
        "step,pop,mean_energy,mean_res,mean_genes,births,deaths_energy,deaths_age,deaths_eaten,deaths_body,escapes,plant_intake,meat_intake,\
         ...
         prey_age_mean,gain_mean,kills_young_share,meat_majority"
    )
    .unwrap();
    ...
    for step in 1..=steps {
        ...
        for i in 0..agents.len() {
            ...
            if attack > 0.0 {
                ...
                if let Some(j) = prey {
                    // Half the prey's energy, plus its body: a base per block, and `keep` times
                    // the upkeep the body has paid so far. An old body is a bigger prize.
                    let p = &agents[j];
                    let gain = MEAT_ENERGY * p.energy.max(0.0) + p.body.mass as f32 * (MEAT_MASS + keep * UPKEEP * p.age as f32);
        ...
        if step % LOG_INTERVAL == 0 || agents.is_empty() {
            ...
            writeln!(log, ",{:.0},{:.2},{:.3},{meat_majority:.3}", prey_age as f64 / kills, meat_intake as f64 / kills, kills_young as f64 / kills).unwrap();
            births = 0;
            sexual_births = 0;
