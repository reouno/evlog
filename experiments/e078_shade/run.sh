#!/bin/bash
# A run of e078 (#91 step 1): `run.sh <world> <steps> <life> <name> [key=value ...]` puts stage C's default
...
# d11) for <steps> steps on one thread, with the keys given after the name. With shade_heat and shade_dry
# 0 (and no pool) the run is e075's kept run (`ladder/*_both`) exactly.
#
...
#
# From the repo root:
#   (nohup bash experiments/e078_shade/run.sh c1225 100000 9 donor genomes=1 \
#     > experiments/e078_shade/results/c1225_life9_donor.log 2>&1 < /dev/null &)
set -e
...
out=${EVLOG_OUT:-experiments/e078_shade/results}
mkdir -p $out
EVLOG_THREADS=1 ./target/release/e078_shade $out/${world}_life${life}_$name $params $d11 $sets $kept life=$life steps=$steps "$@"
