//! e023: eyes that see far (#26). e022's closed world (the canopy, the spill, rain on the
//! mountains, mutation per base) with one law changed, about what a sensor block is:
//!
...
//! probability; 0 is e021's two per child) and `eyes` (the most cells the sensors add to the
//! range, 8 by default; 0 is e022's law). The log gets `fruit` (fallen per step),
...
fn main() {
    ...
    let prefix = format!("experiments/e023_eyes/results/{size}_sigma{sigma_name}{cell_tag}{matter_tag}_r{relief}_f{flow_rate}_{}{breath_tag}{shade_tag}{spill_tag}{mut_tag}{eyes_tag}_seed{seed}", rain.name());
