...
impl Default for Cohort {
    fn default() -> Self {
        Cohort { g: NONE, o: [Organ::default(); 5], cmp: 0.0, pa: 0.0, pb: 0.0, lpk: 0.0, lite: 1.0, damage: 0.0, h: 0.0 }
    }
}
...
pub struct Life {
    ...
    pub gen: Vec<Option<Genotype>>,
    pub egen: Vec<Option<Genotype>>,
    light: Vec<f64>,
    temp: Vec<f64>,
    stormy: Vec<bool>,
    acc: usize,
...
struct Ctx<'a> {
    ...
    ter: &'a Terrain,
    sat: &'a Sat,
    gen: &'a [Option<Genotype>],
    egen: &'a [Option<Genotype>],
    light: &'a [f64],
    temp: &'a [f64],
    stormy: &'a [bool],
    dt: f64,
...
impl Life {
    ...
    pub fn sow(&mut self, p: &Params, sa: &mut [f64], sb: &mut [f64], year: u32) -> LFlux {
        ...
        for _ in 0..p.founders as usize {
            ...
            self.gen.push(Some(Genotype::new(g, NONE, year, false)));
        }
        for _ in 0..(p.founders as usize / 4).max(1) {
            let g = genome::random(&mut self.rng, true);
            self.egen.push(Some(Genotype::new(g, NONE, year, true)));
        }
        ...
        for c in 0..n * n {
            for j in 0..E {
                ...
                if self.rng.f64() < p.eater_mut {
                    ...
                    self.egen.push(Some(Genotype::new(genes, g, year, true)));
        ...
        for c in 0..n * n {
            for i in 0..K {
                ...
                if self.rng.f64() < p.p_mut {
                    ...
                    self.gen.push(Some(Genotype::new(genes, x.g, year, false)));
                    let g = (self.gen.len() - 1) as u32;
                    let part = scale(&mut rest, 0.1);
                    self.deposit(c, g, part);
                }
                ...
                for (j, &m) in nb.iter().enumerate() {
                    ...
                    self.deposit(m, x.g, part);
                }
                self.deposit(c, x.g, rest);
            }
        }
    }
    ...
    fn establish(&mut self, p: &Params, fill: &[f64]) {
        ...
        for c in 0..n * n {
            loop {
                ...
                let mut best: Option<(usize, f64, f64)> = None; // slot, score, survival
                for j in 0..BANK {
                    ...
                    let score = e.m / ph.seed_mass * surv;
                    if best.map_or(true, |b| score > b.1) {
                        best = Some((j, score, surv));
                    }
                }
                let Some((j, _, surv)) = best else { break };
                let e = self.bank[c * BANK + j];
                let slot = match free {
                    Some(i) => i,
                    None => {
...
fn cell(
    ...
    *cover = (-p.ext * above).exp();
    ...
    // Cohorts gone below a seedling's mass die.
    for y in co.iter_mut() {
        if y.live() && y.mass() < p.m_min {
            for o in y.o {
                lit.add(o);
            }
            lit.a += y.pa;
