...
//!
//! e071 (#86) prices the cold (`cold_turn`). A body is as warm as `warm`. Each turn each face of a soft
//! block open to the air or the water (`Agent::open`) over a world cell colder than that loses `cold` x the
//! difference, a hard face loses nothing, and the fat insulates: the loss is times 1 minus the share of the
//! body's store the fat fills. The body pays it from its energy and then its fat (`main.rs`). With `cold` 0
//! none of this is read and the bodies are e070's.
...
pub struct Agent {
    ...
    pub chilled: f32, // e071: energy it lost to the cold
    pub cold_short: bool, // e071: this turn's cold was not paid in full, the upkeep was
}
...
impl Agent {
    ...
    pub fn new(id: u64, genome: Vec<u8>, keys: Vec<u16>, gene_ids: Vec<u16>, body: Body, facing: u8, energy: f64, lineage: u32) -> Agent {
        let mut a = Agent {
            ...
            wcells: [0; CELLS], tips: [[Tip::default(); SIDE_MAX]; 4], filled: [0; CELLS], n_filled: 0, bbox: [0; 4], open: [0; CELLS], water: 1.0, drank: 0, breath: 1.0, inhaled: 0, kids: 0, chilled: 0.0, cold_short: false,
        };
        a.set_body(body);
        a
    }
...
/// A body's heat (e071): each face of a soft block open to a world cell colder than `warm` loses `cold` x
/// the difference, a hard face nothing, and the fat insulates by the share of the body's store it fills.
/// Returns the energy lost this turn, in the body's units.
pub fn cold_turn(a: &Agent, g: Grid, temp: &[f64], warm: f64, cold: f64) -> f64 {
    ...
    for p in a.cells_held() {
        ...
        let (sx, sy) = a.sub_at(g, p, NORTH, 0);
        let gap = warm - temp[g.wcell(sx, sy)];
        if gap > 0.0 {
            faces += a.open[p] as f64 * gap;
        }
    }
...
mod tests {
    ...
    #[test]
    fn open_soft_faces_lose_heat_to_a_colder_cell_and_fat_insulates() {
        // World columns 0-3 are at -10 C (sub-cells x < 16), the rest at 30 C; the body is as warm as 20 C.
        let g = Grid::new(8, 8);
        let temp: Vec<f64> = (0..g.cells()).map(|c| if c % g.w < 4 { -10.0 } else { 30.0 }).collect();
        // Two muscles side by side open six faces: on the cold side 6 x 30 degrees x 0.001, on the warm nothing.
        let mut pair = [0u8; CELLS];
        pair[0] = MUSCLE as u8;
        pair[1] = MUSCLE as u8;
        let mut a = agent_with(pair, 0, 0, EAST);
        assert!((cold_turn(&a, g, &temp, 20.0, 0.001) - 0.18).abs() < 1e-9);
        a.x = 16;
        assert_eq!(cold_turn(&a, g, &temp, 20.0, 0.001), 0.0);
        // The fat insulates by the share of the store it fills: half full loses half, full nothing.
        a.x = 0;
        a.fat = 0.5 * (a.body.store * a.body.mass) as f64;
        assert!((cold_turn(&a, g, &temp, 20.0, 0.001) - 0.09).abs() < 1e-9);
        a.fat = (a.body.store * a.body.mass) as f64;
        assert_eq!(cold_turn(&a, g, &temp, 20.0, 0.001), 0.0);
        // A gut walled in by hard blocks opens no soft face, and a hard face loses nothing.
        let mut shell = [0u8; CELLS];
        shell[SIDE + 1] = DIGESTIVE as u8;
        for q in [1, SIDE, SIDE + 2, 2 * SIDE + 1] {
            shell[q] = HARD as u8;
        }
        assert_eq!(cold_turn(&agent_with(shell, 0, 0, SOUTH), g, &temp, 20.0, 0.001), 0.0);
    }
