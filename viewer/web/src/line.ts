...
export function alongLine(line: Line, step: number): number {
  let lo = 0, hi = line.path.length - 1;
  while (lo < hi) {
    const mid = (lo + hi + 1) >> 1;
    if (line.path[mid][1] <= step) lo = mid;
    else hi = mid - 1;
  }
  ...
  while (lo + 1 < line.path.length && line.path[lo][2] < step) lo++;
  return lo;
}

/** The body of `line` to follow at `step`. */
export function onLine(line: Line, step: number): number {
...
export class Walk {
  id: number;
  at: number; // the step of the last frame it has read
  ended: boolean;

  constructor(id: number, step: number) {
    this.id = id;
    this.at = step;
    this.ended = false;
  }
  ...
  follow(world: World, step: number): number | null {
    ...
    for (const s of world.steps) {
      if (s <= this.at || s > step) continue;
      this.at = s;
      const f = world.frames.get(s)!;
      if (f.births) {
        for (const [child, parent] of f.births) {
          if (parent === this.id && Math.random() < 0.5) this.id = child;
        }
      }
