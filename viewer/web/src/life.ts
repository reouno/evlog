...
class Pool {
  ...
  cap: number;
  n: number;
  sent: number; // how many instances the card has, so an empty pool is not sent again
  owner: Uint32Array | null; // which body each instance is part of, in the pools a click can find one in
  tag: number; // the body being written now

  constructor(scene: THREE.Scene, geo: THREE.BufferGeometry, mat: THREE.Material, cap: number) {
  ...
  tint(col: THREE.Color): void {
    const a = this.mesh.instanceColor!.array as Float32Array, o = this.n * 3;
  ...
  put(x: number, y: number, z: number, sx: number, sy: number, sz: number, col?: THREE.Color): void {
  ...
  putY(x: number, y: number, z: number, sx: number, sy: number, sz: number, cos: number, sin: number, col?: THREE.Color): void {
    if (this.n >= this.cap) return;
    const a = this.mesh.instanceMatrix.array as Float32Array, o = this.n * 16;
    a[o] = sx * cos; a[o + 1] = 0; a[o + 2] = -sx * sin; a[o + 3] = 0;
    a[o + 4] = 0; a[o + 5] = sy; a[o + 6] = 0; a[o + 7] = 0;
    a[o + 8] = sz * sin; a[o + 9] = 0; a[o + 10] = sz * cos; a[o + 11] = 0;
    a[o + 12] = x; a[o + 13] = y; a[o + 14] = z; a[o + 15] = 1;
    if (col !== undefined) this.tint(col);
    if (this.owner) this.owner[this.n] = this.tag;
    this.n++;
  }
  putM(m: THREE.Matrix4, col?: THREE.Color): void {
    if (this.n >= this.cap) return;
...
function rnd(c: number, k: number): number {
 ...
 * corner. */
function curve(p0: number, p1: number, p2: number, p3: number, t: number): number {
...
function pools_of(pools: (Pool | null)[], k: number): Pool {
  return (pools[k] || pools[4]) as Pool;
}
...
export class Life {
  ...
  bodyNear: number;
  detail!: number;
  near!: number; // cells: the lawn, the fallen, a tree in full
  mid!: number; // cells: a tree at all
  drawnN: number;
  drawnId: Uint32Array;
  drawnX: Float32Array;
  drawnY: Float32Array;
  ...
  setDetail(n: number): void {
    ...
    this.near = [24, 34, 46][this.detail];
    this.mid = [70, 90, 112][this.detail];
  }
  ...
  static FACE = [0, Math.PI, -Math.PI / 2, Math.PI / 2];
  ...
  static turnOf(from: number, to: number, t: number): number {
  ...
  static trackIn(out: number[], b: Frame, j: number, after: Frame | null, t: number, cell: number, w: number, d: number): number[] {
  ...
  static track(out: number[], a: Frame, i: number, b: Frame | null, j: number | undefined, t: number, before: Frame | null, after: Frame | null, cell: number, w: number, d: number): number[] {
  ...
  plants(v: Layers, at: THREE.Vector3, ground: Ground, step: number): void {
    const { w, d } = this;
  ...
  tree(c: number, px: number, pz: number, y: number, p: number, wd: number, cold: number, warm: number, near: boolean): void {
  ...
  bole(x: number, y: number, z: number, r: number, h: number, tilt: number, turn: number, col: THREE.Color): void {
    if (tilt < 0.02) {
  ...
  tuft(c: number, px: number, pz: number, y: number, p: number, cold: number, warm: number): void {
  ...
  fallen(c: number, px: number, pz: number, y: number, q: number): void {
  ...
  dead(c: number, px: number, pz: number, y: number, q: number): void {
  ...
  bodies(world: World, a: Frame | null, b: Frame | null, t: number, at: THREE.Vector3, ground: Ground, picked: number | null, before: Frame | null, after: Frame | null): void {
  ...
  under(ray: THREE.Raycaster): number | null {
  ...
  body(world: World, fr: Frame, i: number, x: number, z: number, fade: number, cell: number, at: THREE.Vector3, ground: Ground, picked: number | null, turn: number, next: Uint8Array | null, t: number): void {
