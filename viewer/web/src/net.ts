...
import { RECORD, type Header } from './wire.js';

/** What a record is handed to: its kind (`RECORD`) and its payload. */
export type OnRecord = (kind: number, payload: Uint8Array) => void;
...
export class LiveSource {
  ...
  async control(q: Record<string, string | number>): Promise<void> {
    await fetch('/control?' + new URLSearchParams(q as Record<string, string>));
  }
  setSpeed(s: number): void { this.control({ speed: s }); }
  setPaused(p: boolean): void { this.control({ paused: p ? 1 : 0 }); }
  stepOnce(): void { this.control({ once: 1 }); }
  seek(step: number): void { this.control({ skip: Math.round(step) }); }
}

/** A recorded world: frames are asked for around wherever the watcher is looking. */
export class ReplaySource {
  h: Handlers;
  readonly live: false = false;
  asked: Set<number>;
  first!: number;
  last!: number;
  stride!: number;
  ...
  async start(): Promise<void> {
    const header = (await (await fetch('/world.json')).json()) as Header;
    ...
    this.h.record(RECORD.HEADER, new TextEncoder().encode(JSON.stringify(header)));
  ...
  async fetchFrom(step: number, count = 240): Promise<void> {
...
/** Either of them: the browser plays a recording and a running world the same way. */
export type Source = LiveSource | ReplaySource;
