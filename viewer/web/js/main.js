...
let world, ground, sky, life, rig;
let renderer, scene, camera, source;
...
let plantAt = null;
let miniBase = null;
let miniTiled = null; // the same map nine times over, for a turned map
let place = null; // a cell the watcher picked out of the ground (`placePanel`)
...
function advance(dt) {
    ...
    if (source.live) {
        ...
        if (Math.abs(target - step) > 60 * stride)
            step = target;
        else
            step += (target - step) * Math.min(1, dt * 5);
    }
    else {
        if (playing)
            step += speed * dt;
        ...
        if (step <= source.first + 1 && speed < 0)
            playing = false;
    }
    world.trim(step);
}
...
function setShadows(on) {
    ...
    for (const p of life.pools)
        p.mesh.castShadow = on;
    for (const m of ground.tiles)
        m.receiveShadow = on;
    scene.traverse((o) => { const m = o.material; if (m)
        m.needsUpdate = true; });
}
...
function selection(a, b = null, t = 0) {
    ...
    if (!alive)
        ...
        if (energy < 0.05)
            now.push(fat === undefined ? '力が尽きている' : fat > 0.01 ? '力が尽き、蓄えで生きている' : '蓄えもなく、食べた分で食いつないでいる');
        else if (ripe && energy / ripe > 0.85)
            now.push('もうすぐ子を産む');
        if (born && n < born)
            now.push(`${born - n} 個${(world.params.wear ?? 0) > 0 ? '失った' : '壊された'}`);
    }
...
function lifeChart(id, upto = Infinity) {
...
function placePanel(a, swing) {
    ...
    const habitats = Array.isArray(p.habitats) ? p.habitats : null;
    const whose = on ? (on.gone ? ' · 追っている体が死んだ升' : ' · 追っている体の升') : '';
    $('placewhere').textContent = `x ${x}, y ${z} · ${h >= 0 ? '陸' : '海'}${whose}`;
    const rows = [];
    if (h >= 0)
        rows.push(['標高', relief && metres ? `${Math.round((h / relief) * metres).toLocaleString()} m` : h.toFixed(1)]);
    if (v) {
        for (const spec of world.h.layers) {
            ...
            if (spec.name === 'temperature')
                text = `${(n - tOff).toFixed(1)} °C`;
            else if (spec.name === 'habitat')
                text = habitats?.[Math.round(n)] ?? String(Math.round(n));
            else if (spec.name === 'fire')
                text = n > 0.5 ? '燃えている' : '—';
            else
                text = n < 10 ? n.toFixed(2) : Math.round(n).toLocaleString();
            rows.push([CELL_LABEL[spec.name] ?? spec.name, text]);
        }
    }
    if (world.season.on)
        rows.push(['この升の陽', world.cellSun(c, swing).toFixed(2)]);
    let here = 0;
    if (a) {
        const cell = 1 / world.sub;
        for (let i = 0; i < a.n; i++)
            if (Math.floor(a.a.x[i] * cell) === x && Math.floor(a.a.y[i] * cell) === z)
                here++;
    }
...
function minimap(a, b, t, before, after) {
    ...
    if (!miniBase) {
        ...
        for (let i = 0; i < w * d; i++) {
            ...
            cr = cr * (1 - pl) + 45 * pl * sh * 1.6;
            cg = cg * (1 - pl) + 110 * pl * sh * 1.6;
            cb = cb * (1 - pl) + 40 * pl * sh * 1.6;
            if (wa > 0) {
                const k = Math.min(0.85, wa / 200 + 0.25);
                cr = cr * (1 - k) + 40 * k;
                cg = cg * (1 - k) + 100 * k;
                cb = cb * (1 - k) + 160 * k;
            }
    ...
    if (a) {
        ...
        const cell = 1 / world.sub;
        const q = [0, 0];
        const dot = (x, z, f) => { at(x, z); g.fillRect(p[0] - 0.8 * f, p[1] - 0.8 * f, 1.6 * f, 1.6 * f); };
...
function bindUI(header) {
    for (const k of ['grass', 'trees', 'fruit', 'carrion', 'bodies']) {
        $i('v-' + k).onchange = (e) => life.setVisible(k, e.target.checked);
    }
    $i('s-terrain').oninput = (e) => { UP.terrain = +e.target.value; ground.buildHeight(); groundDue = plantsDue = true; };
    $i('s-plant').oninput = (e) => { UP.plant = +e.target.value; plantsDue = true; };
    $i('s-detail').oninput = (e) => { life.setDetail(+e.target.value); plantsDue = true; };
    $i('s-fog').oninput = (e) => { fogScale = +e.target.value; };
    // Which way the wheel zooms is a hand's own habit, so it is remembered for the next visit.
    const zoomBox = $i('v-zoomaway');
    try {
        zoomBox.checked = localStorage.getItem('evlog.zoomaway') === '1';
    }
    catch (err) { }
    ...
    zoomBox.onchange = () => {
        rig.zoomAway = zoomBox.checked;
        try {
            localStorage.setItem('evlog.zoomaway', zoomBox.checked ? '1' : '0');
        }
        catch (err) { }
    };
    ...
    $('play').onclick = () => {
        ...
        if (source.live)
            source.setPaused(!playing);
    };
    $('stepone').onclick = () => {
        if (source.live)
            source.stepOnce();
        else {
            step += world.h.stride || 1;
            playing = false;
            $('play').textContent = '▶';
        }
    };
    ...
    const fromSlider = () => {
        ...
        if (source.live)
            source.setSpeed(Math.abs(speed));
    };
    ...
    $('goto').onkeydown = (e) => { if (e.key === 'Enter')
        gotoStep(); };
    const seek = $i('seek');
    if (source.live)
        ...
        seek.min = String(rec.first);
        seek.max = String(rec.last);
        seek.step = String(world.h.stride || 1);
