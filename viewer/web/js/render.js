...
export function paint(out, mode, v, c, tempOffset) {
    const f = v[mode];
    if (!f)
        return false;
    const x = f[c];
    switch (mode) {
        case 'temperature': { // blue at -30 C, pale at 7.5, red at 35
            ...
            if (t < 7.5)
                ramp(out, COLDC, MILDC, (t + 30) / 37.5);
            else
                ramp(out, MILDC, HOTC, (t - 7.5) / 27.5);
            return true;
        }
        case 'moisture':
            if (x < 0.5)
                ramp(out, DRYC, MOISTC, x / 0.5);
            else
                ramp(out, MOISTC, WETC, (x - 0.5) / 0.5);
            return true;
        case 'humidity':
            ramp(out, PALE, RAINC, x);
            return true;
        case 'rain':
            ramp(out, PALE, RAINC, Math.sqrt(x));
            return true;
        case 'light':
            ramp(out, DARK, SUNC, x);
            return true;
        case 'grass':
            ramp(out, SAND, LAWN, Math.sqrt(x / 3));
            return true;
        case 'wood':
            ramp(out, SAND, STAND, Math.sqrt(x / 10));
            return true;
        case 'algae':
            ramp(out, PALE, ALGAEC, Math.sqrt(x));
            return true;
        case 'litter':
            ramp(out, SAND, ROT, Math.sqrt(x / 6));
            return true;
        case 'fire':
            ramp(out, EMBERC, FIREC, x);
            return true;
        case 'habitat': {
        ...
        for (let j = 0; j <= d; j++) {
            ...
            for (let i = 0; i <= w; i++) {
                ...
                if (layer) { // the four cells around the vertex, each in its own colour (a habitat is not a number to average)
                    paint(PAINT, layer, v, v0, tOff);
                    let pr = PAINT[0], pg = PAINT[1], pb = PAINT[2];
                    paint(PAINT, layer, v, v1, tOff);
                    pr += PAINT[0];
                    pg += PAINT[1];
                    pb += PAINT[2];
                    paint(PAINT, layer, v, v2, tOff);
                    pr += PAINT[0];
                    pg += PAINT[1];
                    pb += PAINT[2];
                    paint(PAINT, layer, v, v3, tOff);
                    pr += PAINT[0];
                    pg += PAINT[1];
                    pb += PAINT[2];
                    r = pr / 4;
...
export class Rig {
    ...
    constructor(camera, dom, world) {
        ...
        dom.addEventListener('pointermove', (e) => {
            ...
            drag.y = e.clientY;
            if (drag.pan)
                this.pan(-dx * this.dist * 0.0016, dy * this.dist * 0.0016); // the ground follows the hand
            else {
