...
class Pool {
     ...
     * the cost of a frame that rebuilt the plants, and it is the size of the pool, not of the work. */
        ...
        if (this.n === 0 && this.sent === 0)
            return; // nothing there, and nothing was there before
        this.sent = this.n;
        m.clearUpdateRanges();
...
export class Life {
    ...
    static track(out, a, i, b, j, t, before, after, cell, w, d) {
        ...
        if (before) {
            const k = before.index.get(id);
            if (k !== undefined) {
                x0 = x1 + Life.wrap(before.a.x[k] * cell - x1, w);
                z0 = z1 + Life.wrap(before.a.y[k] * cell - z1, d);
            }
        }
        if (after) {
            const k = after.index.get(id);
            if (k !== undefined) {
                x3 = x1 + Life.wrap(after.a.x[k] * cell - x1, w);
                z3 = z1 + Life.wrap(after.a.y[k] * cell - z1, d);
            }
        }
    ...
    plants(v, at, ground, step) {
        ...
        for (let dz = -this.mid; dz <= this.mid; dz++) {
            ...
            for (let dx = -this.mid; dx <= this.mid; dx++) {
                ...
                if (wd <= 0.002 && (!fruit || fruit[c] <= 0) && (!carrion || carrion[c] <= 0))
                    continue;
                ...
                if (wd >= this.treeMin)
                    this.tree(c, px, pz, y, p, wd, cold, warm, near);
                else if (p > 0.002 && near)
                    this.tuft(c, px, pz, y, p, cold, warm);
                if (near && fruit && fruit[c] > 0.001)
                    this.fallen(c, px, pz, y, fruit[c]);
                if (near && carrion && carrion[c] > 0.002)
                    this.dead(c, px, pz, y, carrion[c]);
            }
        }
    ...
    tree(c, px, pz, y, p, wd, cold, warm, near) {
        ...
        if (rot > 0)
            bark.lerp(DEADW, Math.min(1, rot * 1.3));
        ...
        if (cold > 0)
            LEAF.lerp(kind === 1 ? EVER : AMBER, Math.min(1, cold / 0.3) * (kind === 1 ? 0.55 : 0.8));
        if (snow > 0)
            LEAF.lerp(SNOW, snow * 0.3);
        ...
        if (leaf < 0.14)
            return; // bare: the trunk and its limbs are the whole tree now
    ...
    tuft(c, px, pz, y, p, cold, warm) {
        ...
        if (warm > 0)
            TINT.lerp(LUSH, warm * 0.3);
        if (cold > 0)
            TINT.lerp(HAY, Math.min(0.9, cold * 1.2));
        if (snow > 0)
            TINT.lerp(SNOW, snow * 0.42);
    ...
    under(ray) {
        ...
        let first = null;
        for (const h of hits)
            if (first === null || h.distance < first.distance)
                first = h;
        if (first === null)
            return null;
